...
class sfException extends Exception {
	...
	static protected $lastException = null;
	...
	static public function createFromException(Exception $e) {
		$exception = new sfException(
				sprintf('Wrapped %s: %s', get_class($e), $e->getMessage()));
	...
	static protected function outputStackTrace(Exception $exception) {
		...
		if (class_exists('sfContext', false) && sfContext::hasInstance()
			...
			if (sfConfig::get('sf_logging_enabled')) {
				$dispatcher
						->notify(
								new sfEvent($exception, 'application.log',
		...
		$message = null === $exception->getMessage() ? 'n/a'
		...
		if (class_exists('sfContext', false) && sfContext::hasInstance()) {
			...
			$requestTable = self::formatArrayAsHtml(
					sfDebug::requestAsArray($context->getRequest()));
			$responseTable = self::formatArrayAsHtml(
					sfDebug::responseAsArray($context->getResponse()));
			$userTable = self::formatArrayAsHtml(
					sfDebug::userAsArray($context->getUser()));
		...
		if ($template = self::getTemplatePathForError($format, true)) {
			if (isset($dispatcher)) {
				...
				$event = $dispatcher
						->filter(
								new sfEvent($response,
										'response.filter_content'), $content);
	...
	static public function getTemplatePathForError($format, $debug) {
		$templatePaths = array(sfConfig::get('sf_app_config_dir') . '/error',
				sfConfig::get('sf_config_dir') . '/error',
				dirname(__FILE__) . '/data',);

		$template = sprintf('%s.%s.php', $debug ? 'exception' : 'error',
				$format);
	...
	static protected function getTraces($exception, $format = 'txt') {
		...
		array_unshift($traceData,
				array('function' => '',
						'file' => $exception->getFile() != null ? $exception
										->getFile() : null,
						'line' => $exception->getLine() != null ? $exception
										->getLine() : null,
						'args' => array(),));
		...
		for ($i = 0, $count = count($traceData); $i < $count; $i++) {
			$line = isset($traceData[$i]['line']) ? $traceData[$i]['line']
					: null;
			$file = isset($traceData[$i]['file']) ? $traceData[$i]['file']
					: null;
			...
			$traces[] = sprintf($lineFormat,
					...
					self::formatFile($file, $line, $format,
							null === $file ? 'n/a'
									: sfDebug::shortenFilePath($file)),
					null === $line ? 'n/a' : $line, 'trace_' . $i, 'trace_'
							. $i, $i == 0 ? 'block' : 'none',
	 ...
	 * @return string
	 */
	static protected function formatArgs($args, $single = false,
			$format = 'html') {
		$result = array();
		...
		foreach ($args as $key => $value) {
			if (is_object($value)) {
				$formattedValue = ($format == 'html' ? '<em>object</em>'
						: 'object') . sprintf("('%s')", get_class($value));
			} else if (is_array($value)) {
				$formattedValue = ($format == 'html' ? '<em>array</em>'
						: 'array') . sprintf("(%s)", self::formatArgs($value));
			} else if (is_string($value)) {
				$formattedValue = ($format == 'html' ? sprintf("'%s'",
			...
			$result[] = is_int($key) ? $formattedValue
					: sprintf("'%s' => %s", self::escape($key), $formattedValue);
		}
	 ...
	 * @return string
	 */
	static protected function formatFile($file, $line, $format = 'html',
			$text = null) {
		if (null === $text) {
			$text = $file;
		}

		if ('html' == $format && $file && $line
				&& $linkFormat = sfConfig::get('sf_file_link_format',
