...
class sfSimpleAutoload {
	...
	protected $cacheFile = null, $cacheLoaded = false, $cacheChanged = false, $dirs = array(), $files = array(), $classes = array(), $overriden = array();
	...
	public function loadCache() {
		...
		list($this->classes, $this->dirs, $this->files) = unserialize(
		...
		if ($this->cacheChanged) {
			if (is_writable(dirname($this->cacheFile))) {
				file_put_contents($this->cacheFile,
						serialize(
								array($this->classes, $this->dirs, $this->files)));
			}
	...
	public function addFile($file, $register = true) {
		...
		preg_match_all(
				...
				file_get_contents($file), $classes);
