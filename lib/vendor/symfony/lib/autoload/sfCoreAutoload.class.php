...
class sfCoreAutoload {
	...
	static public function make() {
		$libDir = str_replace(DIRECTORY_SEPARATOR, '/',
				realpath(dirname(__FILE__) . DIRECTORY_SEPARATOR . '..'));
		require_once $libDir . '/util/sfFinder.class.php';

		$files = sfFinder::type('file')->prune('plugins')->prune('vendor')
				->prune('skeleton')->prune('default')->prune('helper')
				->name('*.php')->in($libDir);
		...
		foreach ($files as $file) {
			...
			$class = basename($file,
					false === strpos($file, '.class.php') ? '.php'
							: '.class.php');
			...
			if (false !== stripos($contents, 'class ' . $class)
				...
				$classes .= sprintf("    '%s' => '%s',\n", strtolower($class),
						substr(str_replace($libDir, '', $file), 1));
			}
		}
	...
	protected $classes = array('sfaction' => 'action/sfAction.class.php',
