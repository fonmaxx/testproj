 ...
 */
class sfAutoload {
	static protected $freshCache = false, $instance = null;

	protected $overriden = array(), $classes = array();
	...
	public function reloadClasses($force = false) {
		...
		if (!$configuration
				|| !$configuration instanceof sfApplicationConfiguration) {
			return false;
		}

		self::$freshCache = true;
		if (file_exists(
				$configuration->getConfigCache()
						->getCacheName('config/autoload.yml'))) {
		...
		$file = $configuration->getConfigCache()
				->checkConfig('config/autoload.yml');
