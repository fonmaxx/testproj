...
abstract class sfWebController extends sfController {
	...
	public function convertUrlStringToParameters($url) {
		...
		if ($url[0] == '@') {
			...
			throw new InvalidArgumentException(
					sprintf(
							'An internal URI must contain a module and an action (module/action) ("%s" given).',
							$givenUrl));
		}
      ...
      /x', $queryString, $matches, PREG_SET_ORDER | PREG_OFFSET_CAPTURE);
			...
			if (!$matched) {
				throw new sfParseException(
						sprintf('Unable to parse query string "%s".',
								$queryString));
			}
		}
	...
	public function redirect($url, $delay = 0, $statusCode = 302) {
		...
		$response
				->setContent(
						sprintf(
								'<html><head><meta http-equiv="refresh" content="%d;url=%s"/></head></html>',
								$delay,
