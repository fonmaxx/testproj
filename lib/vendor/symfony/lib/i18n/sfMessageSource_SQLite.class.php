...
class sfMessageSource_SQLite extends sfMessageSource_Database {
	...
	protected function getCatalogueDetails($catalogue = 'messages') {
		...
		$rs = sqlite_query(
				"SELECT cat_id FROM catalogue WHERE name = '{$name}'", $db);
	...
	protected function updateCatalogueTime($cat_id, $variant, $db) {
		...
		$result = sqlite_query(
				"UPDATE catalogue SET date_modified = {$time} WHERE cat_id = {$cat_id}",
				$db);
	...
	function save($catalogue = 'messages') {
		...
		foreach ($messages as $message) {
			...
			if (sqlite_query(
					"INSERT INTO trans_unit (cat_id, id, source, date_added) VALUES ({$cat_id}, {$count}, '{$message}', $time)",
					$db)) {
				$count++;
				$inserted++;
			}
	...
	function update($text, $target, $comments, $catalogue = 'messages') {
		...
		sqlite_query(
				"UPDATE trans_unit SET target = '{$target}', comments = '{$comments}', date_modified = '{$time}' WHERE cat_id = {$cat_id} AND source = '{$text}'",
				$db);
	...
	function delete($message, $catalogue = 'messages') {
		...
		sqlite_query(
				"DELETE FROM trans_unit WHERE cat_id = {$cat_id} AND source = '{$message}'",
				$db);
