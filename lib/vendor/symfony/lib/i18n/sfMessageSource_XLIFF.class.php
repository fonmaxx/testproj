...
class sfMessageSource_XLIFF extends sfMessageSource_File {
	...
	protected function createDOMDocument($xml = null) {
		...
		$doctype = $domimp
				->createDocumentType('xliff', '-//XLIFF//DTD XLIFF//EN',
	...
	public function save($catalogue = 'messages') {
		...
		if (is_writable($filename) == false) {
			throw new sfException(
					sprintf(
							"Unable to save to file %s, file must be writable.",
							$filename));
		}
		...
		$lastNodes = $xpath
				->query(
						'//trans-unit[not(@id <= preceding-sibling::trans-unit/@id) and not(@id <= following-sibling::trans-unit/@id)]');
	...
	public function update($text, $target, $comments, $catalogue = 'messages') {
		...
		if (is_writable($filename) == false) {
			throw new sfException(
					sprintf(
							"Unable to update file %s, file must be writable.",
							$filename));
		}
	...
	public function delete($message, $catalogue = 'messages') {
		...
		if (is_writable($filename) == false) {
			throw new sfException(
					sprintf(
							"Unable to modify file %s, file must be writable.",
							$filename));
		}
