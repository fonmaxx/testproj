...
class sfDateFormat {
	 ...
	 * @var array 
	 */
	protected $tokens = array('G' => 'Era', 'y' => 'year', 'M' => 'mon',
			'd' => 'mday', 'h' => 'Hour12', 'H' => 'hours', 'm' => 'minutes',
			's' => 'seconds', 'E' => 'wday', 'D' => 'yday',
			'F' => 'DayInMonth', 'w' => 'WeekInYear', 'W' => 'WeekInMonth',
			'a' => 'AMPM', 'k' => 'HourInDay', 'K' => 'HourInAMPM',
			'z' => 'TimeZone');
				...
				} else // otherwise, default:
 {
			...
			if ($isString) {
				...
				if ($numericalTime === false) {
					throw new sfException(
							sprintf(
									'Impossible to parse date "%s" with format "%s".',
									$time, $pattern));
				}
			} else {
				$numericalTime = $time;
	 ...
	 */
	public function format($time, $pattern = 'F', $inputPattern = null,
			$charset = 'UTF-8') {
	...
	public function getPattern($pattern) {
		if (is_array($pattern) && count($pattern) == 2) {
			return $this->formatInfo
					->formatDateTime($this->getPattern($pattern[0]),
		...
		case 'f':
			return $this->formatInfo
					->formatDateTime($this->formatInfo->LongDatePattern,
		...
		case 'g':
			return $this->formatInfo
					->formatDateTime($this->formatInfo->ShortDatePattern,
							$this->formatInfo->ShortTimePattern);
			break;
		case 'G':
			return $this->formatInfo
	...
	protected function getUnixDate($date) {
		return getdate(
				mktime($date['hours'], $date['minutes'], $date['seconds'],
	...
	protected function getYear($date, $pattern = 'yyyy') {
		...
		default:
			throw new sfException(
					'The pattern for year is either "y", "yy", "yyy" or "yyyy".');
		}
	}
	...
	protected function getMon($date, $pattern = 'M') {
		...
		default:
			throw new sfException(
					'The pattern for month is "M", "MM", "MMM", "MMMM", "MMMMM".');
		}
	}
	...
	protected function getWday($date, $pattern = 'EEEE') {
		...
		default:
			throw new sfException(
					'The pattern for day of the week is "E", "EE", "EEE", or "EEEE".');
		}
	}
	...
	protected function getMday($date, $pattern = 'd') {
		...
		default:
			throw new sfException(
					'The pattern for day of the month is "d", "dd" or "dddd".');
		}
	}
	...
	protected function getHours($date, $pattern = 'H') {
		...
		default:
			throw new sfException(
					'The pattern for 24 hour format is "H" or "HH".');
		}
	}
	...
	protected function getDayInMonth($date, $pattern = 'FF') {
		...
		case 'F':
			return @date('j',
					@mktime(0, 0, 0, $date['mon'], $date['mday'], $date['year']));
			break;
		case 'FF':
			return @date('d',
					@mktime(0, 0, 0, $date['mon'], $date['mday'], $date['year']));
		...
		default:
			throw new sfException(
					'The pattern for day in month is "F" or "FF".');
		}
	}
	...
	protected function getWeekInMonth($date, $pattern = 'W') {
		...
		return @date('W',
				@mktime(0, 0, 0, $date['mon'], $date['mday'], $date['year']))
				- date('W', mktime(0, 0, 0, $date['mon'], 1, $date['year']));
	}
