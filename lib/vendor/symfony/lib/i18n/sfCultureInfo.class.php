...
class sfCultureInfo {
	...
	public function __get($name) {
		...
		if (in_array($getProperty, $this->properties)) {
			...
			throw new sfException(
					sprintf('Property %s does not exists.', $name));
		}
	}
	...
	protected function setCulture($culture) {
		if (!empty($culture)) {
			if (!preg_match('/^[a-z]{2}(_[A-Z]{2,5}){0,2}$/', $culture)) {
				throw new sfException(
						sprintf('Invalid culture supplied: %s', $culture));
			}
		}
	...
	protected function loadCultureData($culture) {
		...
		foreach ($files as $file) {
			...
			if (is_file($filename) == false) {
				throw new sfException(
						sprintf('Data file for "%s" was not found.', $file));
			}
	...
	public function getNumberFormat() {
		if (null === $this->numberFormat) {
			...
			$data = array('NumberElements' => $elements,
					'NumberPatterns' => $patterns, 'Currencies' => $currencies);
	...
	static function getCultures($type = sfCultureInfo::ALL) {
		...
		while (false !== ($entry = $dir->read())) {
			if (is_file($dataDir . $entry) && substr($entry, -4) == $dataExt
					&& $entry != 'root' . $dataExt) {
	...
	public function getCountry($code) {
		...
		if (!isset($countries[$code])) {
			throw new InvalidArgumentException(
					sprintf('The country %s does not exist.', $code));
		}
	...
	public function getCurrency($code) {
		...
		if (!isset($currencies[$code])) {
			throw new InvalidArgumentException(
					sprintf('The currency %s does not exist.', $code));
		}
	...
	public function getLanguage($code) {
		...
		if (!isset($languages[$code])) {
			throw new InvalidArgumentException(
					sprintf('The language %s does not exist.', $code));
		}
	...
	public function getCountries($countries = null) {
		...
		if (null !== $countries) {
			if ($problems = array_diff($countries, array_keys($allCountries))) {
				throw new InvalidArgumentException(
						sprintf('The following countries do not exist: %s.',
								implode(', ', $problems)));
			}

			$allCountries = array_intersect_key($allCountries,
	...
	public function getCurrencies($currencies = null, $full = false) {
		...
		if (null !== $currencies) {
			if ($problems = array_diff($currencies, array_keys($allCurrencies))) {
				throw new InvalidArgumentException(
						sprintf('The following currencies do not exist: %s.',
								implode(', ', $problems)));
			}

			$allCurrencies = array_intersect_key($allCurrencies,
					array_flip($currencies));
		}
	...
	public function getLanguages($languages = null) {
		...
		if (null !== $languages) {
			if ($problems = array_diff($languages, array_keys($allLanguages))) {
				throw new InvalidArgumentException(
						sprintf('The following languages do not exist: %s.',
								implode(', ', $problems)));
			}

			$allLanguages = array_intersect_key($allLanguages,
	...
	public function getTimeZones() {
		...
		foreach ($timeZones as $key => $value) {
			...
			$timeZones[$key]['city'] = str_replace('_', ' ',
