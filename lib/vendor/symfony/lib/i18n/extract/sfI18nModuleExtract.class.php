...
class sfI18nModuleExtract extends sfI18nExtract {
	...
	public function configure() {
		if (!isset($this->parameters['module'])) {
			throw new sfException(
					'You must give a "module" parameter when extracting for a module.');
		}

		$this->module = $this->parameters['module'];

		$options = $this->i18n->getOptions();
		$dirs = $this->i18n->isMessageSourceFileBased($options['source']) ? $this
						->i18n->getConfiguration()->getI18NDirs($this->module)
				: null;
	...
	public function extract() {
		...
		$this
				->extractFromPhpFiles(
						array($moduleDir . '/actions', $moduleDir . '/lib',
								$moduleDir . '/templates',));
		...
		if (file_exists($generator)) {
			...
			$this
					->updateMessages(
							$yamlExtractor
									->extract(file_get_contents($generator)));
		}
			...
			foreach ($validateFiles as $validateFile) {
				...
				$this
						->updateMessages(
								$yamlExtractor
										->extract(
												file_get_contents($validateFile)));
			}
		}
	}
}
