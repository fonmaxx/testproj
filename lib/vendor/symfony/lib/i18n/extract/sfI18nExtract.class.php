...
abstract class sfI18nExtract {
	protected $currentMessages = array(), $newMessages = array(), $allSeenMessages = array(), $culture = null, $parameters = array(), $i18n = null;
	...
	final public function getNewMessages() {
		return array_diff($this->getAllSeenMessages(),
	...
	final public function getOldMessages() {
		return array_diff($this->getCurrentMessages(),
				$this->getAllSeenMessages());
	}
	...
	protected function extractFromPhpFiles($dir) {
		...
		foreach ($files->in($dir) as $file) {
			$messages = array_merge($messages,
					$phpExtractor->extract(file_get_contents($file)));
		}
	...
	protected function updateMessages($messages) {
		$this->allSeenMessages = array_unique(
				array_merge($this->allSeenMessages, $messages));
	}
}
