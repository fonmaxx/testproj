 ...
 */
class sfI18N {
	protected $configuration = null, $dispatcher = null, $cache = null, $options = array(), $culture = 'en', $messageSource = null, $messageFormat = null;
	...
	public function initialize(sfApplicationConfiguration $configuration,
		...
		$this->options = array_merge(
				array('source' => 'XLIFF', 'debug' => false,
						'database' => 'default',
						'untranslated_prefix' => '[T]',
						'untranslated_suffix' => '[/T]',), $options);

		$this->dispatcher
				->connect('user.change_culture',
						array($this, 'listenToChangeCultureEvent'));

		if ($this->isMessageSourceFileBased($this->options['source'])) {
			$this->dispatcher
					->connect('controller.change_action',
							array($this, 'listenToChangeActionEvent'));
		}
	}
	...
	public function setMessageSource($dirs, $culture = null) {
		if (null === $dirs) {
			...
			$this->messageSource = sfMessageSource::factory('Aggregate',
					array_map(array($this, 'createMessageSource'), $dirs));
		}
	...
	public function createMessageSource($dir = null) {
		return sfMessageSource::factory($this->options['source'],
				self::isMessageSourceFileBased($this->options['source']) ? $dir
	...
	public function setCulture($culture) {
		...
		setlocale(LC_ALL, 'en_US.utf8', 'en_US.UTF8', 'en_US.utf-8',
				'en_US.UTF-8');
		setlocale(LC_COLLATE, $culture . '.utf8', $culture . '.UTF8',
				$culture . '.utf-8', $culture . '.UTF-8');
		setlocale(LC_CTYPE, $culture . '.utf8', $culture . '.UTF8',
				$culture . '.utf-8', $culture . '.UTF-8');
		setlocale(LC_MONETARY, $culture . '.utf8', $culture . '.UTF8',
				$culture . '.utf-8', $culture . '.UTF-8');
		setlocale(LC_TIME, $culture . '.utf8', $culture . '.UTF8',
				$culture . '.utf-8', $culture . '.UTF-8');
	...
	public function getMessageSource() {
		if (!isset($this->messageSource)) {
			$dirs = ($this->isMessageSourceFileBased($this->options['source'])) ? $this
							->configuration->getI18NGlobalDirs() : null;
	...
	public function getMessageFormat() {
		if (!isset($this->messageFormat)) {
			$this->messageFormat = new sfMessageFormat(
					$this->getMessageSource(), sfConfig::get('sf_charset'));

			if ($this->options['debug']) {
				$this->messageFormat
						->setUntranslatedPS(
								array($this->options['untranslated_prefix'],
	...
	public function getCountry($iso, $culture = null) {
		$c = sfCultureInfo::getInstance(
				null === $culture ? $this->culture : $culture);
	...
	public function getTimestampForCulture($dateTime, $culture = null) {
		list($day, $month, $year) = $this
				->getDateForCulture($dateTime,
						null === $culture ? $this->culture : $culture);
		list($hour, $minute) = $this
				->getTimeForCulture($dateTime,
						null === $culture ? $this->culture : $culture);

		return null === $day ? null
	...
	public function getDateForCulture($date, $culture = null) {
		...
		if (preg_match("~$dateRegexp~", $date, $matches)) {
			...
			return array($matches[$datePositions['d']],
					$matches[$datePositions['m']],
					$matches[$datePositions['y']]);
		} else {
			return null;
		}
	}
	...
	public function getTimeForCulture($time, $culture = null) {
		...
		$timeRegexp = preg_replace(array('/[hm]+/i', '/a/'),
				array('(\d+)', '(\w+)'), preg_quote($timeFormat));

		// We parse time format to see where things are (h, m)
		$timePositions = array(
				'h' => strpos($timeFormat, 'H') !== false ? strpos(
								$timeFormat, 'H') : strpos($timeFormat, 'h'),
		...
		if (preg_match("~$timeRegexp~", $time, $matches)) {
			...
			if ($timePositions['a'] !== false) {
				...
				} else if (strcasecmp($matches[$timePositions['a']],
						$timeFormatInfo->getPMDesignator()) == 0) {
	...
	static public function isMessageSourceFileBased($source) {
		...
		return class_exists($class)
				&& is_subclass_of($class, 'sfMessageSource_File');
	}
	...
	public function listenToChangeActionEvent(sfEvent $event) {
		...
		$this
				->setMessageSource(
						$this->configuration->getI18NDirs($event['module']));
	}
