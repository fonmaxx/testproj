...
class sfMessageFormat {
	 ...
	 */
	public function format($string, $args = array(), $catalogue = null,
			$charset = null) {
		...
		$s = $this
				->formatString(sfToolkit::I18N_toUTF8($string, $charset),
						$args, $catalogue);
	...
	protected function formatString($string, $args = array(), $catalogue = null) {
		...
		if (empty($catalogue)) {
			$catalogue = empty($this->catalogue) ? 'messages' : $this
					->catalogue;
		}
		...
		return $this->postscript[0] . $this->replaceArgs($string, $args)
				. $this->postscript[1];
	}
