...
class sfNumberFormatInfo {
	...
	function setPattern($type = sfNumberFormatInfo::DECIMAL) {
		if (is_int($type)) {
			$this->pattern = $this
					->parsePattern($this->data['NumberPatterns'][$type]);
		} else {
			$this->pattern = $this->parsePattern($type);
		}

		$this->pattern['negInfty'] = $this->data['NumberElements'][6]
				. $this->data['NumberElements'][9];

		$this->pattern['posInfty'] = $this->data['NumberElements'][11]
				. $this->data['NumberElements'][9];
	}
	...
	protected function parsePattern($pattern) {
		...
		$digitPattern = is_int($decimalPos) ? substr($pattern, 0, $decimalPos)
				: $pattern;
