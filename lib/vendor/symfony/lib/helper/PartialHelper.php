...
function get_component_slot($name, $vars = array()) {
	...
	if (!$viewInstance->hasComponentSlot($name)) {
		...
		throw new sfConfigurationException(
				sprintf('The component slot "%s" is not set.', $name));
	}
...
function get_component($moduleName, $componentName, $vars = array()) {
	...
	require($context->getConfigCache()
			->checkConfig('modules/' . $moduleName . '/config/module.yml'));

	$class = sfConfig::get(
			'mod_' . strtolower($moduleName) . '_partial_view_class', 'sf')
			. 'PartialView';
	$view = new $class($context, $moduleName, $actionName, '');
	$view
			->setPartialVars(
					true === sfConfig::get('sf_escaping_strategy') ? sfOutputEscaper::unescape(
									$vars) : $vars);
...
function get_partial($templateName, $vars = array()) {
	...
	if (false !== $sep = strpos($templateName, '/')) {
		...
		$moduleName = $context->getActionStack()->getLastEntry()
				->getModuleName();
	}
...
function slot($name, $value = null) {
	...
	if (sfConfig::get('sf_logging_enabled')) {
		$context->getEventDispatcher()
				->notify(
						new sfEvent(null, 'application.log',
								array(sprintf('Set slot "%s"', $name))));
	}
...
function has_slot($name) {
	return array_key_exists($name,
			sfContext::getInstance()->getResponse()->getSlots());
}
...
function _call_component($moduleName, $componentName, $vars) {
	...
	if (!$controller->componentExists($moduleName, $componentName)) {
		...
		throw new sfConfigurationException(
				sprintf('The component does not exist: "%s", "%s".',
	...
	$componentInstance->getVarHolder()
			->add(
					true === sfConfig::get('sf_escaping_strategy') ? sfOutputEscaper::unescape(
									$vars) : $vars);
