...
class sfPearEnvironment {
	protected $dispatcher = null, $config = null, $registry = null, $rest = null, $frontend = null, $options = array();
	...
	public function initialize(sfEventDispatcher $dispatcher,
		...
		if (!isset($options['plugin_dir'])) {
			throw new sfConfigurationException(
					'You must provide a "plugin_dir" option.');
		}

		if (!isset($options['cache_dir'])) {
			throw new sfConfigurationException(
					'You must provide a "cache_dir" option.');
		}
		...
		$this
				->initializeConfiguration($options['plugin_dir'],
						$options['cache_dir']);
		...
		$this->rest = new sfPearRestPlugin($this->config,
				array('base_class' => $options['rest_base_class']));
		...
		if (!$this->registry->channelExists($channel, true)) {
			...
			if (!$downloader->discover($channel)) {
				throw new sfPluginException(
						sprintf('Unable to register channel "%s"', $channel));
			}
		}
	...
	protected function initializeFrontend() {
		...
		if (PEAR::isError($this->frontend)) {
			throw new sfPluginException(
					sprintf('Unable to initialize PEAR Frontend object: %s',
							$this->frontend->getMessage()));
		}
	...
	protected function initializeRegistry() {
		...
		if (PEAR::isError($this->registry)) {
			throw new sfPluginException(
					sprintf('Unable to initialize PEAR registry: %s',
							$this->registry->getMessage()));
		}
	}
	...
	public function initializeConfiguration($pluginDir, $cacheDir) {
		...
		if ($this->hasOption('preferred_state')) {
			$this->config
					->set('preferred_state',
							$this->getOption('preferred_state'));
		}
