...
class sfPearRestPlugin extends sfPearRest11 {
	protected $config, $rest10, $restBase, $channel;
	...
	protected function getRESTBase($channelName) {
		...
		if (PEAR::isError($channel)) {
			throw new sfPluginException(
					sprintf('Unable to initialize channel "%s"',
							$channel->getMessage()));
		}
		...
		if (!$channel->supportsREST($mirror)) {
			throw new sfPluginRestException(
					sprintf(
							'The channel "%s" does not support the REST protocol',
							$channelName));
		}
	...
	public function getPluginLicense($plugin, $version) {
		...
		if (PEAR::isError($info)) {
			throw new sfPluginRestException(
					sprintf(
							'Unable to get plugin licence information for plugin "%s": %s',
							$plugin, $info->getMessage()));
		}
	...
	public function getPluginVersions($plugin, $stability = null) {
		$allreleases = $this->_rest
				->retrieveData(
						$this->restBase . 'r/' . strtolower($plugin)
								. '/allreleases.xml');
		if (PEAR::isError($allreleases)) {
			throw new sfPluginRestException(
					sprintf('Unable to get information for plugin "%s": %s',
							$plugin, $allreleases->getMessage()));
		}

		if (!isset($allreleases['r'])
				|| (isset($allreleases['r']) && !is_array($allreleases['r'])
						|| !count($allreleases['r']))) {
			throw new sfPluginRestException(
					sprintf('No release available for plugin "%s"', $plugin));
		}
		...
		if (!count($versions)) {
			throw new sfPluginException(
					sprintf(
							'No release available for plugin "%s" in state "%s"',
							$plugin, $stability));
		}
	...
	public function getPluginDependencies($plugin, $version) {
		$dependencies = $this->_rest
				->retrieveData(
						$this->restBase . 'r/' . strtolower($plugin) . '/deps.'
								. $version . '.txt');
		if (PEAR::isError($dependencies)) {
			throw new sfPluginRestException(
					sprintf(
							'Unable to get dependencies information for plugin "%s": %s',
							$plugin, $dependencies->getMessage()));
		}
	...
	public function getPluginDownloadURL($plugin, $version, $stability) {
		$installed = $this->config->getRegistry()
				->packageInfo($plugin, 'version', $this->channel);
		if ($installed >= $version) {
			throw new sfPluginException(
					sprintf(
							'Plugin "%s" version "%s" is already installed (you tried to install version "%s")',
							$plugin, $installed, $version));
		}

		$info = $this
				->getDownloadURL($this->restBase,
						array('channel' => $this->channel,
								'package' => $plugin, 'version' => $version),
						$stability, $installed);
		if (PEAR::isError($info)) {
			throw new sfPluginRestException(
					sprintf(
							'Unable to get download information for plugin "%s | %s | %s": %s',
							$plugin, $version, $stability, $info->getMessage()));
		}

		if (!isset($info['url'])) {
			throw new sfPluginRestException(
					sprintf('Plugin "%s" cannot be installed (No URL found)',
							$plugin));
		}
	...
	protected function getAllowedStates($stability = null) {
		$stability = null === $stability ? $this->config
						->get('preferred_state', null, $this->channel)
				: $stability;
	...
	public function __call($method, $arguments) {
		if (method_exists($this->rest10, $method)) {
			return call_user_func_array(array($this->rest10, $method),
					$arguments);
		}
	}
}
