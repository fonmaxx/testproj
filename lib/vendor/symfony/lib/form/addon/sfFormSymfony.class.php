...
class sfFormSymfony extends sfForm {
	 ...
	 */
	static public function setEventDispatcher(
	...
	protected function doBind(array $values) {
		if (self::$dispatcher) {
			$values = self::$dispatcher
					->filter(new sfEvent($this, 'form.filter_values'), $values)
					->getReturnValue();
		}
		...
		try {
			...
			if (self::$dispatcher) {
				self::$dispatcher
						->notify(
								new sfEvent($this, 'form.validation_error',
										array('error' => $error)));
			}
	...
	public function __call($method, $arguments) {
		if (self::$dispatcher) {
			$event = self::$dispatcher
					->notifyUntil(
							new sfEvent($this, 'form.method_not_found',
									array('method' => $method,
											'arguments' => $arguments)));
		...
		throw new sfException(
				sprintf('Call to undefined method %s::%s.', get_class($this),
						$method));
	}
}
