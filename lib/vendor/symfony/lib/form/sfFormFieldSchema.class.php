 ...
 */
class sfFormFieldSchema extends sfFormField implements ArrayAccess, Iterator,
		Countable {
	protected $count = 0, $fieldNames = array(), $fields = array();
	 ...
	 */
	public function __construct(sfWidgetFormSchema $widget,
		...
		foreach ($this as $name => $field) {
			if ($field instanceof sfFormFieldSchema && $recursive) {
				$fields = array_merge($fields,
						$field->getHiddenFields($recursive));
		...
		if (!isset($this->fields[$name])) {
			...
			if ($widget instanceof sfWidgetFormSchema) {
				...
				if ($error && !$error instanceof sfValidatorErrorSchema) {
					$error = new sfValidatorErrorSchema(
							$error->getValidator(), array($error));
				}
			} else {
				$class = 'sfFormField';
			}

			$this->fields[$name] = new $class($widget, $this, $name,
					isset($this->value[$name]) ? $this->value[$name] : null,
					$error);
		}
