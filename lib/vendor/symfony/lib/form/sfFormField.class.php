...
class sfFormField {
	...
	protected $widget = null, $parent = null, $name = '', $value = null, $error = null;
	 ...
	 */
	public function __construct(sfWidgetForm $widget,
			sfFormField $parent = null, $name, $value,
	...
	function render($attributes = array()) {
		if ($this->parent) {
			return $this->parent->getWidget()
					->renderField($this->name, $this->value, $attributes,
							$this->error);
		} else {
			return $this->widget
					->render($this->name, $this->value, $attributes,
							$this->error);
		}
	}
	 ...
	 */
	public function renderRow($attributes = array(), $label = null,
			$help = null) {
		if (null === $this->parent) {
			throw new LogicException(
					sprintf('Unable to render the row for "%s".', $this->name));
		}

		$field = $this->parent->getWidget()
				->renderField($this->name, $this->value,
						!is_array($attributes) ? array() : $attributes,
						$this->error);

		$error = $this->error instanceof sfValidatorErrorSchema ? $this->error
						->getGlobalErrors() : $this->error;

		$help = null === $help ? $this->parent->getWidget()
						->getHelp($this->name) : $help;

		return strtr(
				$this->parent->getWidget()->getFormFormatter()
						->formatRow($this->renderLabel($label), $field, $error,
								$help), array('%hidden_fields%' => ''));
	}
	...
	public function renderError() {
		if (null === $this->parent) {
			throw new LogicException(
					sprintf('Unable to render the error for "%s".', $this->name));
		}

		$error = $this->getWidget() instanceof sfWidgetFormSchema ? $this
						->getWidget()->getGlobalErrors($this->error)
				: $this->error;

		return $this->parent->getWidget()->getFormFormatter()
	...
	public function renderHelp() {
		if (null === $this->parent) {
			throw new LogicException(
					sprintf('Unable to render the help for "%s".', $this->name));
		}

		return $this->parent->getWidget()->getFormFormatter()
				->formatHelp($this->parent->getWidget()->getHelp($this->name));
	}
	...
	public function renderLabel($label = null, $attributes = array()) {
		if (null === $this->parent) {
			throw new LogicException(
					sprintf('Unable to render the label for "%s".', $this->name));
		}
		...
		$html = $this->parent->getWidget()->getFormFormatter()
				->generateLabel($this->name, $attributes);
	...
	public function renderLabelName() {
		if (null === $this->parent) {
			throw new LogicException(
					sprintf('Unable to render the label name for "%s".',
							$this->name));
		}
	...
	public function renderName() {
		return $this->parent ? $this->parent->getWidget()
						->generateName($this->name) : $this->name;
	}
	...
	public function renderId() {
		return $this->widget
				->generateId(
						$this->parent ? $this->parent->getWidget()
