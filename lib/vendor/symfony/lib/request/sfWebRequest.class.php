...
class sfWebRequest extends sfRequest {
	const PORT_HTTP = 80,PORT_HTTPS = 443;

	protected $languages = null, $charsets = null, $acceptableContentTypes = null, $pathInfoArray = null, $relativeUrlRoot = null, $getParameters = null, $postParameters = null, $requestParameters = null, $formats = array(), $format = null, $fixedFileArray = false;
	...
	public function initialize(sfEventDispatcher $dispatcher,
			$parameters = array(), $attributes = array(), $options = array()) {
		$options = array_merge(
				array('path_info_key' => 'PATH_INFO',
		...
		// GET parameters
		$this->getParameters = get_magic_quotes_gpc() ? sfToolkit::stripslashesDeep(
						$_GET) : $_GET;
	...
	public function isAbsUri() {
		...
		return isset($pathArray['REQUEST_URI']) ? preg_match('/^http/',
						$pathArray['REQUEST_URI']) : false;
	}
	...
	public function getUriPrefix() {
		...
		if ($this->isForwardedSecure()) {
			$port = isset($this->options['https_port'])
					&& self::PORT_HTTPS != $this->options['https_port'] ? $this
							->options['https_port'] : null;
		} elseif (($secure && self::PORT_HTTPS == $port)
				|| (!$secure && self::PORT_HTTP == $port)) {
			$port = null;
		}
	...
	public function getPathInfo() {
		...
		if (!isset($pathArray[$sf_path_info_key])
			...
			if (isset($pathArray['REQUEST_URI'])) {
				...
				$pathInfo = preg_replace(
						'/^' . preg_quote($script_name, '/') . '/', '',
						$pathInfo);
				$prefix_name = preg_replace('#/[^/]+$#', '', $script_name);
				$pathInfo = preg_replace(
						'/^' . preg_quote($prefix_name, '/') . '/', '',
						$pathInfo);
		...
		if (isset($_SERVER['SERVER_SOFTWARE'])
				&& false !== stripos($_SERVER['SERVER_SOFTWARE'], 'iis')
	...
	public function getPathInfoPrefix() {
		...
		if (!isset($this->options['no_script_name'])
				|| !$this->options['no_script_name']) {
			$scriptName = $this->getScriptName();
			$prefix = null === $prefix ? $scriptName
	...
	public function addRequestParameters($parameters) {
		$this->requestParameters = array_merge($this->requestParameters,
				$parameters);
	...
	public function getScriptName() {
		...
		return isset($pathArray['SCRIPT_NAME']) ? $pathArray['SCRIPT_NAME']
				: (isset($pathArray['ORIG_SCRIPT_NAME']) ? $pathArray['ORIG_SCRIPT_NAME']
						: '');
	}
	...
	public function getPreferredCulture(array $cultures = null) {
		...
		$preferredCultures = array_values(
				array_intersect($preferredCultures, $cultures));

		return isset($preferredCultures[0]) ? $preferredCultures[0]
				: $cultures[0];
	}
	...
	public function getLanguages() {
		...
		$languages = $this
				->splitHttpAcceptHeader($_SERVER['HTTP_ACCEPT_LANGUAGE']);
	...
	public function getCharsets() {
		...
		$this->charsets = $this
				->splitHttpAcceptHeader($_SERVER['HTTP_ACCEPT_CHARSET']);
	...
	public function getAcceptableContentTypes() {
		...
		$this->acceptableContentTypes = $this
				->splitHttpAcceptHeader($_SERVER['HTTP_ACCEPT']);
	...
	public function getHttpHeader($name, $prefix = 'http') {
		...
		return isset($pathArray[$name]) ? sfToolkit::stripslashesDeep(
						$pathArray[$name]) : null;
	}
	...
	public function getCookie($name, $defaultValue = null) {
		...
		if (isset($_COOKIE[$name])) {
			$retval = get_magic_quotes_gpc() ? sfToolkit::stripslashesDeep(
							$_COOKIE[$name]) : $_COOKIE[$name];
		}
	...
	public function isSecure() {
		...
		return (isset($pathArray['HTTPS'])
				&& ('on' == strtolower($pathArray['HTTPS'])
						|| 1 == $pathArray['HTTPS']))
				|| (isset($pathArray['HTTP_SSL_HTTPS'])
						&& ('on' == strtolower($pathArray['HTTP_SSL_HTTPS'])
	...
	public function getRelativeUrlRoot() {
		if (null === $this->relativeUrlRoot) {
			if (!isset($this->options['relative_url_root'])) {
				$this->relativeUrlRoot = preg_replace('#/[^/]+\.php5?$#', '',
						$this->getScriptName());
			} else {
	...
	public function getMimeType($format) {
		return isset($this->formats[$format]) ? $this->formats[$format][0]
				: null;
	}
	...
	public function setFormat($format, $mimeTypes) {
		$this->formats[$format] = is_array($mimeTypes) ? $mimeTypes
				: array($mimeTypes);
	}
	...
	public function getFiles($key = null) {
		...
		return null === $key ? $this->fixedFileArray
				: (isset($this->fixedFileArray[$key]) ? $this
								->fixedFileArray[$key] : array());
	}
	...
	public function getGetParameter($name, $default = null) {
		if (isset($this->getParameters[$name])) {
			...
			return sfToolkit::getArrayValueForPath($this->getParameters, $name,
					$default);
		}
	}
	...
	public function getPostParameter($name, $default = null) {
		if (isset($this->postParameters[$name])) {
			...
			return sfToolkit::getArrayValueForPath($this->postParameters,
					$name, $default);
		}
	}
	...
	public function getUrlParameter($name, $default = null) {
		if (isset($this->requestParameters[$name])) {
			...
			return sfToolkit::getArrayValueForPath($this->requestParameters,
					$name, $default);
		}
	}
	...
	public function checkCSRFProtection() {
		...
		$form
				->bind(
						$form->isCSRFProtected() ? array(
										$form->getCSRFFieldName() => $this
												->getParameter(
														$form
																->getCSRFFieldName()))
								: array());
	...
	protected function parseRequestParameters() {
		return $this->dispatcher
				->filter(
						new sfEvent($this, 'request.filter_parameters',
								$this->getRequestContext()), array())
				->getReturnValue();
	}
	...
	public function getRequestContext() {
		return array('path_info' => $this->getPathInfo(),
				'prefix' => $this->getPathInfoPrefix(),
				'method' => $this->getMethod(),
				'format' => $this->getRequestFormat(),
				'host' => $this->getHost(), 'is_secure' => $this->isSecure(),
