...
class sfDatabaseManager {
	protected $configuration = null, $databases = array();
	...
	public function loadConfiguration() {
		if ($this->configuration instanceof sfApplicationConfiguration) {
			$databases = include($this->configuration->getConfigCache()
					->checkConfig('config/databases.yml'));
		} else {
			$configHandler = new sfDatabaseConfigHandler();
			$databases = $configHandler
					->evaluate(
							array(
	...
	public function getDatabase($name = 'default') {
		...
		throw new sfDatabaseException(
				sprintf('Database "%s" does not exist.', $name));
	}
