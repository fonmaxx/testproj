	...
	public function connect() {
		if (!$dsn = $this->getParameter('dsn')) {
			...
			throw new sfDatabaseException(
					'Database configuration is missing the "dsn" parameter.');
		}
		...
		try {
			...
			$options = ($persistent) ? array(PDO::ATTR_PERSISTENT => true)
					: array();

			$this->connection = new $pdo_class($dsn, $username, $password,
					$options);
		...
		if ($nulls) {
			$this->connection
					->setAttribute(PDO::ATTR_ORACLE_NULLS,
