...
class sfExecutionFilter extends sfFilter {
	...
	public function execute($filterChain) {
		...
		if (sfConfig::get('sf_debug') && sfConfig::get('sf_logging_enabled')) {
			...
			$timer->addTime();
			$timer = sfTimerManager::getTimer(
					sprintf('View "%s" for "%s/%s"', $viewName,
	...
	protected function handleView($filterChain, $actionInstance, $viewName) {
		...
		$this
				->executeView($actionInstance->getModuleName(),
	...
	protected function executeView($moduleName, $actionName, $viewName,
		...
		case sfView::RENDER_VAR:
			$viewData = $view->render();
			$controller->getActionStack()->getLastEntry()
