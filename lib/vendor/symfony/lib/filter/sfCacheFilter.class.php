...
class sfCacheFilter extends sfFilter {
	protected $cacheManager = null, $request = null, $response = null, $routing = null, $cache = array();
	...
	protected function setCacheExpiration($uri) {
		...
		if (!$this->response->hasHttpHeader('Expires')) {
			$this->response
					->setHttpHeader('Expires',
							$this->response->getDate(time() + $lifetime),
							false);
	...
	protected function setCacheValidation($uri) {
		...
		if (!$this->response->hasHttpHeader('Last-Modified')) {
			$this->response
					->setHttpHeader('Last-Modified',
							$this->response->getDate(time()), false);
		}
	...
	protected function checkCacheValidation() {
		...
		if ($this->response->hasHttpHeader('Last-Modified')
			...
			if ($this->request->getHttpHeader('IF_MODIFIED_SINCE')
					== $lastModified) {
				$this->response->setStatusCode(304);
				$this->response->setHeaderOnly(true);
