...
class sfFilterConfigHandler extends sfYamlConfigHandler {
	...
	public function execute($configFiles) {
		...
		foreach ($config as $category => $keys) {
			...
			$type = isset($keys['param']['type']) ? $keys['param']['type']
					: null;
			unset($keys['param']['type']);

			if ($condition) {
				// parse parameters
				$parameters = isset($keys['param']) ? var_export(
								$keys['param'], true) : 'null';
		...
		if (!$rendering) {
			throw new sfParseException(
					sprintf(
							'Configuration file "%s" must register a filter of type "rendering".',
							$configFiles[0]));
		}
		...
		if (!$execution) {
			throw new sfParseException(
					sprintf(
							'Configuration file "%s" must register a filter of type "execution".',
							$configFiles[0]));
		}
	...
	protected function addFilter($category, $class, $parameters) {
		return sprintf(
				"\nlist(\$class, \$parameters) = (array) sfConfig::get('sf_%s_filter', array('%s', %s));\n"
						...
						. "\$this->register(\$filter);", $category, $class,
				$parameters);
	}
	...
	static public function getConfiguration(array $configFiles) {
		...
		foreach (array_slice($configFiles, 1) as $i => $configFile) {
			...
			foreach (self::parseYaml($configFile) as $key => $value) {
				$value = (array) $value;
				$config[$key] = isset($previous[$key]) ? sfToolkit::arrayDeepMerge(
			...
			foreach (array_keys($previous) as $key) {
				if (!isset($config[$key])) {
					throw new sfConfigurationException(
							sprintf(
									'The filter name "%s" is defined in "%s" but not present in "%s" file. To disable a filter, add a "enabled" key with a false value.',
									$key, $configFiles[$i], $configFile));
				}
			}
		}
