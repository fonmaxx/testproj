...
abstract class sfApplicationConfiguration extends ProjectConfiguration {
	static protected $coreLoaded = false, $loadedHelpers = array();

	protected $configCache = null, $application = null, $environment = null, $debug = false, $config = array(), $cache = null;
	...
	public function __construct($environment, $debug, $rootDir = null,
		...
		if (file_exists(
				$file = sfConfig::get('sf_app_cache_dir')
	...
	public function initConfiguration() {
		...
		if (!$this->isDebug() && !sfConfig::get('sf_test')
				&& !self::$coreLoaded) {
		...
		// autoloader(s)
		$this->dispatcher
				->connect('autoload.filter_config',
	...
	protected function initializePlugins() {
		foreach ($this->pluginConfigurations as $name => $configuration) {
			if (false === $configuration->initialize()
					&& is_readable(
							$config = $configuration->getRootDir()
									. '/config/config.php')) {
				require $config;
			}
		}
		...
		if ($this
				->hasLockFile(
						sfConfig::get('sf_data_dir') . DIRECTORY_SEPARATOR
								. $this->getApplication() . '_'
								. $this->getEnvironment() . '-cli.lck', 5)
				|| $this
						->hasLockFile(
								sfConfig::get('sf_data_dir')
										. DIRECTORY_SEPARATOR
										. $this->getApplication() . '_'
										. $this->getEnvironment() . '.lck')) {
			...
			$files = array(
					...
					$this->getSymfonyLibDir()
							. '/exception/data/unavailable.php',);
	...
	public function setRootDir($rootDir) {
		...
		sfConfig::add(
				array('sf_app' => $this->getApplication(),
						'sf_environment' => $this->getEnvironment(),
						'sf_debug' => $this->isDebug(),));

		$this
				->setAppDir(
						sfConfig::get('sf_apps_dir') . DIRECTORY_SEPARATOR
								. $this->getApplication());
	}
	...
	public function setAppDir($appDir) {
		sfConfig::add(
				array('sf_app_dir' => $appDir,
						// SF_APP_DIR directory structure
						'sf_app_config_dir' => $appDir . DIRECTORY_SEPARATOR
								. 'config',
						'sf_app_lib_dir' => $appDir . DIRECTORY_SEPARATOR
								. 'lib',
						'sf_app_module_dir' => $appDir . DIRECTORY_SEPARATOR
								. 'modules',
						'sf_app_template_dir' => $appDir . DIRECTORY_SEPARATOR
								. 'templates',
						'sf_app_i18n_dir' => $appDir . DIRECTORY_SEPARATOR
								. 'i18n',));
	}
	...
	public function setCacheDir($cacheDir) {
		...
		sfConfig::add(
				array(
						'sf_app_base_cache_dir' => $cacheDir
								. DIRECTORY_SEPARATOR . $this->getApplication(),
						'sf_app_cache_dir' => $appCacheDir = $cacheDir
								. DIRECTORY_SEPARATOR . $this->getApplication()
								. DIRECTORY_SEPARATOR . $this->getEnvironment(),
						// SF_CACHE_DIR directory structure
						'sf_template_cache_dir' => $appCacheDir
								. DIRECTORY_SEPARATOR . 'template',
						'sf_i18n_cache_dir' => $appCacheDir
								. DIRECTORY_SEPARATOR . 'i18n',
						'sf_config_cache_dir' => $appCacheDir
								. DIRECTORY_SEPARATOR . 'config',
						'sf_test_cache_dir' => $appCacheDir
								. DIRECTORY_SEPARATOR . 'test',
						'sf_module_cache_dir' => $appCacheDir
	...
	public function getControllerDirs($moduleName) {
		if (!isset($this->cache['getControllerDirs'][$moduleName])) {
			...
			$dirs[sfConfig::get('sf_app_module_dir') . '/' . $moduleName
					. '/actions'] = false; // application

			foreach ($this->getPluginPaths() as $path) {
				if (is_dir($dir = $path . '/modules/' . $moduleName
						. '/actions')) {
			...
			if (is_dir(
					$dir = $this->getSymfonyLibDir() . '/controller/'
							. $moduleName . '/actions')) {
	...
	public function getLibDirs($moduleName) {
		...
		$dirs = array_merge($dirs,
				$this->getPluginSubPaths('/modules/' . $moduleName . '/lib')); // plugins
		$dirs[] = $this->getSymfonyLibDir() . '/controller/' . $moduleName
				. '/lib'; // core modules
	...
	public function getTemplateDirs($moduleName) {
		...
		$dirs = array_merge($dirs,
				$this->getPluginSubPaths('/modules/' . $moduleName
								. '/templates')); // plugins
		$dirs[] = $this->getSymfonyLibDir() . '/controller/' . $moduleName
	...
	public function getHelperDirs($moduleName = '') {
		...
		return array_merge($dirs,
				array(sfConfig::get('sf_app_lib_dir') . '/helper',
						// application
	...
	public function getI18NDirs($moduleName) {
		...
		// module
		if (is_dir(
				$dir = sfConfig::get('sf_app_module_dir') . '/' . $moduleName
						. '/i18n')) {
			$dirs[] = $dir;
		}
	...
	public function getConfigPaths($configPath) {
		...
		$files = array($this->getSymfonyLibDir() . '/config/'
				. $globalConfigPath, // symfony
		);
		...
		$files = array_merge($files,
				array($this->getRootDir() . '/' . $globalConfigPath,
						// project
	...
	public function loadHelpers($helpers, $moduleName = '') {
		foreach ((array) $helpers as $helperName) {
			...
			if (isset($this->cache['loadedHelpers'][$moduleName][$helperName])) {
				include_once $this
						->cache['loadedHelpers'][$moduleName][$helperName];
