...
class sfProjectConfiguration {
	protected $rootDir = null, $symfonyLibDir = null, $dispatcher = null, $plugins = array(), $pluginPaths = array(), $overriddenPluginPaths = array(), $pluginConfigurations = array(), $pluginsLoaded = false;
	...
	public function __construct($rootDir = null,
		...
		if (null === self::$active
				|| $this instanceof sfApplicationConfiguration) {
			self::$active = $this;
		}

		$this->rootDir = null === $rootDir ? self::guessRootDir()
				: realpath($rootDir);
		$this->symfonyLibDir = realpath(dirname(__FILE__) . '/..');
		$this->dispatcher = null === $dispatcher ? new sfEventDispatcher()
				: $dispatcher;
	...
	public function loadPlugins() {
		foreach ($this->getPluginPaths() as $path) {
			if (false
					=== $plugin = array_search($path,
							$this->overriddenPluginPaths)) {
				$plugin = basename($path);
			}
			$class = $plugin . 'Configuration';

			if (is_readable(
					$file = sprintf('%s/config/%s.class.php', $path, $class))) {
				require_once $file;
	...
	public function setRootDir($rootDir) {
		...
		sfConfig::add(
				array('sf_root_dir' => $rootDir,
						// global directory structure
						'sf_apps_dir' => $rootDir . DIRECTORY_SEPARATOR
								. 'apps',
						'sf_lib_dir' => $rootDir . DIRECTORY_SEPARATOR . 'lib',
						'sf_log_dir' => $rootDir . DIRECTORY_SEPARATOR . 'log',
						'sf_data_dir' => $rootDir . DIRECTORY_SEPARATOR
								. 'data',
						'sf_config_dir' => $rootDir . DIRECTORY_SEPARATOR
								. 'config',
						'sf_test_dir' => $rootDir . DIRECTORY_SEPARATOR
								. 'test',
						'sf_plugins_dir' => $rootDir . DIRECTORY_SEPARATOR
								. 'plugins',));
	...
	public function setWebDir($webDir) {
		sfConfig::add(
				array('sf_web_dir' => $webDir,
						'sf_upload_dir' => $webDir . DIRECTORY_SEPARATOR
								. 'uploads',));
	}
	...
	public function getModelDirs() {
		return array_merge($this->getPluginSubPaths('/lib/model'),
				// plugins
	...
	public function getGeneratorTemplateDirs($class, $theme) {
		return array_merge(
				array(
						sfConfig::get('sf_data_dir') . '/generator/' . $class
								. '/' . $theme . '/template'),
				// project
				$this
						->getPluginSubPaths(
				...
				$this
						->getPluginSubPaths(
								'/data/generator/' . $class
										. '/default/template') // plugins (default theme)
		);
	}
	...
	public function getGeneratorSkeletonDirs($class, $theme) {
		return array_merge(
				...
				$this
						->getPluginSubPaths(
								'/data/generator/' . $class
										. '/default/skeleton') // plugins (default theme)
		);
	}
	...
	public function getGeneratorTemplate($class, $theme, $path) {
		...
		throw new sfException(
				sprintf('Unable to load "%s" generator template in: %s.',
						$path, implode(', ', $dirs)));
	}
		...
		foreach ($plugins as $plugin) {
			if (false !== $pos = array_search($plugin, $this->plugins)) {
				...
				throw new InvalidArgumentException(
						sprintf('The plugin "%s" does not exist.', $plugin));
			}
		}
	...
	public function getAllPluginPaths() {
		...
		$finder = sfFinder::type('dir')->maxdepth(0)
				->ignore_version_control(false)->follow_link()->name('*Plugin');
		$dirs = array($this->getSymfonyLibDir() . '/plugins',
	...
	public function getPluginConfiguration($name) {
		if (!isset($this->pluginConfigurations[$name])) {
			throw new InvalidArgumentException(
					sprintf(
							'There is no configuration object for the "%s" object.',
							$name));
		}
	 ...
	 */
	static public function getApplicationConfiguration($application,
			$environment, $debug, $rootDir = null,
			sfEventDispatcher $dispatcher = null) {
		...
		if (!file_exists(
				$file = $rootDir . '/apps/' . $application . '/config/'
						. $class . '.class.php')) {
			throw new InvalidArgumentException(
					sprintf('The application "%s" does not exist.',
							$application));
		}
	...
	public function __call($method, $arguments) {
		$event = $this->dispatcher
				->notifyUntil(
						new sfEvent($this, 'configuration.method_not_found',
								array('method' => $method,
										'arguments' => $arguments)));
