...
abstract class sfPluginConfiguration {
	protected $configuration = null, $dispatcher = null, $name = null, $rootDir = null;
	...
	public function __construct(sfProjectConfiguration $configuration,
			$rootDir = null, $name = null) {
		$this->configuration = $configuration;
		$this->dispatcher = $configuration->getEventDispatcher();
		$this->rootDir = null === $rootDir ? $this->guessRootDir()
				: realpath($rootDir);
	...
	public function initializeAutoload() {
		...
		if (is_readable($file = $this->rootDir . '/config/autoload.yml')) {
			...
			$this->configuration->getEventDispatcher()
					->disconnect('autoload.filter_config',
							array($this, 'filterAutoloadConfig'));
		} else {
	...
	public function connectTests() {
		$this->dispatcher
				->connect('task.test.filter_test_files',
						array($this, 'filterTestFiles'));
	}
	...
	public function filterTestFiles(sfEvent $event, $files) {
		...
		foreach ($names as $name) {
			$finder = sfFinder::type('file')->follow_link()
					->name(basename($name) . 'Test.php');
			$files = array_merge($files,
					$finder->in($directory . '/' . dirname($name)));
		}
