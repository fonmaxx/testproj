...
abstract class sfYamlConfigHandler extends sfConfigHandler {
	...
	static public function parseYaml($configFile) {
		...
		if ($config === false) {
			...
			throw new sfParseException(
					sprintf('Configuration file "%s" could not be parsed',
							$configFile));
		}
	...
	protected function mergeConfigValue($keyName, $category) {
		...
		if ($category && isset($this->yamlConfig[$category][$keyName])
				&& is_array($this->yamlConfig[$category][$keyName])) {
			$values = array_merge($values,
					$this->yamlConfig[$category][$keyName]);
		}
	...
	static public function flattenConfigurationWithEnvironment($config) {
		return sfToolkit::arrayDeepMerge(
				...
				isset($config[sfConfig::get('sf_environment')])
						&& is_array($config[sfConfig::get('sf_environment')]) ? $config[sfConfig::get(
								'sf_environment')] : array());
	}
}
