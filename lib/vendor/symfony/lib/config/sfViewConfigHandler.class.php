...
class sfViewConfigHandler extends sfYamlConfigHandler {
	...
	public function execute($configFiles) {
		...
		foreach ($this->yamlConfig as $viewName => $values) {
			...
			$data[] = ($first ? '' : 'else ')
					. "if (\$this->actionName.\$this->viewName == '$viewName')\n"
					. "{\n";
		...
		foreach ($this->yamlConfig as $viewName => $values) {
			...
			$data[] = ($first ? '' : 'else ')
					. "if (\$templateName.\$this->viewName == '$viewName')\n"
					. "{\n";
	...
	protected function addTemplate($viewName = '') {
		...
		$defaultTemplateName = $templateName ? "'$templateName'"
				: '$this->actionName';
	...
	protected function addLayout($viewName = '') {
		...
		$hasLocalLayout = isset($this->yamlConfig[$viewName]['layout'])
				|| (isset($this->yamlConfig[$viewName])
						&& array_key_exists('has_layout',
								$this->yamlConfig[$viewName]));

		// the layout value
		$layout = $this->getConfigValue('has_layout', $viewName) ? $this
						->getConfigValue('layout', $viewName) : false;
...
EOF;
	...
	protected function addHtmlHead($viewName = '') {
		...
		foreach ($this->mergeConfigValue('metas', $viewName) as $name => $content) {
			$data[] = sprintf(
					"  \$response->addMeta('%s', '%s', false, false);", $name,
					str_replace('\'', '\\\'',
							preg_replace('/&amp;(?=\w+;)/', '&',
	...
	private function addAssets($type, $assets) {
		...
		foreach ((array) $assets as $asset) {
			...
			if ('-*' == $key) {
				...
				$tmp[$key] = sprintf("  \$response->add%s('%s', '%s', %s);",
						$type, $key, $position,
	...
	protected function addEscaping($viewName = '') {
		...
		if (isset($escaping['method'])) {
			$data[] = sprintf(
					"  \$this->getAttributeHolder()->setEscapingMethod(%s);",
					var_export($escaping['method'], true));
		}
	...
	static protected function mergeConfig($config) {
		...
		$config['all']['stylesheets'] = array_merge(
				isset($config['default']['stylesheets'])
						&& is_array($config['default']['stylesheets']) ? $config['default']['stylesheets']
						: array(),
				isset($config['all']['stylesheets'])
						&& is_array($config['all']['stylesheets']) ? $config['all']['stylesheets']
						: array());
		unset($config['default']['stylesheets']);

		$config['all']['javascripts'] = array_merge(
				isset($config['default']['javascripts'])
						&& is_array($config['default']['javascripts']) ? $config['default']['javascripts']
						: array(),
				isset($config['all']['javascripts'])
						&& is_array($config['all']['javascripts']) ? $config['all']['javascripts']
						: array());
