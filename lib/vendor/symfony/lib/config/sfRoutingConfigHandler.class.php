	...
	public function execute($configFiles) {
		...
		foreach ($this->parse($configFiles) as $name => $routeConfig) {
			...
			foreach (sfPatternRouting::flattenRoutes($routes) as $name => $route) {
				...
				$data[] = sprintf('$this->routes[\'%s\'] = unserialize(%s);',
						$name, var_export(serialize($route), true));
			}
		}
	...
	protected function getOptions() {
		$config = sfFactoryConfigHandler::getConfiguration(
				sfContext::getInstance()->getConfiguration()
						->getConfigPaths('config/factories.yml'));
	...
	protected function parse($configFiles) {
		...
		foreach ($config as $name => $params) {
			if ((isset($params['type']) && 'collection' == $params['type'])
					|| (isset($params['class'])
							&& false !== strpos($params['class'], 'Collection'))) {
