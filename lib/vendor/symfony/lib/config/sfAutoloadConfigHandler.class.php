		...
		foreach ($config['autoload'] as $name => $entry) {
			...
			if (isset($entry['files'])) {
				...
				require_once(sfConfig::get('sf_symfony_lib_dir')
						. '/util/sfFinder.class.php');
				$finder = sfFinder::type('file')->name('*' . $ext)
						->follow_link();

				// recursive mapping?
				$recursive = isset($entry['recursive']) ? $entry['recursive']
						: false;
				if (!$recursive) {
					$finder->maxdepth(0);
				}
				...
				if (isset($entry['exclude']) && is_array($entry['exclude'])) {
					$finder->prune($entry['exclude'])
							->discard($entry['exclude']);
				}
				...
				if ($matches = glob($path)) {
					foreach ($finder->in($matches) as $file) {
						$mapping = array_merge($mapping,
								$this
										->parseFile($path, $file,
												isset($entry['prefix']) ? $entry['prefix']
														: ''));
					}
				}
			}
								...
								. str_replace('\*', '(.+?)',
										preg_quote(
												str_replace('/',
														DIRECTORY_SEPARATOR,
														$path), '~')) . '~',
						str_replace('/', DIRECTORY_SEPARATOR, $file), $match);
	...
	static public function getConfiguration(array $configFiles) {
		...
		foreach ($configFiles as $i => $configFile) {
			...
			$path = str_replace(DIRECTORY_SEPARATOR, '/',
					realpath(
							join('/',
									array_slice(explode('/', $configFilePath),
											0, -2))));
		...
		foreach ($config['autoload'] as $name => $values) {
			if (isset($values['path'])) {
				$config['autoload'][$name]['path'] = self::replacePath(
						$values['path']);
			}
		}

		$event = $configuration->getEventDispatcher()
				->filter(new sfEvent(__CLASS__, 'autoload.filter_config'),
						$config);
