 ...
 */
class sfConfigCache {
	protected $configuration = null, $handlers = array(), $userHandlers = array();
		...
		if (isset($this->handlers[$handler])) {
			...
			foreach (array_keys($this->handlers) as $key) {
				...
				if (preg_match('#' . $pattern . '$#', $handler, $matches)) {
					...
					array_shift($matches);
					$handlerInstance->getParameterHolder()
							->set('wildcardValues', $matches);
	...
	public function checkConfig($configPath, $optional = false) {
		...
		if (!isset($files[0])) {
			...
			throw new sfConfigurationException(
					sprintf(
							'Configuration "%s" does not exist or is unreadable.',
							$configPath));
		}
	...
	public function getCacheName($config) {
		if (strlen($config) > 3 && ctype_alpha($config[0]) && $config[1] == ':'
				&& ($config[2] == '\\' || $config[2] == '/')) {
	...
	protected function loadConfigHandlers() {
		...
		while (($directory = readdir($fp)) !== false) {
			...
			$configPath = $sf_app_module_dir . '/' . $directory
	...
	protected function writeCacheFile($config, $cache, $data) {
		...
		if (!$fp = @fopen($tmpFile, 'wb')) {
			throw new sfCacheException(
					sprintf(
							'Failed to write cache file "%s" generated from configuration file "%s".',
							$tmpFile, $config));
		}
