...
class sfGeneratorConfigHandler extends sfYamlConfigHandler {
	...
	public function execute($configFiles) {
		...
		if (!isset($config['generator'])) {
			throw new sfParseException(
					sprintf(
							...
							isset($configFiles[1]) ? $configFiles[1]
									: $configFiles[0]));
		}
		...
		preg_match('#.*/modules/([^/]+)/#',
				str_replace('\\', '/', $configFiles[0]), $match);
		...
		$retval = sprintf($retval, date('Y/m/d H:i:s'),
				self::getContent($generatorManager, $config['class'],
						$generatorParam));

		return $retval;
	}

	static public function getContent(sfGeneratorManager $generatorManager,
			$class, $parameters) {
