...
class sfFactoryConfigHandler extends sfYamlConfigHandler {
	...
	public function execute($configFiles) {
		...
		$factories = array('view_cache_manager', 'logger', 'i18n',
				'controller', 'request', 'response', 'routing', 'storage',
				'user', 'view_cache', 'mailer');
			...
			if (isset($keys['param'])) {
				if (!is_array($keys['param'])) {
					throw new InvalidArgumentException(
							sprintf(
									'The "param" key for the "%s" factory must be an array (in %s).',
									$class, $configFiles[0]));
				}
			...
			case 'storage':
				...
				if (is_subclass_of($class, 'sfDatabaseSessionStorage')) {
					$defaultParameters[] = sprintf(
							...
							isset($parameters['database']) ? $parameters['database']
									: 'default');
			...
			case 'logger':
				...
				$instances[] = sprintf(
						"  \$class = sfConfig::get('sf_factory_logger', '%s');\n  \$this->factories['logger'] = new \$class(\$this->dispatcher, array_merge(array('auto_shutdown' => false), sfConfig::get('sf_factory_logger_parameters', %s)));\n"
								. "  %s", $class,
						var_export($parameters, true), $loggers);
				break;
