...
class sfRoute implements Serializable {
	protected $isBound = false, $context = null, $parameters = null, $suffix = null, $defaultParameters = array(), $defaultOptions = array(), $compiled = false, $options = array(), $pattern = null, $staticPrefix = null, $regex = null, $variables = array(), $defaults = array(), $requirements = array(), $tokens = array(), $customToken = false;
	...
	public function matchesUrl($url, $context = array()) {
		...
		if ('' !== $this->staticPrefix
				&& 0 !== strpos($url, $this->staticPrefix)) {
			return false;
		}
		...
		if (isset($matches['_star'])) {
			...
			unset($matches['_star'], $parameters['module'],
					$parameters['action']);
		}
	...
	public function matchesParameters($params, $context = array()) {
		...
		$defaults = $this
				->mergeArrays($this->getDefaultParameters(), $this->defaults);
		...
		foreach (array_keys($this->variables) as $variable) {
			...
			if (!preg_match('#' . $this->requirements[$variable] . '#',
					$tparams[$variable])) {
				return false;
			}
		}
		...
		if (!$this->options['extra_parameters_as_query_string']) {
			if (false === strpos($this->regex, '<_star>')
					&& array_diff_key($params, $this->variables, $defaults)) {
				return false;
			}
		}
	...
	protected function generateWithTokens($parameters) {
		...
		foreach ($tokens as $token) {
			switch ($token[0]) {
			case 'variable':
				if (!$optional || !isset($this->defaults[$token[3]])
						|| $parameters[$token[3]] != $this->defaults[$token[3]]) {
			...
			// handle custom tokens
				if ($segment = call_user_func_array(
						array($this, 'generateFor'
								. ucfirst(array_shift($token))),
	...
	public function compile() {
		...
		foreach ($this->tokens as $token) {
			call_user_func_array(
					array($this, 'compileFor' . ucfirst(array_shift($token))),
					$token);
		}
		...
		$this->regex = "#^" . implode("", $this->segments) . ""
	...
	protected function postCompile() {
		...
		for ($i = $this->firstOptional, $max = count($this->segments); $i
				< $max; $i++) {
			$this->segments[$i] = (0 == $i ? '/?' : '')
					. str_repeat(' ', $i - $this->firstOptional) . '(?:'
					. $this->segments[$i];
	...
	protected function tokenize() {
		...
		while (strlen($buffer)) {
			if (false
					!== $this
							->tokenizeBufferBefore($buffer, $this->tokens,
									$afterASeparator, $currentSeparator)) {
				// a custom token
				$this->customToken = true;
			} else if ($afterASeparator
					&& preg_match(
							'#^' . $this->options['variable_prefix_regex']
									. '(' . $this->options['variable_regex']
									. ')#', $buffer, $match)) {
				// a variable
				$this->tokens[] = array('variable', $currentSeparator,
						$match[0], $match[1]);
				...
				$afterASeparator = false;
					...
					&& preg_match(
							'#^(' . $this->options['text_regex'] . ')(?:'
									. $this
											->options['segment_separators_regex']
									. '|$)#', $buffer, $match)) {
				// a text
				...
				$this->tokens[] = array('separator', $currentSeparator,
						$match[0], null);
				...
				$afterASeparator = true;
					...
					!== $this
							->tokenizeBufferAfter($buffer, $this->tokens,
									$afterASeparator, $currentSeparator)) {
				// a custom token
				$this->customToken = true;
			} else {
				// parsing problem
				throw new InvalidArgumentException(
						sprintf('Unable to parse "%s" route near "%s".',
								$this->pattern, $buffer));
			}
		}
	 ...
	 */
	protected function tokenizeBufferBefore(&$buffer, &$tokens,
			&$afterASeparator, &$currentSeparator) {
		return false;
	}
	 ...
	 * @return Boolean true if a token has been generated, false otherwise
	 */
	protected function tokenizeBufferAfter(&$buffer, &$tokens,
			&$afterASeparator, &$currentSeparator) {
		return false;
	}
	...
	protected function compileForText($separator, $text) {
		if ('*' == $text) {
			$this->segments[] = '(?:' . preg_quote($separator, '#')
					. '(?P<_star>.*))?';
		} else {
	...
	protected function compileForVariable($separator, $name, $variable) {
		if (!isset($this->requirements[$variable])) {
			$this->requirements[$variable] = $this
					->options['variable_content_regex'];
		}

		$this->segments[] = preg_quote($separator, '#') . '(?P<' . $variable
				. '>' . $this->requirements[$variable] . ')';
	...
	protected function initializeOptions() {
		...
		$preg_quote_hash = create_function('$a',
				'return preg_quote($a, \'#\');');

		// compute some regexes
		$this->options['variable_prefix_regex'] = '(?:'
				. implode('|',
						array_map($preg_quote_hash,
		...
		if (count($this->options['segment_separators'])) {
			...
			$preg_quote_hash_53 = create_function('$a',
	...
	protected function parseStarParameter($star) {
		...
		for ($i = 0, $max = count($tmp); $i < $max; $i += 2) {
			...
			if (!empty($tmp[$i])) {
				$parameters[$tmp[$i]] = isset($tmp[$i + 1]) ? urldecode(
								$tmp[$i + 1]) : true;
			}
		}
	...
	protected function fixSuffix() {
		...
		if ($length > 0 && '/' == $this->pattern[$length - 1]) {
			...
			$this->pattern = substr($this->pattern, 0, $length - 1);
				...
				'#\.(?:' . $this->options['variable_prefix_regex']
						. $this->options['variable_regex'] . '|'
						. $this->options['variable_content_regex'] . ')$#i',
				$this->pattern)) {
	...
	public function serialize() {
		...
		return serialize(
				array($this->tokens, $this->defaultOptions, $this->options,
						$this->pattern, $this->staticPrefix, $this->regex,
						$this->variables, $this->defaults, $this->requirements,
						$this->suffix, $this->customToken));
	}

	public function unserialize($data) {
		list($this->tokens, $this->defaultOptions, $this->options, $this
				->pattern, $this->staticPrefix, $this->regex, $this->variables, $this
				->defaults, $this->requirements, $this->suffix, $this
				->customToken) = unserialize($data);
		$this->compiled = true;
	}
}
