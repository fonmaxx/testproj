...
abstract class sfRouting {
	protected $dispatcher = null, $cache = null, $defaultParameters = array(), $options = array();
	...
	public function __construct(sfEventDispatcher $dispatcher,
		...
		if (!isset($this->options['auto_shutdown'])
	...
	public function initialize(sfEventDispatcher $dispatcher,
		...
		$options['debug'] = isset($options['debug']) ? (boolean) $options['debug']
				: false;
		...
		$this
				->setDefaultParameter('module',
						isset($options['default_module']) ? $options['default_module']
								: 'default');
		$this
				->setDefaultParameter('action',
						isset($options['default_action']) ? $options['default_action']
								: 'index');
		...
		$this->dispatcher
				->connect('request.filter_parameters',
	...
	public function loadConfiguration() {
		$this->dispatcher
				->notify(new sfEvent($this, 'routing.load_configuration'));
	}
	 ...
	 */
	abstract public function generate($name, $params = array(),
			$absolute = false);
	...
	public function getDefaultParameter($key) {
		return isset($this->defaultParameters[$key]) ? $this
						->defaultParameters[$key] : null;
	}
	...
	protected function fixGeneratedUrl($url, $absolute = false) {
		...
		if ($absolute && isset($this->options['context']['host'])
				&& 0 !== strpos($url, 'http')) {
			$url = 'http'
					. (isset($this->options['context']['is_secure'])
							&& $this->options['context']['is_secure'] ? 's' : '')
					. '://' . $this->options['context']['host'] . $url;
		}
