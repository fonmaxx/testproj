...
class sfPatternRouting extends sfRouting {
	protected $currentRouteName = null, $currentInternalUri = array(), $routes = array(), $defaultParamsDirty = false, $cacheData = array(), $cacheChanged = false;
	...
	public function initialize(sfEventDispatcher $dispatcher,
		...
		$options = array_merge(
				array('variable_prefixes' => array(':'),
						...
						'lookup_cache_dedicated_keys' => false,), $options);
	...
	public function insertRouteBefore($pivot, $name, $route) {
		if (!isset($this->routes[$pivot])) {
			throw new sfConfigurationException(
					sprintf(
							'Unable to insert route "%s" before inexistent route "%s".',
							$name, $pivot));
		}
	...
	public function generate($name, $params = array(), $absolute = false) {
		...
		if ($name) {
			...
			if (false
					=== $route = $this->getRouteThatMatchesParameters($params)) {
				throw new sfConfigurationException(
						sprintf(
								'Unable to find a matching route to generate url for params "%s".',
			...
			$this->dispatcher
					->notify(
							new sfEvent($this, 'application.log',
									array(
											sprintf(
													'Match route "%s" (%s) for %s with parameters %s',
													$info['name'],
													$info['pattern'], $url,
													str_replace("\n", '',
															var_export(
	...
	public function findRoute($url) {
		...
		if (null !== $this->cache) {
			if ($this->options['lookup_cache_dedicated_keys']) {
				$this->cache
						->set('symfony.routing.data.' . $cacheKey,
								serialize($info));
			} else {
				$this->cacheChanged = true;
	...
	protected function getRouteThatMatchesUrl($url) {
		...
		foreach ($this->routes as $name => $route) {
			if (false
					=== $parameters = $route
							->matchesUrl($url, $this->options['context'])) {
				continue;
			}
	...
	protected function getRouteThatMatchesParameters($parameters) {
		...
		foreach ($this->routes as $route) {
			if ($route
					->matchesParameters($parameters, $this->options['context'])) {
				return $route;
			}
		}
	...
	public function shutdown() {
		if (null !== $this->cache && $this->cacheChanged) {
			...
			$this->cache
					->set('symfony.routing.data', serialize($this->cacheData));
		}
	}
}
