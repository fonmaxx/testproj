...
class sfObjectRoute extends sfRequestRoute {
	protected $object = false, $objects = false;
	...
	public function __construct($pattern, array $defaults = array(),
		...
		if (!isset($options['model'])) {
			throw new InvalidArgumentException(
					sprintf(
							'You must pass a "model" option for a %s object (%s).',
		...
		if (!isset($options['type'])) {
			throw new InvalidArgumentException(
					sprintf(
							'You must pass a "type" option for a %s object (%s).',
							get_class($this), $pattern));
		}
		...
		if (!in_array($options['type'], array('object', 'list'))) {
			throw new InvalidArgumentException(
					sprintf(
							'The "type" option can only be "object" or "list", "%s" given (%s).',
	...
	public function matchesParameters($params, $context = array()) {
		return parent::matchesParameters(
				'object' == $this->options['type'] ? $this
								->convertObjectToArray($params) : $params);
	}
	...
	public function getObject() {
		...
		if ('object' != $this->options['type']) {
			throw new LogicException(
					sprintf('The route "%s" is not of type "object".',
							$this->pattern));
		}
		...
		if (!($this->object = $this->getObjectForParameters($this->parameters))
			...
			throw new sfError404Exception(
					sprintf(
							'Unable to find the %s object with the following parameters "%s").',
							$this->options['model'],
							str_replace("\n", '',
									var_export(
											$this
	...
	public function getObjects() {
		...
		if ('list' != $this->options['type']) {
			throw new LogicException(
					sprintf('The route "%s" is not of type "list".',
							$this->pattern));
		}
		...
		if (!count($this->objects) && isset($this->options['allow_empty'])
			...
			throw new sfError404Exception(
					sprintf(
							'No %s object found for the following parameters "%s").',
							$this->options['model'],
							str_replace("\n", '',
									var_export(
											$this
	...
	protected function getObjectForParameters($parameters) {
		...
		if (!isset($this->options['method'])) {
			throw new InvalidArgumentException(
					sprintf(
							'You must pass a "method" option for a %s object.',
							get_class($this)));
		}

		return call_user_func(array($className, $this->options['method']),
	...
	protected function doConvertObjectToArray($object) {
		$method = isset($this->options['convert']) ? $this->options['convert']
				: 'toParams';
	...
	protected function getRealVariables() {
		...
		foreach (array_keys($this->variables) as $variable) {
			if (0 === strpos($variable, 'sf_')
					|| in_array($variable, array('module', 'action'))) {
				continue;
			}
