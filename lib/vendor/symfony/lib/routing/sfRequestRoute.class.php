...
class sfRequestRoute extends sfRoute {
	...
	public function __construct($pattern, $defaults = array(),
			$requirements = array(), $options = array()) {
		if (!isset($requirements['sf_method'])) {
			$requirements['sf_method'] = array('get', 'head');
		} else {
			$requirements['sf_method'] = array_map('strtolower',
					(array) $requirements['sf_method']);
		}
	...
	public function matchesUrl($url, $context = array()) {
		...
		if (in_array(strtolower($context['method']),
				$this->requirements['sf_method'])) {
			return $parameters;
		}
	...
	public function matchesParameters($params, $context = array()) {
		if (isset($params['sf_method'])) {
			// enforce the sf_method requirement
			if (!in_array(strtolower($params['sf_method']),
					$this->requirements['sf_method'])) {
				return false;
			}
