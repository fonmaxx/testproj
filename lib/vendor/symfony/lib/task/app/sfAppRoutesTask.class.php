...
EOF;
	...
	protected function execute($arguments = array(), $options = array()) {
		...
		// display
		$arguments['name'] ? $this
						->outputRoute($arguments['application'],
								$arguments['name'])
	...
	protected function outputRoutes($application) {
		$this
				->logSection('app',
						sprintf('Current routes for application "%s"',
								$application));
		...
		foreach ($this->routes as $name => $route) {
			...
			$method = isset($requirements['sf_method']) ? strtoupper(
							is_array($requirements['sf_method']) ? implode(
											', ', $requirements['sf_method'])
									: $requirements['sf_method']) : 'ANY';
		...
		$this
				->log(
						sprintf($format1,
								$this->formatter->format('Name', 'COMMENT'),
								$this->formatter->format('Method', 'COMMENT'),
								$this->formatter->format('Pattern', 'COMMENT')));
		foreach ($this->routes as $name => $route) {
			$requirements = $route->getRequirements();
			$method = isset($requirements['sf_method']) ? strtoupper(
							is_array($requirements['sf_method']) ? implode(
	...
	protected function outputRoute($application, $name) {
		$this
				->logSection('app',
						sprintf('Route "%s" for application "%s"', $name,
								$application));
		...
		$this
				->log(
						sprintf('%s        %s',
								$this->formatter->format('Class', 'COMMENT'),
								get_class($route)));
		...
		$this
				->log(
						sprintf('%s     %s',
								$this->formatter->format('Defaults', 'COMMENT'),
								$defaults));
		...
		$this
				->log(
						sprintf('%s       %s',
								$this->formatter->format('Tokens', 'COMMENT'),
								$tokens));
	}
