...
abstract class sfData {
	protected $deleteCurrentData = true, $object_references = array();
	...
	public function getFiles($element = null) {
		...
		if (is_array($element)) {
			...
			$files = sfFinder::type('file')->name('*.yml')->sort_by_name()
					->in($element);
		} else {
			throw new sfInitializationException(
					sprintf(
							'You must give an array, a directory or a file to sfData::getFiles() (%s given).',
							$element));
		}
