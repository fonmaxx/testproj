...
class sfDebug {
	...
	public static function symfonyInfoAsArray() {
		return array('version' => SYMFONY_VERSION,
	...
	public static function phpInfoAsArray() {
		$values = array('php' => phpversion(), 'os' => php_uname(),
		...
		if ($values['extensions']) {
			foreach ($values['extensions'] as $key => $extension) {
				$values['extensions'][$key] = phpversion($extension) ? sprintf(
								'%s (%s)', $extension, phpversion($extension))
						: $extension;
			}
		}
	...
	public static function globalsAsArray() {
		$values = array();
		foreach (array('cookie', 'server', 'get', 'post', 'files', 'env',
				'session') as $name) {
	...
	public static function requestAsArray(sfRequest $request = null) {
		...
		return array('options' => $request->getOptions(),
				'parameterHolder' => self::flattenParameterHolder(
						$request->getParameterHolder(), true),
				'attributeHolder' => self::flattenParameterHolder(
						$request->getAttributeHolder(), true),);
	}
	...
	public static function responseAsArray(sfResponse $response = null) {
		...
		return array(
				'status' => array('code' => $response->getStatusCode(),
						'text' => $response->getStatusText()),
				'options' => $response->getOptions(),
				'cookies' => method_exists($response, 'getCookies') ? $response
								->getCookies() : array(),
				'httpHeaders' => method_exists($response, 'getHttpHeaders') ? $response
								->getHttpHeaders() : array(),
				'javascripts' => method_exists($response, 'getJavascripts') ? $response
								->getJavascripts('ALL') : array(),
				'stylesheets' => method_exists($response, 'getStylesheets') ? $response
								->getStylesheets('ALL') : array(),
				'metas' => method_exists($response, 'getMetas') ? $response
								->getMetas() : array(),
				'httpMetas' => method_exists($response, 'getHttpMetas') ? $response
								->getHttpMetas() : array(),);
	}
	...
	public static function userAsArray(sfUser $user = null) {
		...
		$data = array('options' => $user->getOptions(),
				'attributeHolder' => self::flattenParameterHolder(
						$user->getAttributeHolder(), true),
				'culture' => $user->getCulture(),);

		if ($user instanceof sfBasicSecurityUser) {
			$data = array_merge($data,
					array('authenticated' => $user->isAuthenticated(),
