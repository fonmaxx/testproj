...
abstract class sfWebDebugPanel {
	protected $webDebug = null, $status = sfLogger::INFO;
	...
	public function getToggler($element, $title = 'Toggle details') {
		return '<a href="#" onclick="sfWebDebugToggle(\'' . $element
				. '\'); return false;" title="' . $title . '"><img src="'
				. $this->webDebug->getOption('image_root_path')
				. '/toggle.gif" alt="' . $title . '"/></a>';
	}
	...
	public function getToggleableDebugStack($debugStack) {
		...
		$html .= '<div class="sfWebDebugDebugInfo" id="' . $element
				. '" style="display:none">';
		foreach ($debugStack as $j => $trace) {
			...
			$isProjectFile = $file
					&& 0 === strpos($file, sfConfig::get('sf_root_dir'))
					&& !preg_match('/(cache|plugins|vendor)/', $file);

			$html .= sprintf('<span%s>#%s &raquo; ',
					$isProjectFile ? ' class="sfWebDebugHighlight"' : '',
					$keys[$j] + 1);
			...
			$html .= sprintf('from %s line %s',
					$this->formatFileLink($file, $line), $line);
			$html .= '</span><br/>';
		}
		$html .= "</div>\n";
	...
	public function formatFileLink($file, $line = null, $text = null) {
		...
		if ($linkFormat = sfConfig::get('sf_file_link_format',
			...
			return sprintf(
					...
					htmlspecialchars($shortFile, ENT_QUOTES,
	...
	public function formatSql($sql) {
		return preg_replace(
				'/\b(UPDATE|SET|SELECT|FROM|AS|LIMIT|ASC|COUNT|DESC|WHERE|LEFT JOIN|INNER JOIN|RIGHT JOIN|ORDER BY|GROUP BY|IN|LIKE|DISTINCT|DELETE|INSERT|INTO|VALUES)\b/',
				'<span class="sfWebDebugLogInfo">\\1</span>', $sql);
	}
}
