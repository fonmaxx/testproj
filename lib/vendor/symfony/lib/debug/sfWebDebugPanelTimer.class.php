...
class sfWebDebugPanelTimer extends sfWebDebugPanel {
	...
	public function __construct(sfWebDebug $webDebug) {
		...
		$this->webDebug->getEventDispatcher()
				->connect('debug.web.filter_logs', array($this, 'filterLogs'));
	}

	public function getTitle() {
		return '<img src="' . $this->webDebug->getOption('image_root_path')
				. '/time.png" alt="Time" /> ' . $this->getTotalTime() . ' ms';
	}
	...
	public function getPanelContent() {
		if (sfTimerManager::getTimers()) {
			...
			foreach (sfTimerManager::getTimers() as $name => $timer) {
				$panel .= sprintf(
						'<tr><td class="sfWebDebugLogType">%s</td><td class="sfWebDebugLogNumber" style="text-align: right">%d</td><td style="text-align: right">%.2f</td><td style="text-align: right">%d</td></tr>',
						$name, $timer->getCalls(),
						$timer->getElapsedTime() * 1000,
						$totalTime ? ($timer->getElapsedTime() * 1000 * 100
										/ $totalTime) : 'N/A');
			}
			$panel .= '</table>';
	...
	protected function getTotalTime() {
		return null !== self::$startTime ? sprintf('%.0f',
						(microtime(true) - self::$startTime) * 1000) : 0;
	}
}
