...
class sfWebDebugPanelLogs extends sfWebDebugPanel {
	...
	public function getPanelContent() {
		$event = $this->webDebug->getEventDispatcher()
				->filter(new sfEvent($this, 'debug.web.filter_logs'),
      ...
      <tr>
		...
		foreach ($logs as $log) {
			...
			$html .= sprintf(
					...
					ucfirst($priority), $log['type'], $line_nb,
					'<img src="'
							. $this->webDebug->getOption('image_root_path')
							. '/' . $priority . '.png" alt="'
							. ucfirst($priority) . '"/>',
					class_exists($log['type'], false) ? $this
									->formatFileLink($log['type'])
							: $log['type'],
		...
		foreach ($this->webDebug->getLogger()->getTypes() as $type) {
			$types[] = '<a href="#" onclick="sfWebDebugToggleMessages(\''
					. $type . '\'); return false;">' . $type . '</a>';
		}
      ...
      <ul id="sfWebDebugLogMenu">
        ...
        <li><a href="#" onclick="sfWebDebugShowOnlyLogLines(\'info\'); return false;"><img src="'
				. $this->webDebug->getOption('image_root_path')
				. '/info.png" alt="Show only infos" /></a></li>
        <li><a href="#" onclick="sfWebDebugShowOnlyLogLines(\'warning\'); return false;"><img src="'
				. $this->webDebug->getOption('image_root_path')
				. '/warning.png" alt="Show only warnings" /></a></li>
        <li><a href="#" onclick="sfWebDebugShowOnlyLogLines(\'error\'); return false;"><img src="'
				. $this->webDebug->getOption('image_root_path')
				. '/error.png" alt="Show only errors" /></a></li>
        <li>' . implode("</li>\n<li>", $types)
				. '</li>
      </ul>
	...
	protected function formatLogLine($logLine) {
		...
		if (!$constants) {
			foreach (array('sf_app_dir', 'sf_root_dir', 'sf_symfony_lib_dir') as $constant) {
				$constants[realpath(sfConfig::get($constant))
		...
		$logLine = str_replace(array_keys($constants),
				array_values($constants), $logLine);

		$logLine = sfToolkit::pregtr($logLine,
				array(
