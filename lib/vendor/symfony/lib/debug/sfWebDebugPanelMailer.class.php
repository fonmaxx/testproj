...
class sfWebDebugPanelMailer extends sfWebDebugPanel {
	...
	public function __construct(sfWebDebug $webDebug) {
		...
		$this->webDebug->getEventDispatcher()
				->connect('mailer.configure',
						array($this, 'listenForMailerConfigure'));
	}

	public function getTitle() {
		if ($this->mailer && ($logger = $this->mailer->getLogger())
				&& $logger->countMessages()) {
			return '<img src="' . $this->webDebug->getOption('image_root_path')
					. '/email.png" alt="Emailer" /> '
					. $logger->countMessages();
		}
	}
	...
	public function getPanelContent() {
		...
		if (sfMailer::SINGLE_ADDRESS == $strategy) {
			$html[] = ' - <em>all emails are delivered to</em>: '
					. $this->mailer->getDeliveryAddress();
		}
	...
	protected function renderMessageInformation(Swift_Message $message) {
		...
		$to = null === $message->getTo() ? ''
				: implode(', ', array_keys($message->getTo()));

		$html = array();
		$html[] = sprintf('<h3>%s (to: %s) %s</h3>', $message->getSubject(),
				$to, $this->getToggler('sfWebDebugMailTemplate' . $i));
		$html[] = '<div id="sfWebDebugMailTemplate' . $i . '" style="display:'
				. (1 == $i ? 'block' : 'none') . '">';
		$html[] = '<pre>'
				. htmlentities($message->toString(), ENT_QUOTES,
						$message->getCharset()) . '</pre>';
		$html[] = '</div>';
