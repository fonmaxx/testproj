 ...
 */
class sfWebDebug {
	protected $dispatcher = null, $logger = null, $options = array(), $panels = array();
	...
	public function __construct(sfEventDispatcher $dispatcher,
			sfVarLogger $logger, array $options = array()) {
	...
	public function configure() {
		$this
				->setPanel('symfony_version',
						new sfWebDebugPanelSymfonyVersion($this));
	...
	public function injectToolbar($content) {
		...
		if (false !== $pos = $posFunction($content, '</head>')) {
			...
			$content = $substrFunction($content, 0, $pos) . $styles
		...
		if (false === $pos = $posrFunction($content, '</body>')) {
			...
			$content = $substrFunction($content, 0, $pos)
					...
					. $this->getJavascript() . '</script>' . $debug
					. $substrFunction($content, $pos);
		}
	...
	public function asHtml() {
		$current = isset(
				$this->options['request_parameters']['sfWebDebugPanel']) ? $this
						->options['request_parameters']['sfWebDebugPanel']
				: null;
			...
			if ($title = $panel->getTitle()) {
				if (($content = $panel->getPanelContent())
					...
					$titles[] = sprintf(
							...
							$panel->getStatus() ? ' class="sfWebDebug'
											. ucfirst(
													$this
															->getPriority(
																	$panel
							...
							$panel->getTitleUrl() ? ''
									: ' onclick="sfWebDebugShowDetailsFor(\''
											. $id . '\'); return false;"',
							$title);
					$panels[] = sprintf(
							'<div id="%s" class="sfWebDebugTop" style="display:%s"><h1>%s</h1>%s</div>',
							$id, $name == $current ? 'block' : 'none',
							$panel->getPanelTitle(), $content);
				} else {
      ...
      <div id="sfWebDebug">
        <div id="sfWebDebugBar">
          <a href="#" onclick="sfWebDebugToggleMenu(); return false;"><img src="'
				. $this->options['image_root_path']
				. '/sf.png" alt="Debug toolbar" /></a>

          <ul id="sfWebDebugDetails" class="sfWebDebugMenu">
            ' . implode("\n", $titles)
				. '
            <li class="last">
              <a href="#" onclick="document.getElementById(\'sfWebDebug\').style.display=\'none\'; return false;"><img src="'
				. $this->options['image_root_path']
