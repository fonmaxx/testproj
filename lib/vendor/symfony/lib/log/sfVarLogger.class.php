...
class sfVarLogger extends sfLogger {
	protected $logs = array(), $xdebugLogging = false;
	...
	public function initialize(sfEventDispatcher $dispatcher,
			$options = array()) {
		$this->xdebugLogging = isset($options['xdebug_logging']) ? $options['xdebug_logging']
				: false;
	...
	protected function doLog($message, $priority) {
		...
		$this->logs[] = array('priority' => $priority,
	...
	protected function getDebugBacktrace() {
		...
		foreach ($traces as $i => $trace) {
			$class = isset($trace['class']) ? $trace['class']
					: substr($file = basename($trace['file']), 0,
							strpos($file, '.'));

			if (!class_exists($class)
					|| (!in_array($class,
							array('sfLogger', 'sfEventDispatcher'))
							&& !is_subclass_of($class, 'sfLogger')
							&& !is_subclass_of($class, 'sfEventDispatcher'))) {
