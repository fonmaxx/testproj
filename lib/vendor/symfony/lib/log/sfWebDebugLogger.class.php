...
class sfWebDebugLogger extends sfVarLogger {
	protected $context = null, $webDebugClass = null, $webDebug = null;
	...
	public function initialize(sfEventDispatcher $dispatcher,
		...
		$this->webDebugClass = isset($options['web_debug_class']) ? $options['web_debug_class']
				: 'sfWebDebug';

		if (sfConfig::get('sf_web_debug')) {
			$dispatcher
					->connect('context.load_factories',
							array($this, 'listenForLoadFactories'));
			$dispatcher
					->connect('response.filter_content',
							array($this, 'filterResponseContent'));
		}
			...
			$this->dispatcher
					->notify(
							new sfEvent($this, 'application.log',
									array('priority' => sfLogger::ERR,
											sprintf($message, 'Strict notice'))));
			break;
		case E_NOTICE:
			$this->dispatcher
					->notify(
							new sfEvent($this, 'application.log',
									array('priority' => sfLogger::NOTICE,
											sprintf($message, 'Notice'))));
			break;
		case E_WARNING:
			$this->dispatcher
					->notify(
							new sfEvent($this, 'application.log',
									array('priority' => sfLogger::WARNING,
	...
	public function listenForLoadFactories(sfEvent $event) {
		$path = sprintf('%s/%s/images',
				$event->getSubject()->getRequest()->getRelativeUrlRoot(),
				sfConfig::get('sf_web_debug_web_dir'));
		$path = str_replace('//', '/', $path);

		$this->webDebug = new $this->webDebugClass($this->dispatcher, $this,
				array('image_root_path' => $path,
						'request_parameters' => $event->getSubject()
								->getRequest()->getParameterHolder()->getAll(),));
	}
	...
	public function filterResponseContent(sfEvent $event, $content) {
		...
		foreach (sfTimerManager::getTimers() as $name => $timer) {
			$messages[] = sprintf('%s %.2f ms (%d)', $name,
					$timer->getElapsedTime() * 1000, $timer->getCalls());
		}
		$this->dispatcher
				->notify(new sfEvent($this, 'application.log', $messages));
		...
		if (null === $this->webDebug || !$this->context->has('request')
				|| !$this->context->has('response')
				|| !$this->context->has('controller')
				|| $request->isXmlHttpRequest()
				|| strpos($response->getContentType(), 'html') === false
				|| '3' == substr($response->getStatusCode(), 0, 1)
				|| $this->context->getController()->getRenderMode()
						!= sfView::RENDER_CLIENT || $response->isHeaderOnly()) {
			return $content;
		}
