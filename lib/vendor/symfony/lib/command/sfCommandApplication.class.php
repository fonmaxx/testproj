...
abstract class sfCommandApplication {
	protected $commandManager = null, $trace = false, $verbose = true, $nowrite = false, $name = 'UNKNOWN', $version = 'UNKNOWN', $tasks = array(), $currentTask = null, $dispatcher = null, $options = array(), $formatter = null;
	...
	public function __construct(sfEventDispatcher $dispatcher,
			sfFormatter $formatter = null, $options = array()) {
		$this->dispatcher = $dispatcher;
		$this->formatter = null === $formatter ? $this
						->guessBestFormatter(STDOUT) : $formatter;
		...
		$argumentSet = new sfCommandArgumentSet(
				array(
						new sfCommandArgument('task',
								sfCommandArgument::REQUIRED,
								'The task to execute'),));
		$optionSet = new sfCommandOptionSet(
				array(
						new sfCommandOption('--help', '-H',
								sfCommandOption::PARAMETER_NONE,
								'Display this help message.'),
						new sfCommandOption('--quiet', '-q',
								sfCommandOption::PARAMETER_NONE,
								'Do not log messages to standard output.'),
						new sfCommandOption('--trace', '-t',
								sfCommandOption::PARAMETER_NONE,
								'Turn on invoke/execute tracing, enable full backtrace.'),
						new sfCommandOption('--version', '-V',
								sfCommandOption::PARAMETER_NONE,
								'Display the program version.'),
						new sfCommandOption('--color', '',
								sfCommandOption::PARAMETER_NONE,
	...
	public function registerTask(sfTask $task) {
		if (isset($this->tasks[$task->getFullName()])) {
			throw new sfCommandException(
					sprintf(
							'The task named "%s" in "%s" task is already registered by the "%s" task.',
							$task->getFullName(), get_class($task),
							get_class($this->tasks[$task->getFullName()])));
		}
		...
		foreach ($task->getAliases() as $alias) {
			if (isset($this->tasks[$alias])) {
				throw new sfCommandException(
						sprintf('A task named "%s" is already registered.',
								$alias));
			}
	...
	public function getTask($name) {
		if (!isset($this->tasks[$name])) {
			throw new sfCommandException(
					sprintf('The task "%s" does not exist.', $name));
		}
	...
	public function getLongVersion() {
		return sprintf('%s version %s', $this->getName(),
	...
	public function help() {
		$messages = array($this->formatter->format('Usage:', 'COMMENT'),
		...
		foreach ($this->commandManager->getOptionSet()->getOptions() as $option) {
			$messages[] = sprintf('  %-24s %s  %s',
					...
					$option->getShortcut() ? $this->formatter
									->format('-' . $option->getShortcut(), 'INFO')
							: '  ', $option->getHelp());
		}
	...
	public function renderException($e) {
		...
		if ($this->trace) {
			$messages[] = $title
					. str_repeat(' ', $len - $this->strlen($title));
		}
		...
		foreach ($messages as $message) {
			fwrite(STDERR,
					$this->formatter->format($message, 'ERROR', STDERR) . "\n");
		}
		fwrite(STDERR, "\n");

		if (null !== $this->currentTask
				&& $e instanceof sfCommandArgumentsException) {
			fwrite(STDERR,
					$this->formatter
							->format(
									sprintf($this->currentTask->getSynopsis(),
											$this->getName()), 'INFO', STDERR)
							. "\n");
			fwrite(STDERR, "\n");
		}
		...
		if ($this->trace) {
			fwrite(STDERR,
					$this->formatter->format("Exception trace:\n", 'COMMENT'));
			...
			for ($i = 0, $count = count($trace); $i < $count; $i++) {
				...
				fwrite(STDERR,
						sprintf(" %s%s%s at %s:%s\n", $class, $type, $function,
								$this->formatter->format($file, 'INFO', STDERR),
								$this->formatter->format($line, 'INFO', STDERR)));
			}
	...
	public function getTaskToExecute($name) {
		...
		if (false !== $pos = strpos($name, ':')) {
			...
			foreach ($this->tasks as $task) {
				if ($task->getNamespace()
						&& !in_array($task->getNamespace(), $namespaces)) {
			...
			if (!isset($abbrev[$namespace])) {
				throw new sfCommandException(
						sprintf(
								'There are no tasks defined in the "%s" namespace.',
								$namespace));
			} else if (count($abbrev[$namespace]) > 1) {
				throw new sfCommandException(
						sprintf('The namespace "%s" is ambiguous (%s).',
								$namespace, implode(', ', $abbrev[$namespace])));
			} else {
		...
		if (isset($abbrev[$name]) && count($abbrev[$name]) == 1) {
			return $this
					->getTask(
							$namespace ? $namespace . ':' . $abbrev[$name][0]
									: $abbrev[$name][0]);
		}
		...
		if (!isset($abbrev[$fullName])) {
			throw new sfCommandException(
					sprintf('Task "%s" is not defined.', $fullName));
		} else if (count($abbrev[$fullName]) > 1) {
			throw new sfCommandException(
					sprintf('Task "%s" is ambiguous (%s).', $fullName,
							implode(', ', $abbrev[$fullName])));
		} else {
	...
	protected function guessBestFormatter($stream) {
		return $this->isStreamSupportsColors($stream) ? new sfAnsiColorFormatter()
				: new sfFormatter();
	}
}
