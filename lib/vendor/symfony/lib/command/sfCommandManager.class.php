...
class sfCommandManager {
	protected $arguments = '', $errors = array(), $optionSet = null, $argumentSet = array(), $optionValues = array(), $argumentValues = array(), $parsedArgumentValues = array();
	 ...
	 */
	public function __construct(sfCommandArgumentSet $argumentSet = null,
			sfCommandOptionSet $optionSet = null) {
		if (null === $argumentSet) {
	...
	public function process($arguments = null) {
		...
		while (!in_array($argument = array_shift($this->arguments),
				array('', null))) {
			if ('--' == $argument) {
				// stop options parsing
				$this->parsedArgumentValues = array_merge(
						$this->parsedArgumentValues, $this->arguments);
				break;
			}
		...
		foreach ($this->argumentSet->getArguments() as $argument) {
			if (array_key_exists($position, $this->parsedArgumentValues)) {
				if ($argument->isArray()) {
					$this->argumentValues[$argument->getName()] = array_slice(
							$this->parsedArgumentValues, $position);
					break;
				} else {
					$this->argumentValues[$argument->getName()] = $this
							->parsedArgumentValues[$position];
				}
			}
			++$position;
		}
		...
		if (count($this->parsedArgumentValues)
				< $this->argumentSet->getArgumentRequiredCount()) {
			$this->errors[] = 'Not enough arguments.';
		} else if (count($this->parsedArgumentValues)
				> $this->argumentSet->getArgumentCount()) {
			$this->errors[] = sprintf('Too many arguments ("%s" given).',
					implode(' ', $this->parsedArgumentValues));
		}
	}
	...
	protected function parseShortOption($argument) {
		...
		for ($i = 0, $count = strlen($argument); $i < $count; $i++) {
			...
			if (!$this->optionSet->hasShortcut($shortcut)) {
				$this->errors[] = sprintf('The option "-%s" does not exist.',
						$shortcut);
				continue;
			}
			...
			if ($option->isParameterRequired()) {
				if ($i + 1 < strlen($argument)) {
					...
					if (count($this->arguments)
						...
						$this->errors[] = sprintf(
								'Option "-%s" requires an argument', $shortcut);
						$value = null;
					}
				}
	...
	protected function parseLongOption($argument) {
		if (false !== strpos($argument, '=')) {
			...
			if (!$this->optionSet->hasOption($name)) {
				$this->errors[] = sprintf('The "--%s" option does not exist.',
						$name);
				return;
			}
			...
			if (!$option->acceptParameter()) {
				$this->errors[] = sprintf(
						'Option "--%s" does not take an argument.', $name);
				$value = true;
			}
		} else {
			$name = $argument;
