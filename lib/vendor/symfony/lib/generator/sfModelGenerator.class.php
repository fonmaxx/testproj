...
abstract class sfModelGenerator extends sfGenerator {
	protected $configuration = null, $primaryKey = array(), $modelClass = '', $params = array(), $config = array(), $formObject = null;
	...
	public function generate($params = array()) {
		...
		$this
				->setGeneratedModuleName(
						'auto' . ucfirst($this->params['moduleName']));

		// theme exists?
		$theme = isset($this->params['theme']) ? $this->params['theme']
				: 'default';
		$this->setTheme($theme);
		$themeDir = $this->generatorManager->getConfiguration()
				->getGeneratorTemplate($this->getGeneratorClass(), $theme, '');
		if (!is_dir($themeDir)) {
			throw new sfConfigurationException(
					sprintf('The theme "%s" does not exist.', $theme));
		}
		...
		// generate files
		$this
				->generatePhpFiles($this->generatedModuleName,
						sfFinder::type('file')->relative()->in($themeDir));

		// move helper file
		if (file_exists(
				$file = $this->generatorManager->getBasePath() . '/'
						. $this->getGeneratedModuleName() . '/lib/helper.php')) {
			@rename($file,
					$this->generatorManager->getBasePath() . '/'
							. $this->getGeneratedModuleName() . '/lib/Base'
							. ucfirst($this->moduleName)
							. 'GeneratorHelper.class.php');
		}
	...
	public function getActionsBaseClass() {
		return isset($this->params['actions_base_class']) ? $this
						->params['actions_base_class'] : 'sfActions';
	}
	...
	public function getSingularName() {
		return isset($this->params['singular']) ? $this->params['singular']
				: sfInflector::underscore($this->getModelClass());
	}
	...
	public function getPluralName() {
		return isset($this->params['plural']) ? $this->params['plural']
				: $this->getSingularName() . '_list';
	}
	...
	public function getI18nCatalogue() {
		return isset($this->params['i18n_catalogue']) ? $this
						->params['i18n_catalogue'] : 'messages';
	}
	...
	public function getRetrieveByPkParamsForAction($indent) {
		...
		foreach ($this->getPrimaryKeys() as $pk) {
			$params[] = sprintf("\$request->getParameter('%s')",
					sfInflector::underscore($pk));
		}
	...
	public function getPrimaryKeyUrlParams($prefix = '', $full = false) {
		...
		foreach ($this->getPrimaryKeys() as $pk) {
			...
			if ($full) {
				$params[] = sprintf("%s='.%s->%s()", $fieldName, $prefix,
						$this->getColumnGetter($fieldName, false));
			} else {
				$params[] = sprintf("%s='.%s", $fieldName,
						$this->getColumnGetter($fieldName, true, $prefix));
			}
		}
	 ...
	 * @return string PHP code
	 */
	abstract public function getColumnGetter($column, $developed = false,
			$prefix = '');
	...
	public function getLinkToAction($actionName, $params, $pk_link = false) {
		...
		return '[?php echo link_to(__(\'' . $params['label']
				. '\', array(), \'' . $this->getI18nCatalogue() . '\'), \''
...
EOF;
	...
	public function renderField($field) {
		...
		if ($renderer = $field->getRenderer()) {
			$html = sprintf(
					...
					$this->asPhp($renderer), $html,
					$this->asPhp($field->getRendererArguments()));
		} else if ($field->isComponent()) {
			return sprintf(
					"get_component('%s', '%s', array('type' => 'list', '%s' => \$%s))",
					$this->getModuleName(), $field->getName(),
					$this->getSingularName(), $this->getSingularName());
		} else if ($field->isPartial()) {
			return sprintf(
					"get_partial('%s/%s', array('type' => 'list', '%s' => \$%s))",
					$this->getModuleName(), $field->getName(),
					$this->getSingularName(), $this->getSingularName());
		} else if ('Date' == $field->getType()) {
			$html = sprintf(
					"false !== strtotime($html) ? format_date(%s, \"%s\") : '&nbsp;'",
					$html, $field->getConfig('date_format', 'f'));
		} else if ('Boolean' == $field->getType()) {
			$html = sprintf(
					"get_partial('%s/list_field_boolean', array('value' => %s))",
					$this->getModuleName(), $html);
		}

		if ($field->isLink()) {
			$html = sprintf("link_to(%s, '%s', \$%s)", $html,
					$this->getUrlForAction('edit'), $this->getSingularName());
		}
	...
	public function getI18NString($key) {
		...
		foreach ($this->configuration
				->getContextConfiguration($context, $fields) as $field) {
			$vars[] = '\'%%' . $field->getName() . '%%\' => '
					. $this->renderField($field);
		}

		return sprintf("__('%s', array(%s), '%s')", $value,
				implode(', ', $vars), $this->getI18nCatalogue());
	}
	...
	public function getFormObject() {
		if (null === $this->formObject) {
			$class = null === $this->configuration ? $this->getModelClass()
							. 'Form' : $this->configuration->getFormClass();
	...
	public function getFormMultipartHtml() {
		if (isset($this->params['non_verbose_templates'])
				&& $this->params['non_verbose_templates']) {
	...
	protected function validateParameters($params) {
		foreach (array('model_class', 'moduleName') as $key) {
			if (!isset($params[$key])) {
				throw new sfParseException(
						sprintf(
								'sfModelGenerator must have a "%s" parameter.',
								$key));
			}
		}
		...
		if (!class_exists($params['model_class'])) {
			throw new sfInitializationException(
					sprintf(
							'Unable to generate a module for non-existent model "%s".',
							$params['model_class']));
		}
	...
	protected function loadConfiguration() {
		try {
			$this->generatorManager->getConfiguration()
					->getGeneratorTemplate($this->getGeneratorClass(),
							$this->getTheme(), '../parts/configuration.php');
		} catch (sfException $e) {
			return null;
		}
		...
		$this->getGeneratorManager()
				->save($basePath,
						$this->evalTemplate('../parts/configuration.php'));

		require_once $this->getGeneratorManager()->getBasePath() . '/'
				. $basePath;

		$class = 'Base' . ucfirst($this->getModuleName())
				. 'GeneratorConfiguration';
		foreach ($config->getLibDirs($this->getModuleName()) as $dir) {
			if (!is_file(
					$configuration = $dir . '/' . $this->getModuleName()
							. 'GeneratorConfiguration.class.php')) {
				continue;
			}
		...
		foreach ($this->config as $context => $value) {
			...
			throw new InvalidArgumentException(
					sprintf(
							'Your generator configuration contains some errors for the "%s" context. The following configuration cannot be parsed: %s.',
	...
	public function getUrlForAction($action) {
		if (isset($this->params['route_prefix'])) {
			return 'list' == $action ? $this->params['route_prefix']
					: $this->params['route_prefix'] . '_' . $action;
		} else {
