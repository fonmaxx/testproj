...
class sfPostgreSQLSessionStorage extends sfDatabaseSessionStorage {
	...
	public function sessionDestroy($id) {
		...
		throw new sfDatabaseException(
				sprintf(
						'sfPostgreSQLSessionStorage cannot destroy session id "%s".',
						$id));
	}
	...
	public function sessionGC($lifetime) {
		...
		if (!@pg_query($this->db, $sql)) {
			throw new sfDatabaseException(
					'sfPostgreSQLSessionStorage cannot delete old sessions.');
		}
	...
	public function sessionRead($id) {
		...
		// delete the record associated with this id
		$sql = 'SELECT ' . $db_data_col . ' FROM ' . $db_table . ' WHERE '
				. $db_id_col . ' = \'' . $id . '\'';
		...
		if ($result != false && @pg_num_rows($result) == 1) {
			...
			$sql = 'INSERT INTO ' . $db_table . ' (' . $db_id_col . ', '
					. $db_data_col . ', ' . $db_time_col . ') VALUES (\'' . $id
					. '\', \'\', ' . time() . ')';
			...
			throw new sfDatabaseException(
					sprintf(
							'sfPostgreSQLSessionStorage cannot create new record for id "%s".',
							$id));
		}
	}
	...
	public function sessionWrite($id, $data) {
		...
		// delete the record associated with this id
		$sql = 'UPDATE ' . $db_table . ' SET ' . $db_data_col . ' = \'' . $data
				. '\', ' . $db_time_col . ' = ' . time() . ' WHERE '
				. $db_id_col . ' = \'' . $id . '\'';
		...
		throw new sfDatabaseException(
				sprintf(
						'sfPostgreSQLSessionStorage cannot write session data for id "%s".',
						$id));
	}
}
