...
class sfSessionTestStorage extends sfStorage {
	protected $sessionId = null, $sessionData = array();
	...
	public function initialize($options = null) {
		if (!isset($options['session_path'])) {
			throw new InvalidArgumentException(
					'The "session_path" option is mandatory for the sfSessionTestStorage class.');
		}
		...
		$this->sessionId = null !== $this->options['session_id'] ? $this
						->options['session_id']
				: (array_key_exists('session_id', $_SERVER) ? $_SERVER['session_id']
						: null);

		if ($this->sessionId) {
			// we read session data from temp file
			$file = $this->options['session_path'] . DIRECTORY_SEPARATOR
					. $this->sessionId . '.session';
			$this->sessionData = file_exists($file) ? unserialize(
	...
	public function shutdown() {
		if ($this->sessionId) {
			...
			umask($current_umask);
			file_put_contents(
					$this->options['session_path'] . DIRECTORY_SEPARATOR
							. $this->sessionId . '.session',
