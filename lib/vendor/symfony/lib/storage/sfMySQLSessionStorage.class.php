...
class sfMySQLSessionStorage extends sfDatabaseSessionStorage {
	...
	public function sessionDestroy($id) {
		...
		throw new sfDatabaseException(
				sprintf('%s cannot destroy session id "%s" (%s).',
						get_class($this), $id, $this->db_error()));
	}
	...
	public function sessionGC($lifetime) {
		...
		if (!$this->db_query($sql)) {
			throw new sfDatabaseException(
					sprintf('%s cannot delete old sessions (%s).',
							get_class($this), $this->db_error()));
		}
	...
	public function sessionRead($id) {
		...
		if ($result != false && $this->db_num_rows($result) == 1) {
			...
			// can't create record
			throw new sfDatabaseException(
					sprintf('%s cannot create new record for id "%s" (%s).',
							get_class($this), $id, $this->db_error()));
		}
	}
	...
	public function sessionWrite($id, $data) {
		...
		throw new sfDatabaseException(
				sprintf('%s cannot write session data for id "%s" (%s).',
						get_class($this), $id, $this->db_error()));
	}
