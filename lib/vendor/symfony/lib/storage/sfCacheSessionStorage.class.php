...
class sfCacheSessionStorage extends sfStorage {
	protected $id = null, $context = null, $dispatcher = null, $request = null, $response = null, $cache = null, $data = array(), $dataChanged = false;
	...
	public function initialize($options = array()) {
		...
		if (isset($this->options['cache']) && $this->options['cache']['class']) {
			$this->cache = new $this->options['cache']['class'](
					is_array($this->options['cache']['param']) ? $this
									->options['cache']['param'] : array());
		} else {
			throw new InvalidArgumentException(
					'sfCacheSessionStorage requires cache option.');
		}
		...
		if (strpos($cookie, ':') !== false) {
			...
			if ($signature
					== sha1($id . ':' . $this->options['session_cookie_secret'])) {
				// cookie is valid
				$this->id = $id;
			} else {
		...
		if (empty($this->id)) {
			...
			$this->id = md5(
					rand(0, 999999) . $ip . $ua
							. $this->options['session_cookie_secret']);

			if (sfConfig::get('sf_logging_enabled')) {
				$this->dispatcher
						->notify(
								new sfEvent($this, 'application.log',
										array('New session created')));
			}

			// only send cookie when id is issued
			$this->response
					->setCookie($this->options['session_name'],
							$this->id . ':'
									. sha1(
											$this->id . ':'
													. $this
			...
			if (sfConfig::get('sf_logging_enabled')) {
				$this->dispatcher
						->notify(
								new sfEvent($this, 'application.log',
										array('Restored previous session')));
			}
		}
		session_id($this->id);
	...
	public function regenerate($destroy = false) {
		...
		$this->id = md5(
				rand(0, 999999) . $_SERVER['REMOTE_ADDR'] . $ua
						. $this->options['session_cookie_secret']);
		...
		if (sfConfig::get('sf_logging_enabled')) {
			$this->dispatcher
					->notify(
							new sfEvent($this, 'application.log',
									array(
											'new session created due to expiraton')));
		}
	}
		...
		if ($this->dataChanged === true) {
			...
			if (sfConfig::get('sf_logging_enabled')) {
				$this->dispatcher
						->notify(
								new sfEvent($this, 'application.log',
										array('Storing session to cache')));
			}
		}
	}
}
