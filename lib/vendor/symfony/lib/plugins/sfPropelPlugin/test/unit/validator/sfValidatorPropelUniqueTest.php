...
$validator = new sfValidatorPropelUnique(
		array('model' => 'Author', 'column' => 'name'));
...
try {
	...
	$t
			->is(isset($errors['name']), true,
					'->clean() throws an error on the column');
} catch (Exception $e) {
	$t->fail('->clean() throws an error on the column');
	$t->diag('    ' . $e->getMessage());
}
...
try {
	...
	$t
			->is(isset($errors['author_name']), true,
					'->clean() throws an error on the field');
} catch (Exception $e) {
	$t->fail('->clean() throws an error on the field');
	$t->diag('    ' . $e->getMessage());
}
