...
class my_lime_test extends lime_test {
	public function is_array_explicit($test, $target, $prefix = '') {
		foreach ($test as $key => $value) {
			if (is_array($value)) {
				$this
						->is_array_explicit($value, $target[$key],
								$prefix . ' ' . $key);
			} else {
				$this
						->is($value, $target[$key],
								sprintf('%s %s is %s', $prefix, $key, $value));
			}
		}
	}
	...
	public function is_line_by_line($exp1, $exp2) {
		...
		for ($i = 0; $i < $nb_lines; $i++) {
			...
			$this
					->is(trim($array_exp1[$i]), trim($array_exp2[$i]),
							sprintf('Line %d matches %s', $i, $array_exp1[$i]));
		}
	}
}
...
require_once(dirname(__FILE__)
		. '/../../lib/addon/sfPropelDatabaseSchema.class.php');
...
$p = new sfPropelDatabaseSchema();
$new_yml_transformed = $p
		->convertNewToOldYaml(
				sfYaml::load(dirname(__FILE__) . '/fixtures/new_schema.yml'));
...
$p = new sfPropelDatabaseSchema();
$old_yml_transformed = $p
		->convertOldToNewYaml(
				sfYaml::load(dirname(__FILE__) . '/fixtures/schema.yml'));
