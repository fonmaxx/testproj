			...
			$this->getColumnNameMaxLength()
					- strlen(
							$this
									->underscore(
											$tables['middleTable']
													->getClassname()) . '_list')) ?> => new sfWidgetFormPropelChoice(array('model' => '<?php echo $tables['relatedTable']
			->getClassname() ?>', 'add_empty' => true)),
<?php endforeach; ?>
    ));
...
<?php if ($column->isPrimaryKey())
		continue ?>
      '<?php echo $this->translateColumnName($column) ?>'<?php echo str_repeat(
			' ', $this->getColumnNameMaxLength() - strlen($column->getName())) ?> => <?php echo $this
			...
			$this->getColumnNameMaxLength()
					- strlen(
							$this
									->underscore(
											$tables['middleTable']
													->getClassname()) . '_list')) ?> => new sfValidatorPropelChoice(array('model' => '<?php echo $tables['relatedTable']
			->getClassname() ?>', 'required' => false)),
<?php endforeach; ?>
    ));

    $this->widgetSchema->setNameFormat('<?php echo $this
		->underscore($this->table->getClassname()) ?>_filters[%s]');
...
<?php foreach ($this->getManyToManyTables() as $tables) : ?>
  public function add<?php echo $tables['middleTable']->getPhpName() ?>ListColumnCriteria(Criteria $criteria, $field, $values)
  {
    ...
    $criteria->addJoin(<?php echo constant(
			$tables['middleTable']->getPhpName() . '::PEER') ?>::<?php echo strtoupper(
			$tables['column']->getName()) ?>, <?php echo constant(
			$this->table->getPhpName() . '::PEER') ?>::<?php echo strtoupper(
			$this->getPrimaryKey()->getName()) ?>);

    $value = array_pop($values);
    $criterion = $criteria->getNewCriterion(<?php echo constant(
			$tables['middleTable']->getPhpName() . '::PEER') ?>::<?php echo $tables['relatedColumn']
			->getName() ?>, $value);

    foreach ($values as $value)
    {
      $criterion->addOr($criteria->getNewCriterion(<?php echo constant(
			$tables['middleTable']->getPhpName() . '::PEER') ?>::<?php echo $tables['relatedColumn']
			->getName() ?>, $value));
    }
