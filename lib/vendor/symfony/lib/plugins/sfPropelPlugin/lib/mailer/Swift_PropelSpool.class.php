...
class Swift_PropelSpool extends Swift_ConfigurableSpool {
	protected $model = null, $column = null, $method = null;
	 ...
	 */
	public function __construct($model = 'MailMessage', $column = 'message',
			$method = 'doSelect') {
		$this->model = $model;
		$this->column = $column;
		$this->method = $method;
	...
	public function queueMessage(Swift_Mime_Message $message) {
		...
		if (!$object instanceof BaseObject) {
			throw new InvalidArgumentException(
					'The mailer message object must be a BaseObject object.');
		}

		$model = constant($this->model . '::PEER');
		$method = 'set'
				. call_user_func(array($model, 'translateFieldName'),
						$this->column, BasePeer::TYPE_FIELDNAME,
	...
	public function flushQueue(Swift_Transport $transport,
		...
		foreach ($objects as $object) {
			if (is_resource($object->getMessage())) {
				$message = unserialize(
						stream_get_contents($object->getMessage()));
			} else {
