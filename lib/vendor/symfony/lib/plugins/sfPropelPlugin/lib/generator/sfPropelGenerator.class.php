...
class sfPropelGenerator extends sfModelGenerator {
	protected $tableMap = null, $dbMap = null;
	...
	protected function loadPrimaryKeys() {
		...
		if (!count($this->primaryKey)) {
			throw new sfException(
					sprintf(
							'Cannot generate a module for a model without a primary key (%s)',
							$this->modelClass));
		}
	}
	...
	protected function loadMapBuilderClasses() {
		...
		$this->tableMap = call_user_func(
				array($this->modelClass . 'Peer', 'getTableMap'));
	...
	public function getColumnGetter($column, $developed = false, $prefix = '') {
		try {
			$getter = 'get'
					. call_user_func(
							array(constant($this->getModelClass() . '::PEER'),
									'translateFieldName'), $column,
		...
		return sprintf('$%s%s->%s()', $prefix, $this->getSingularName(),
				$getter);
	}
	...
	public function getDefaultFieldsConfiguration() {
		...
		foreach ($this->getTableMap()->getColumns() as $column) {
			...
			$fields[$name] = array_merge(
					array('is_link' => (Boolean) $column->isPrimaryKey(),
							'is_real' => true, 'is_partial' => false,
							'is_component' => false,
							'type' => $this->getType($column),),
					isset($this->config['fields'][$name]) ? $this
									->config['fields'][$name] : array());
		}

		foreach ($this->getManyToManyTables() as $tables) {
			$name = sfInflector::underscore(
					$tables['middleTable']->getClassname()) . '_list';
			$names[] = $name;
			$fields[$name] = array_merge(
					array('is_link' => false, 'is_real' => false,
							'is_partial' => false, 'is_component' => false,
							'type' => 'Text',),
					isset($this->config['fields'][$name]) ? $this
									->config['fields'][$name] : array());
		}
	...
	public function getFieldsConfiguration($context) {
		...
		foreach ($this->getTableMap()->getColumns() as $column) {
			$name = $this->translateColumnName($column);
			$names[] = $name;
			$fields[$name] = isset($this->config[$context]['fields'][$name]) ? $this
							->config[$context]['fields'][$name] : array();
		}
