...
class sfPropelFormGenerator extends sfGenerator {
	...
	public function generate($params = array()) {
		...
		if (!isset($this->params['connection'])) {
			throw new sfParseException(
					'You must specify a "connection" parameter.');
		}
		...
		if (!file_exists($file)) {
			...
			file_put_contents($file,
					$this->evalTemplate('sfPropelFormBaseTemplate.php'));
		}

		// create a form class for every Propel class
		foreach ($this->dbMap->getTables() as $tableName => $table) {
			$behaviors = $table->getBehaviors();
			if (isset($behaviors['symfony']['form'])
					&& 'false' === $behaviors['symfony']['form']) {
				continue;
			}
			...
			$packages = explode('.',
					constant(
							constant($table->getClassname() . '::PEER')
									. '::CLASS_DEFAULT'));
			array_pop($packages);
			if (false
					=== $pos = array_search($this->params['model_dir_name'],
							$packages)) {
				throw new InvalidArgumentException(
						sprintf(
								'Unable to find the model dir name (%s) in the package %s.',
								$this->params['model_dir_name'],
								constant(
										constant(
												$table->getClassname()
														. '::PEER')
			...
			file_put_contents(
					$baseDir . '/base/Base' . $table->getClassname()
							. 'Form.class.php',
					$this->evalTemplate('sfPropelFormGeneratedTemplate.php'));
			if (!file_exists(
					$classFile = $baseDir . '/' . $table->getClassname()
							. 'Form.class.php')) {
				file_put_contents($classFile,
						$this->evalTemplate('sfPropelFormTemplate.php'));
			}
		}
	}
				...
				if ($column->isForeignKey() && $column->isPrimaryKey()
					...
					foreach ($table->getColumns() as $relatedColumn) {
						if ($relatedColumn->isForeignKey()
							...
							$tables[] = array('middleTable' => $table,
									'relatedTable' => $this
											->getForeignTable($relatedColumn),
									'column' => $column,
		...
		foreach ($this->table->getColumns() as $column) {
			if (!$column->isPrimaryKey() && $column->isForeignKey()) {
				$names[] = array(
						...
						$column->getPhpName(), $column->isNotNull(), false);
			}
		}

		foreach ($this->getManyToManyTables() as $tables) {
			$names[] = array($tables['relatedTable']->getClassname(),
					$tables['middleTable']->getClassname(), false, true);
		}
	...
	public function getWidgetOptionsForColumn(ColumnMap $column) {
		...
		if (!$column->isPrimaryKey() && $column->isForeignKey()) {
			$options[] = sprintf('\'model\' => \'%s\', \'add_empty\' => %s',
					$this->getForeignTable($column)->getClassname(),
					$column->isNotNull() ? 'false' : 'true');

			$refColumn = $this->getForeignTable($column)
					->getColumn($column->getRelatedColumnName());
			if (!$refColumn->isPrimaryKey()) {
				$options[] = sprintf('\'key_method\' => \'get%s\'',
						$refColumn->getPhpName());
			}
		}
	...
	public function getValidatorOptionsForColumn(ColumnMap $column) {
		...
		if ($column->isForeignKey()) {
			$options[] = sprintf('\'model\' => \'%s\', \'column\' => \'%s\'',
					$this->getForeignTable($column)->getClassname(),
					$this->translateColumnName($column, true));
		} else if ($column->isPrimaryKey()) {
			$options[] = sprintf(
					'\'choices\' => array($this->getObject()->get%s()), \'empty_value\' => $this->getObject()->get%1$s()',
					$this
							->translateColumnName($column, false,
			...
			case PropelColumnTypes::SMALLINT:
				$options[] = sprintf('\'min\' => %s, \'max\' => %s', -32768,
						32767);
				break;

			case PropelColumnTypes::INTEGER:
				$options[] = sprintf('\'min\' => %s, \'max\' => %s',
	...
	public function getColumnNameMaxLength() {
		...
		foreach ($this->getManyToManyTables() as $tables) {
			if (($m = strlen(
					$this->underscore($tables['middleTable']->getClassname())
							. '_list')) > $max) {
				$max = $m;
			}
		}
	...
	public function getPrimaryKeyColumNamesAsString() {
		return sprintf('array(\'%s\')',
				implode('\', \'', $this->getPrimaryKeyColumNames()));
	}
	...
	public function isI18n() {
		return method_exists(constant($this->table->getClassname() . '::PEER'),
				'getI18nModel');
	}
	...
	public function getI18nModel() {
		return call_user_func(
				array(constant($this->table->getClassname() . '::PEER'),
						'getI18nModel'));
	}
	...
	public function getUniqueColumnNames() {
		...
		foreach (call_user_func(
				array(constant($this->table->getClassname() . '::PEER'),
						'getUniqueColumnNames')) as $unique) {
			$uniqueColumn = array();
			foreach ($unique as $column) {
				$uniqueColumn[] = $this
						->translateColumnName($this->table->getColumn($column));
			}
	...
	public function translateColumnName($column, $related = false,
			$to = BasePeer::TYPE_FIELDNAME) {
		$peer = $related ? constant(
						$column->getTable()->getDatabaseMap()
								->getTable($column->getRelatedTableName())
								->getPhpName() . '::PEER')
				: constant($column->getTable()->getPhpName() . '::PEER');
		$field = $related ? $column->getRelatedName()
	...
	protected function loadBuilders() {
		...
		$classes = sfFinder::type('file')->name('*TableMap.php')
				->in(
						$this->generatorManager->getConfiguration()
								->getModelDirs());
		foreach ($classes as $class) {
			$omClass = basename($class, 'TableMap.php');
			if (class_exists($omClass)
					&& is_subclass_of($omClass, 'BaseObject')
					&& constant($omClass . 'Peer::DATABASE_NAME')
