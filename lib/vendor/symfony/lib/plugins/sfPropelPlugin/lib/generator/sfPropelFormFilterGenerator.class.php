...
class sfPropelFormFilterGenerator extends sfPropelFormGenerator {
	...
	public function generate($params = array()) {
		...
		$file = sfConfig::get('sf_lib_dir')
				. '/filter/BaseFormFilterPropel.class.php';
		if (!file_exists($file)) {
			...
			file_put_contents($file,
					$this->evalTemplate('sfPropelFormFilterBaseTemplate.php'));
		}

		// create a form class for every Propel class
		foreach ($this->dbMap->getTables() as $tableName => $table) {
			$behaviors = $table->getBehaviors();
			if (isset($behaviors['symfony']['filter'])
					&& 'false' === $behaviors['symfony']['filter']) {
				continue;
			}
			...
			file_put_contents(
					...
					$this
							->evalTemplate(
									'sfPropelFormFilterGeneratedTemplate.php'));
			if (!file_exists(
					$classFile = $baseDir . '/' . $table->getClassname()
							. 'FormFilter.class.php')) {
				file_put_contents($classFile,
						$this->evalTemplate('sfPropelFormFilterTemplate.php'));
			}
		}
	}
	...
	public function getValidatorOptionsForColumn(ColumnMap $column) {
		...
		if ($column->isForeignKey()) {
			...
			$options[] = sprintf('\'model\' => \'%s\', \'column\' => \'%s\'',
					$column->getTable()->getClassname(),
					$this->translateColumnName($column));
		} else {
			switch ($column->getType()) {
			case PropelColumnTypes::BOOLEAN:
