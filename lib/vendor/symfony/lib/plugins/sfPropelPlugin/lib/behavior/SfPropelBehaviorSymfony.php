...
class SfPropelBehaviorSymfony extends SfPropelBehaviorBase {
	protected $parameters = array('form' => 'true', 'filter' => 'true',);
	...
	public function modifyDatabase() {
		foreach ($this->getDatabase()->getTables() as $table) {
			...
			// symfony behaviors
			if (!isset($behaviors['symfony_behaviors'])
					&& $this->getBuildProperty('propel.builder.addBehaviors')) {
				$class = Propel::importClass(
						$this
								->getBuildProperty(
...
EOF;
	...
	public function staticMethods() {
		...
		foreach ($this->getTable()->getUnices() as $unique) {
			$unices[] = sprintf("array('%s')",
					implode("', '", $unique->getColumns()));
		}
