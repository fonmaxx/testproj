...
public function __call(\$method, \$arguments)
{
  ...
  {
    throw new sfException(sprintf('Call to undefined method Base{$this
				->getTable()->getPhpName()}::%s', \$method));
  }
...
// symfony_behaviors behavior
    ...
    {
      if (false !== \$sf_hook_retval = call_user_func(\$sf_hook, 'Base{$this
				->getTable()->getPhpName()}Peer', \$values, \$con))
      {
...
EOF;
			...
			file_put_contents($file,
					sprintf($code, var_export(unserialize($behaviors), true)));
			return true;
		}
	}
	...
	protected function getBehaviorsFilePath($absolute = false) {
		...
		return $base
				. ClassTools::getFilePath(
						$this->getTable()->getPackage() . '.om',
						sprintf('Base%sBehaviors',
