...
class SfPropelBehaviorTimestampable extends SfPropelBehaviorBase {
	protected $parameters = array('create_column' => null,
			'update_column' => null,);
	...
	public function preInsert() {
		...
		if ($column = $this->getParameter('create_column')) {
			return <<<EOF
if (!\$this->isColumnModified({$this->getTable()->getColumn($column)
					->getConstantName()}))
{
...
EOF;
	...
	public function preSave() {
		...
		if ($column = $this->getParameter('update_column')) {
			return <<<EOF
if (\$this->isModified() && !\$this->isColumnModified({$this->getTable()
					->getColumn($column)->getConstantName()}))
{
  \$this->set{$this->getTable()->getColumn($column)->getPhpName()}(time());
}
