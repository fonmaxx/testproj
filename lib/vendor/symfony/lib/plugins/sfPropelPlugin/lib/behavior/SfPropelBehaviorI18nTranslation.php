...
class SfPropelBehaviorI18nTranslation extends SfPropelBehaviorBase {
	protected $parameters = array('culture_column' => null,);
	...
	public function filterDoSave($line) {
		...
		$phpName = $foreignKey->getPhpName() ? $foreignKey->getPhpName()
				: $foreignKey->getForeignTable()->getPhpName();
		$refPhpName = $foreignKey->getRefPhpName() ? $foreignKey
						->getRefPhpName() : $this->getTable()->getPhpName();
		$search = sprintf('$this->a%s->isModified()', $phpName);
		$insert = sprintf(
				' || ($this->a%s->getCulture() && $this->a%1$s->getCurrent%s()->isModified())',
				$phpName, $refPhpName);
	...
	public function getForeignKey() {
		...
		throw new Exception(
				'The foreign key that references the I18N model could not be found.');
	}
	...
	public function getCultureColumn() {
		return $this->getTable()
				->getColumn($this->getParameter('culture_column'));
	}
}
