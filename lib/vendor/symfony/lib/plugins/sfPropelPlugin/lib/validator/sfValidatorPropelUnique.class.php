	...
	protected function doClean($values) {
		...
		foreach ($columns as $i => $column) {
			...
			$colName = call_user_func(
					array(constant($this->getOption('model') . '::PEER'),
		...
		$error = new sfValidatorError($this, 'invalid',
				array('column' => implode(', ', $this->getOption('column'))));
		...
		throw new sfValidatorErrorSchema($this,
				array(isset($fields[0]) ? $fields[0] : $columns[0] => $error));
	}
	...
	protected function getPrimaryKeys() {
		if (null === $this->getOption('primary_key')) {
			...
			foreach ($tableMap->getColumns() as $column) {
				...
				$primaryKeys[] = call_user_func(
						array(constant($this->getOption('model') . '::PEER'),
								'translateFieldName'), $column->getPhpName(),
		...
		if (!is_array($this->getOption('primary_key'))) {
			$this
					->setOption('primary_key',
							array($this->getOption('primary_key')));
		}
