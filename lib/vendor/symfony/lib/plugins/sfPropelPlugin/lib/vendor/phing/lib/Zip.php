...
class Archive_Zip {
	...
	function create($p_filelist, $p_params = 0) {
		...
		if ($this
				->_check_parameters($p_params,
						array('no_compression' => false, 'add_path' => "",
								'remove_path' => "",
	...
	function extract($p_params = 0) {
		...
		if ($this
				->_check_parameters($p_params,
						array('extract_as_string' => false, 'add_path' => '',
								'remove_path' => '',
								'remove_all_path' => false,
	...
	function delete($p_params) {
		...
		if ($this
				->_check_parameters($p_params,
						array('by_name' => '', 'by_index' => '',
								'by_ereg' => '', 'by_preg' => '')) != 1) {
			return 0;
		}

		// ----- Check that at least one rule is set
		if (($p_params['by_name'] == '') && ($p_params['by_index'] == '')
				&& ($p_params['by_ereg'] == '') && ($p_params['by_preg'] == '')) {
			$this
					->_errorLog(ARCHIVE_ZIP_ERR_INVALID_PARAMETER,
	...
	function duplicate($p_archive) {
		...
		// ----- Look if the $p_archive is a string (so a filename)
			...
			if (!is_file($p_archive)) {
				$this
						->_errorLog(ARCHIVE_ZIP_ERR_MISSING_FILE,
	...
	function _create($p_list, &$p_result_list, &$p_params) {
		...
		$v_result = $this
				->_addList($p_list, $p_result_list, $p_add_dir, $p_remove_dir,
	...
	function _add($p_list, &$p_result_list, &$p_params) {
		...
		while ($v_size != 0) {
			$v_read_size = ($v_size < ARCHIVE_ZIP_READ_BLOCK_SIZE ? $v_size
		...
		$v_header_list = array();
		if (($v_result = $this
				->_addFileList($p_list, $v_header_list, $p_add_dir,
		...
		for ($i = 0, $v_count = 0; $i < sizeof($v_header_list); $i++) {
			...
			if ($v_header_list[$i]['status'] == 'ok') {
				if (($v_result = $this
						->_writeCentralFileHeader($v_header_list[$i])) != 1) {
					fclose($v_zip_temp_fd);
					$this->_closeFd();
					@unlink($v_zip_temp_name);
			...
			$this
					->_convertHeader2FileInfo($v_header_list[$i],
							$p_result_list[$i]);
		}
		...
		if (($v_result = $this
				->_writeCentralHeader(
						$v_count + $v_central_dir['entries'], $v_size,
	 ...
	 *
	 */
	function _addList($p_list, &$p_result_list, $p_add_dir, $p_remove_dir,
			$p_remove_all_dir, &$p_params) {
		$v_result = 1;
		...
		// ----- Create the central dir footer
		if (($v_result = $this
				->_writeCentralHeader($v_count, $v_size, $v_offset, $v_comment))
				!= 1) {
			// ----- Reset the file list
			unset($v_header_list);
	 ...
	 *
	 */
	function _addFileList($p_list, &$p_result_list, $p_add_dir, $p_remove_dir,
			$p_remove_all_dir, &$p_params) {
		$v_result = 1;
		$v_header = array();
		...
		for ($j = 0; ($j < count($p_list)) && ($v_result == 1); $j++) {
			...
			if ((is_file($p_filename))
					|| ((is_dir($p_filename)) && !$p_remove_all_dir)) {
				// ----- Add the file
				if (($v_result = $this
						->_addFile($p_filename, $v_header, $p_add_dir,
			...
			if (is_dir($p_filename)) {
				...
				while ($p_hitem = readdir($p_hdir)) {
					...
					if (is_file($v_path . $p_hitem)) {
						...
						// ----- Add the file
						if (($v_result = $this
								->_addFile($v_path . $p_hitem, $v_header,
					...
					// ----- Recursive call to _addFileList()
						...
						$v_result = $this
								->_addFileList($p_temp_list, $p_result_list,
										$p_add_dir, $p_remove_dir,
										$p_remove_all_dir, $p_params);
			...
			//      if (substr($p_filename, 0, strlen($p_remove_dir)) == $p_remove_dir)
 {
				...
				if ($v_compare == 2) {
					...
					$v_stored_filename = substr($p_filename,
							strlen($p_remove_dir));
				}
			}
		}
		...
		if ((isset($p_params[ARCHIVE_ZIP_PARAM_PRE_ADD]))
			...
			// header.
			eval(
					'$v_result = ' . $p_params[ARCHIVE_ZIP_PARAM_PRE_ADD]
							. '(ARCHIVE_ZIP_PARAM_PRE_ADD, $v_local_header);');
			if ($v_result == 0) {
		...
		if ((isset($p_params[ARCHIVE_ZIP_PARAM_POST_ADD]))
			...
			// header.
			eval(
					'$v_result = ' . $p_params[ARCHIVE_ZIP_PARAM_POST_ADD]
							. '(ARCHIVE_ZIP_PARAM_POST_ADD, $v_local_header);');
			if ($v_result == 0) {
				// ----- Ignored
				$v_result = 1;
			}
	...
	function _writeFileHeader(&$p_header) {
		...
		$v_binary_data = pack("VvvvvvVVVvv", 0x04034b50, $p_header['version'],
				$p_header['flag'], $p_header['compression'], $v_mtime,
				$v_mdate, $p_header['crc'], $p_header['compressed_size'],
				$p_header['size'], strlen($p_header['stored_filename']),
				$p_header['extra_len']);
		...
		if (strlen($p_header['stored_filename']) != 0) {
			fputs($this->_zip_fd, $p_header['stored_filename'],
	...
	function _writeCentralFileHeader(&$p_header) {
		...
		if ($p_header['comment_len'] != 0) {
			fputs($this->_zip_fd, $p_header['comment'],
	...
	function _extractByRule(&$p_file_list, &$p_params) {
		...
		for ($i = 0, $v_nb_extracted = 0; $i < $v_central_dir['entries']; $i++) {
			...
			if ((isset($p_params[ARCHIVE_ZIP_PARAM_BY_NAME]))
				...
				for ($j = 0; ($j < sizeof($p_params[ARCHIVE_ZIP_PARAM_BY_NAME]))
						&& (!$v_extract); $j++) {
 ...
 elseif ($v_header['stored_filename']
			...
			// ----- Look for extract by index rule
				...
				for ($j = $j_start; ($j
						< sizeof($p_params[ARCHIVE_ZIP_PARAM_BY_INDEX]))
						&& (!$v_extract); $j++) {

					if (($i
							>= $p_params[ARCHIVE_ZIP_PARAM_BY_INDEX][$j]['start'])
							&& ($i
									<= $p_params[ARCHIVE_ZIP_PARAM_BY_INDEX][$j]['end'])) {
						$v_extract = true;
					}
			...
			if ($v_extract) {
				...
				if ($p_params[ARCHIVE_ZIP_PARAM_EXTRACT_AS_STRING]) {
					...
					// ----- Extracting the file
					if (($v_result = $this
							->_extractFileAsString($v_header, $v_string)) != 1) {
						// ----- Close the zip file
						$this->_closeFd();
					...
					// ----- Get the only interesting attributes
					if (($v_result = $this
							->_convertHeader2FileInfo($v_header,
					...
					// ----- Extracting the file
					if (($v_result = $this
							->_extractFile($v_header, $p_path, $p_remove_path,
	...
	function _extractFile(&$p_entry, $p_path, $p_remove_path,
		...
		// ----- Look for path to remove
			...
			if ($this
					->_tool_PathInclusion($p_remove_path, $p_entry['filename'])
					== 2) {
		...
		if ($p_entry['status'] == 'ok') {
			...
			// ----- Check the directory availability and create it if necessary
				...
				if (($v_result = $this
						->_dirCheck($v_dir_to_check,
								(($p_entry['external'] & 0x00000010)
										== 0x00000010))) != 1) {
		...
		if ($p_entry['status'] == 'ok') {
			...
			if (!(($p_entry['external'] & 0x00000010) == 0x00000010)) {
				...
				if ($p_entry['compressed_size'] == $p_entry['size']) {
					...
					if (($v_dest_file = @fopen($p_entry['filename'], 'wb'))
							== 0) {
		...
		if ((isset($p_params[ARCHIVE_ZIP_PARAM_POST_EXTRACT]))
			...
			// header.
			eval(
					'$v_result = ' . $p_params[ARCHIVE_ZIP_PARAM_POST_EXTRACT]
	...
	function _readFileHeader(&$p_header) {
		...
		if ($p_header['mdate'] && $p_header['mtime']) {
			...
			$p_header['mtime'] = mktime($v_hour, $v_minute, $v_seconde,
					$v_month, $v_day, $v_year);
	...
	function _readCentralFileHeader(&$p_header) {
		...
		if ($p_header['filename_len'] != 0)
			$p_header['filename'] = fread($this->_zip_fd,
					$p_header['filename_len']);
		else
			$p_header['filename'] = '';
		...
		// ----- Get comment
		if ($p_header['comment_len'] != 0)
			$p_header['comment'] = fread($this->_zip_fd,
					$p_header['comment_len']);
		else
			$p_header['comment'] = '';
	...
	function _readEndCentralDir(&$p_central_dir) {
		...
		if ($v_data['comment_size'] != 0)
			$p_central_dir['comment'] = fread($this->_zip_fd,
					$v_data['comment_size']);
		else
	...
	function _deleteByRule(&$p_result_list, &$p_params) {
		...
		for ($i = 0, $v_nb_extracted = 0; $i < $v_central_dir['entries']; $i++) {
			...
			$v_result = $this
					->_readCentralFileHeader($v_header_list[$v_nb_extracted]);
			if ($v_result != 1) {
				// ----- Clean
				$this->_closeFd();
					...
					if (substr($p_params[ARCHIVE_ZIP_PARAM_BY_NAME][$j], -1)
						...
						if ((strlen(
							...
							$v_found = true;
								...
								& 0x00000010) == 0x00000010) /* Indicates a folder */
								&& ($v_header_list[$v_nb_extracted]['stored_filename']
										. '/'
										== $p_params[ARCHIVE_ZIP_PARAM_BY_NAME][$j])) {
							$v_found = true;
 ...
 elseif ($v_header_list[$v_nb_extracted]['stored_filename']
		...
		if ($v_nb_extracted > 0) {
			...
			// ----- Creates a temporay file
			$v_zip_temp_name = ARCHIVE_ZIP_TEMPORARY_DIR
					. uniqid('archive_zip-') . '.tmp';
			...
			for ($i = 0; $i < sizeof($v_header_list); $i++) {
				...
				// ----- Read the file header
				if (($v_result = $this->_readFileHeader($v_header_list[$i]))
						!= 1) {
					// ----- Clean
					$this->_closeFd();
					$v_temp_zip->_closeFd();
				...
				$v_result = $this
						->_tool_CopyBlock($this->_zip_fd, $v_temp_zip->_zip_fd,
			...
			for ($i = 0; $i < sizeof($v_header_list); $i++) {
				// ----- Create the file header
				$v_result = $v_temp_zip
						->_writeCentralFileHeader($v_header_list[$i]);
				if ($v_result != 1) {
					// ----- Clean
					$v_temp_zip->_closeFd();
					$this->_closeFd();
			...
			// ----- Create the central dir footer
			$v_result = $v_temp_zip
					->_writeCentralHeader(sizeof($v_header_list), $v_size,
							$v_offset, $v_comment);
			if ($v_result != 1) {
				// ----- Clean
				unset($v_header_list);
	...
	function _dirCheck($p_dir, $p_is_dir = false) {
		...
		if (!@mkdir($p_dir, 0777)) {
			$this
					->_errorLog(ARCHIVE_ZIP_ERR_DIR_CREATE_FAIL,
	...
	function _merge(&$p_archive_to_add) {
		...
		$v_result = $p_archive_to_add
				->_readEndCentralDir($v_central_dir_to_add);
		if ($v_result != 1) {
			$this->_closeFd();
		...
		if (($v_result = $this
				->_writeCentralHeader(
						$v_central_dir['entries']
								+ $v_central_dir_to_add['entries'], $v_size,
						$v_offset, $v_comment)) != 1) {
			$this->_closeFd();
			$p_archive_to_add->_closeFd();
			@fclose($v_zip_temp_fd);
		...
		if ($p_dir != "") {
			...
			for ($i = sizeof($v_list) - 1; $i >= 0; $i--) {
				...
				if ($v_list[$i] == ".") {
					...
					$i--;
				} else if (($v_list[$i] == "") && ($i != (sizeof($v_list) - 1))
						&& ($i != 0)) {
	...
	function _tool_PathInclusion($p_dir, $p_path) {
		...
		$i = 0;
		$j = 0;
		while (($i < $v_list_dir_size) && ($j < $v_list_path_size)
				&& ($v_result)) {
	...
	function _tool_CopyBlock($p_src, $p_dest, $p_size, $p_mode = 0) {
		...
		if ($p_mode == 0) {
			while ($p_size != 0) {
				$v_read_size = ($p_size < ARCHIVE_ZIP_READ_BLOCK_SIZE ? $p_size
						: ARCHIVE_ZIP_READ_BLOCK_SIZE);
