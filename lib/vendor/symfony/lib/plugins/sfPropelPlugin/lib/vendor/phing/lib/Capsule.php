...
class Capsule {
	...
	function display($__template) {
		...
		if (isset($this->vars['this'])) {
			throw new Exception(
					"Assigning a variable named \$this to a context conflicts with class namespace.");
		}
		...
		ini_set('include_path',
				$this->templatePath . PATH_SEPARATOR . $__old_inc_path);
		...
		if (!empty($php_errormsg)) {
			throw new Exception(
					"Unable to parse template " . $__template . ": "
							. $php_errormsg);
		}
	}
	...
	function parse($template, $outputFile = null, $append = false) {
		...
		if ($outputFile !== null) {
			$outputFile = $this
					->resolvePath($outputFile, $this->outputDirectory);

			$flags = null;
			if ($append)
				$flags = FILE_APPEND;

			if (!file_put_contents($outputFile, $output, $flags)
					&& $output != "") {
	...
	protected function resolvePath($file, $basepath) {
		...
		// also account for C:\ style path
				&& !($file{1} == ':'
						&& ($file{2} == DIRECTORY_SEPARATOR || $file{2} == '/'))) {
			if ($basepath != null) {
