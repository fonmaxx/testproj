...
class StringHelper {
	...
	public static function root($qualifiedName, $separator = '.') {
		...
		return ($loc === false) ? $qualifiedName
				: substr($qualifiedName, 0, $loc);
	}
	...
	public static function isBoolean($s) {
		...
		return (boolean) in_array($test,
				array_merge(self::$FALSE_VALUES, self::$TRUE_VALUES));
	}
	...
	public static function endsWith($check, $string) {
		if ($check === "" || $check === $string) {
			...
			return (strpos(strrev($string), strrev($check)) === 0) ? true
					: false;
		}
	}
	...
	public static function substring($string, $startpos, $endpos = -1) {
		...
		if ($startpos > $len - 1 || $startpos < 0) {
			trigger_error(
					"substring(), Startindex out of bounds must be 0<n<$len",
					E_USER_ERROR);
		}
		if ($endpos > $len - 1 || $endpos < $startpos) {
			trigger_error(
					"substring(), Endindex out of bounds must be $startpos<n<"
							. ($len - 1), E_USER_ERROR);
		}
		if ($startpos === $endpos) {
