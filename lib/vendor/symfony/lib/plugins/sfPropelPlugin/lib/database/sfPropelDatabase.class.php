...
class sfPropelDatabase extends sfPDODatabase {
	...
	static public function getConfiguration() {
		return array(
				'propel' => Propel::getConfiguration(
						PropelConfiguration::TYPE_ARRAY));
	}
	...
	public function initialize($parameters = null, $name = 'propel') {
		...
		if (!Propel::getConfiguration(PropelConfiguration::TYPE_OBJECT)
				->getParameter('datasources.default')) {
	...
	public function setDefaultConfig() {
		Propel::getConfiguration(PropelConfiguration::TYPE_OBJECT)
				->setParameter('datasources.default',
						$this->getParameter('datasource'));
	}
	...
	public function addConfig() {
		if ($dsn = $this->getParameter('dsn')) {
			...
			$options = array('dsn', 'phptype', 'hostspec', 'database',
					'username', 'password', 'port', 'protocol', 'encoding',
					'persistent', 'socket', 'compat_assoc_lower',
					'compat_rtrim_string');
			...
			$this
					->setParameter('options',
							array_merge(
									...
									array(
											'ATTR_PERSISTENT' => $this
													->getParameter('persistent'))));
		}

		$propelConfiguration = Propel::getConfiguration(
				PropelConfiguration::TYPE_OBJECT);

		if ($this->hasParameter('debug')) {
			$propelConfiguration
					->setParameter('debugpdo.logging',
							sfToolkit::arrayDeepMerge(
									$propelConfiguration
											->getParameter('debugpdo.logging',
													array()),
									$this->getParameter('debug')));
		}

		$event = new sfEvent($propelConfiguration,
				'propel.filter_connection_config',
				array('name' => $this->getParameter('datasource'),
						'database' => $this));
		$event = sfProjectConfiguration::getActive()->getEventDispatcher()
				->filter($event,
						array('adapter' => $this->getParameter('phptype'),
								'connection' => array(
		...
		$propelConfiguration
				->setParameter(
						'datasources.' . $this->getParameter('datasource'),
