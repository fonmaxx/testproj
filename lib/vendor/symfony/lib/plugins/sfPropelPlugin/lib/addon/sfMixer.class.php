 ...
 */
class sfMixer {
	static protected $mixins = array(), $mixinParameters = array(), $mixinInstances = array();
	...
	static public function register($name, $callable) {
		...
		if (!$hook && isset(self::$mixins[$name])) {
			throw new Exception(
					sprintf(
							'The class "%s" has already a mixin for method "%s".',
							$class, $mixinMethod));
		}
		...
		self::$mixinParameters[$name][] = array('lazy' => $lazy,
	...
	static public function callMixins($hookName = null, $moreParams = array()) {
		...
		if ('__call' == $function) {
			if ($callable = self::getCallable($class . ':' . $method)) {
				...
				throw new Exception(
						sprintf('Call to undefined method %s::%s.', $class,
								$method));
			}
		} else {
			$hookName = $hookName ? $hookName : $method;
			foreach (self::getCallables($class . ':' . $method . ':'
					. $hookName) as $callable) {
