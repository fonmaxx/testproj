...
class sfPropelDatabaseSchema {
		...
		if (isset($schema['classes'])) {
			...
			foreach ($schema['classes'] as $className => $classParams) {
				...
				if (isset($classParams['columns'])) {
					$tableParams = array_merge($classParams['columns'],
							$tableParams);
				...
				if ($tableAttributes || $classParams) {
					$tableParams['_attributes'] = array_merge(
							$tableAttributes, $classParams);
				}
	...
	public function asXML() {
		...
		$xml .= "<database name=\"$this->connection_name\""
				. $this->getAttributesFor($this->database) . ">\n";
			...
			foreach ($this->database['_propel_behaviors'] as $name => $parameters) {
				...
				if (is_array($parameters) && count($parameters)) {
					...
					foreach ($parameters as $key => $value) {
						$xml .= "    <parameter name=\"$key\" value=\"{$this
								->fixXMLBoolean($value)}\"/>\n";
					}
					$xml .= "  </behavior>\n";
				} else {
		...
		foreach ($this->getChildren($this->database) as $tb_name => $table) {
			...
			if ($isNestedSet = isset($table['_nestedSet'])) {
				...
				if (!isset($treeConfig['left']) || !isset($treeConfig['right'])
						|| !isset($table[$treeConfig['left']])
						|| !isset($table[$treeConfig['right']])
						|| (isset($treeConfig['scope'])
								&& !isset($table[$treeConfig['scope']]))) {
					throw new sfException(
							sprintf(
									'Incorrect NestedSet configuration for "%s" table.',
									$tb_name));
				}
			}

			$xml .= "\n  <table name=\"$tb_name\""
					. $this->getAttributesFor($table);
			if ($isNestedSet) {
				$xml .= ' treeMode="NestedSet"';
			}
			if (isset($table['_behaviors'])) {
				$xml .= sprintf(" behaviors=\"%s\"",
						htmlspecialchars(serialize($table['_behaviors'])),
						ENT_QUOTES, sfConfig::get('sf_charset'));
			}
			$xml .= ">\n";
				...
				foreach ($table['_propel_behaviors'] as $behavior_name => $parameters) {
					if ($parameters) {
						...
						foreach ($parameters as $param_name => $param_value) {
							$xml .= "      <parameter name=\"$param_name\" value=\"{$this
									->fixXMLBoolean($param_value)}\" />\n";
						}
						$xml .= "    </behavior>\n";
					} else {
						$xml .= "    <behavior name=\"$behavior_name\" />\n";
			...
			foreach ($this->getChildren($table) as $col_name => $column) {
				...
				if ($isNestedSet && in_array($col_name, $treeConfig)) {
					...
					} elseif (isset($treeConfig['scope'])
							&& $col_name == $treeConfig['scope']) {
						$column['treeScopeKey'] = 'true';
					}
				}

				$xml .= "    <column name=\"$col_name\""
						. $this
								->getAttributesForColumn($tb_name, $col_name,
										$column);
			}
			...
			if (isset($table['_foreignKeys'])) {
				foreach ($table['_foreignKeys'] as $fkey_name => $fkey) {
					if (!isset($fkey['foreignTable'])) {
						if (!isset($fkey['foreignClass'])
								|| !$fkey['foreignTable'] = $this
										->findTable($fkey['foreignClass'])) {
							throw new sfException(
									sprintf(
											'Unable to resolve foreign table for foreign key "%s": %s',
											$fkey_name,
											var_export($fkey, true)));
						}
					}
	...
	protected function fixYAMLDatabase() {
		...
		$this
				->setIfNotSet($this->database['_attributes'],
						'defaultIdMethod', 'native');
		$this
				->setIfNotSet($this->database['_attributes'], 'package',
						'lib.model');
	}
		...
		foreach ($this->getTables() as $i18n_table => $columns) {
			...
			if ($pos > 0 && $pos == strlen($i18n_table) - 5
				...
				if ($main_table) {
					...
					$this
							->setIfNotSet(
									$this->database[$main_table]['_attributes'],
									'isI18N', 1);
					$this
							->setIfNotSet(
									$this->database[$main_table]['_attributes'],
					...
					$this
							->setIfNotSet($this->database[$i18n_table], 'id',
									array('type' => 'integer',
											'required' => true,
											'primaryKey' => true,
											...
											'onDelete' => 'cascade',));
					$this
							->setIfNotSet($this->database[$i18n_table],
									'culture',
									array('isCulture' => true,
											'type' => 'varchar',
											'size' => '7',
											'required' => true,
											'primaryKey' => true,));
					...
					throw new sfException(
							sprintf(
									'Missing main table for internationalized table "%s".',
									$i18n_table));
				}
			}
		}
				...
				if ($attributes == null) {
					...
					if ($pos > 0 && $pos == strlen($column) - 3) {
						...
						if ($foreign_table) {
							...
							throw new sfException(
									sprintf(
											'Unable to resolve foreign table for column "%s".',
											$column));
						}
					}
					...
					if (!is_array($attributes)) {
						...
						$this->database[$table][$column] = $this
								->getAttributesFromCompactType($attributes);
					} else {
		...
		foreach ($this->getTables() as $tb_name => $table) {
			if (($tb_name == $table_name)
					|| (isset($table['_attributes']['phpName'])
							&& ($table['_attributes']['phpName']
									== sfInflector::camelize($table_name)
									|| $table['_attributes']['phpName']
											== $table_name)
							|| (sfInflector::underscore($table_name)
									== $tb_name))) {
				$table_match = $tb_name;
				break;
			}
	...
	protected function getAttributesForColumn($tb_name, $col_name, $column) {
		...
		if (is_array($column)) {
			foreach ($column as $key => $value) {
				if (!in_array($key,
						array('foreignClass', 'foreignTable',
								'foreignReference', 'fkPhpName',
								'fkRefPhpName', 'onDelete', 'onUpdate',
								'index', 'unique', 'sequence', 'inheritance'))) {
					$attributes_string .= " $key=\""
							. htmlspecialchars(
									$this->getCorrectValueFor($key, $value),
									ENT_QUOTES, sfConfig::get('sf_charset'))
							. "\"";
				}
			}
			if (isset($column['inheritance'])) {
				...
				$extended_package = isset(
						$this->database[$tb_name]['_attributes']['package']) ? $this
								->database[$tb_name]['_attributes']['package']
						: $this->database['_attributes']['package'];
				$extended_class = isset(
						$this->database[$tb_name]['_attributes']['phpName']) ? $this
								->database[$tb_name]['_attributes']['phpName']
				...
				foreach ($column['inheritance'] as $key => $class) {
					...
					$package = null;
					if (is_array($class)) {
						$package = isset($class['package']) ? $class['package']
								: null;
						$class = $class['phpName'];
					}

					$attributes_string .= vsprintf(
							'      <inheritance extends="%s.%s" key="%s" class="%s"%s />',
							array($extended_package, $extended_class, $key,
									$class,
			...
			throw new sfException(
					sprintf(
							'Incorrect settings for column "%s" of table "%s".',
							$col_name, $tb_name));
		}

		// conventions for foreign key attributes
		if (is_array($column)
				&& (isset($column['foreignTable'])
						|| isset($column['foreignClass']))) {
			if (isset($column['foreignTable'])) {
				...
				if (!$foreignTable) {
					...
					$foreignTable = sfInflector::underscore(
							$column['foreignClass']);
				}
				$attributes_string .= "    <foreign-key foreignTable=\""
						. $foreignTable . "\"";
			}
	...
	protected function getCorrectValueFor($key, $value) {
		$booleans = array('required', 'primaryKey', 'autoincrement',
				'autoIncrement', 'isI18N', 'isCulture');
	...
	public function loadXML($file) {
		...
		// database
		list($database_name, $database_attributes) = $this
				->getNameAndAttributes($schema->attributes());
		if ($database_name) {
		...
		foreach ($schema as $table) {
			list($table_name, $table_attributes) = $this
					->getNameAndAttributes($table->attributes());
			if ($table_name) {
			...
			foreach ($table->xpath('column') as $column) {
				list($column_name, $column_attributes) = $this
						->getNameAndAttributes($column->attributes());
				if ($column_name) {
					$database[$table_name][$column_name] = $column_attributes;
				} else {
					throw new sfException(
							'A column tag misses the name attribute.');
				}
			}
			...
			foreach ($table->xpath('foreign-key') as $foreign_key) {
				...
				if (isset($foreign_key['foreignTable'])) {
					...
					throw new sfException(
							'A foreign key misses the foreignTable attribute.');
				}
				...
				foreach ($foreign_key->xpath('reference') as $reference) {
					...
					foreach ($reference->attributes() as $reference_attribute_name => $reference_attribute_value) {
						$reference_attributes[$reference_attribute_name] = strval(
			...
			if (isset($this->database[$table]['_foreignKeys'])) {
				...
				foreach ($foreign_keys as $foreign_key_name => $foreign_key_attributes) {
					...
					if (count($foreign_key_attributes['references']) == 1) {
						...
						$this
								->database[$table][$reference['local']]['foreignTable'] = $foreign_key_attributes['foreignTable'];
						$this
								->database[$table][$reference['local']]['foreignReference'] = $reference['foreign'];
						if (isset($foreign_key_attributes['onDelete'])) {
							$this
									->database[$table][$reference['local']]['onDelete'] = $foreign_key_attributes['onDelete'];
						}
						if (isset($foreign_key_attributes['onUpdate'])) {
							$this
									->database[$table][$reference['local']]['onUpdate'] = $foreign_key_attributes['onUpdate'];
						}

						// remove complex foreign key
						unset(
								$this
										->database[$table]['_foreignKeys'][$foreign_key_name]);
					}

					$this
							->removeEmptyKey($this->database[$table],
									'_foreignKeys');
				}
			}
		}
		...
		foreach ($this->getTables() as $table => $columns) {
			if (isset($this->database[$table]['_indexes'])) {
				...
				foreach ($indexes as $index => $references) {
					...
					if (count($references) == 1
							&& false !== substr($index, 0, strlen($index) - 6)
							&& array_key_exists(
									substr($index, 0, strlen($index) - 6),
									$columns)) {
			...
			if (isset($this->database[$table]['_uniques'])) {
				...
				foreach ($uniques as $index => $references) {
					...
					if (count($references) == 1
							&& false !== substr($index, 0, strlen($index) - 7)
							&& array_key_exists(
									substr($index, 0, strlen($index) - 7),
									$columns)) {
						$reference = $references[0];
	...
	protected function fixXMLColumns() {
		foreach ($this->getTables() as $table => $columns) {
			foreach ($columns as $column => $attributes) {
				...
				$is_foreign_key = isset($attributes['type'])
						&& $attributes['type'] == 'integer'
						&& isset($attributes['foreignReference'])
						&& $attributes['foreignReference'] == 'id';
				$has_foreign_table = isset($attributes['foreignTable'])
						&& array_key_exists($attributes['foreignTable'],
								$this->getTables());
				$has_other_attribute = isset($attributes['onDelete']);
				if ($has_fk_name && $has_foreign_table && $is_foreign_key
						&& !$has_other_attribute) {
