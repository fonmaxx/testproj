...
class sfPropelBehavior {
	static protected $loaded = array(), $behaviors = array();

	static public function registerMethods($name, $callables) {
		if (!isset(self::$behaviors[$name])) {
			self::$behaviors[$name] = array('methods' => array(),
					'hooks' => array());
		}
		...
		foreach ($behaviors as $name => $parameters) {
			if (is_int($name)) {
				...
				foreach ($parameters as $key => $value) {
					sfConfig::set(
							'propel_behavior_' . $name . '_' . $class . '_'
									. $key, $value);
				}
			}
			...
			if (!isset(self::$behaviors[$name])) {
				throw new sfConfigurationException(
						sprintf('Propel behavior "%s" is not registered', $name));
			}
	...
	static protected function callableToString($callable) {
		return is_array($callable) ? (is_object($callable[0]) ? get_class(
								$callable[0]) : $callable[0]) . '::'
						. $callable[1] : var_export($callable, true);
	}
}
