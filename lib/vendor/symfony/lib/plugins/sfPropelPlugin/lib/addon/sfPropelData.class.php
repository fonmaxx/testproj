...
class sfPropelData extends sfData {
	protected $deletedClasses = array(), $con = null;
	...
	public function loadDataFromArray($data) {
		...
		foreach ($data as $class => $datas) {
			...
			$tableMap = $this->dbMap
					->getTable(
							constant(constant($class . '::PEER')
									. '::TABLE_NAME'));

			$column_names = call_user_func_array(
					array(constant($class . '::PEER'), 'getFieldNames'),
					array(BasePeer::TYPE_FIELDNAME));
			...
			foreach ($datas as $key => $data) {
				...
				if (!$obj instanceof BaseObject) {
					throw new RuntimeException(
							sprintf(
									'The class "%s" is not a Propel class. This probably means there is already a class named "%s" somewhere in symfony or in your project.',
									$class, $class));
				}
				...
				if (!is_array($data)) {
					throw new InvalidArgumentException(
							sprintf(
									'You must give a name for each fixture data entry (class %s).',
									$class));
				}
				...
				foreach ($data as $name => $value) {
					if (is_array($value) && 's' == substr($name, -1)) {
						...
						$this
								->loadMany2Many($obj, substr($name, 0, -1),
										$value);
					...
					if ($isARealColumn) {
						if ($column->isForeignKey() && null !== $value) {
							$relatedTable = $this->dbMap
									->getTable($column->getRelatedTableName());
							if (!isset(
									$this
											->object_references[$relatedTable
													->getPhpName() . '_'
													. $value])) {
								throw new InvalidArgumentException(
										sprintf(
												'The object "%s" from class "%s" is not defined in your data file.',
												$value,
							...
							$value = $this
									->object_references[$relatedTable
											->getPhpName() . '_' . $value]
									->getByName($column->getRelatedName(),
					...
					if (false !== $pos = array_search($name, $column_names)) {
						$obj->setByPosition($pos, $value);
							...
							array($obj,
									$method = 'set'
											. sfInflector::camelize($name)))) {
						$obj->$method($value);
					} else {
						throw new InvalidArgumentException(
								sprintf(
										'Column "%s" does not exist for class "%s".',
										$name, $class));
					}
				}
				$obj->save($this->con);
				...
				if (method_exists($obj, 'getPrimaryKey')) {
					$this
							->object_references[Propel::importClass(
									constant(
											constant($class . '::PEER')
													. '::CLASS_DEFAULT')) . '_'
									. $key] = $obj;
	...
	protected function loadMany2Many($obj, $middleTableName, $values) {
		...
		foreach ($middleTable->getColumns() as $column) {
			if ($column->isForeignKey()
					&& constant(
							constant(get_class($obj) . '::PEER')
									. '::TABLE_NAME')
							!= $column->getRelatedTableName()) {
				$relatedClass = $this->dbMap
						->getTable($column->getRelatedTableName())
						->getPhpName();
				break;
			}
		...
		if (!isset($relatedClass)) {
			throw new InvalidArgumentException(
					sprintf(
							'Unable to find the many-to-many relationship for object "%s".',
							get_class($obj)));
		}
		...
		foreach ($values as $value) {
			...
			$middle
					->$relatedSetter(
							$this->object_references[$relatedClass . '_'
											. $value]);
			$middle->save();
		}
	}
		...
		foreach ($files as $file) {
			...
			foreach (array_reverse($classes) as $class) {
				...
				if (!class_exists(constant($class . '::PEER'))) {
					throw new InvalidArgumentException(
							sprintf('Unknown class "%sPeer".', $class));
				}

				call_user_func(
						array(constant($class . '::PEER'), 'doDeleteAll'),
						$this->con);
	...
	protected function loadMapBuilders($connectionName) {
		...
		$files = sfFinder::type('file')->name('*TableMap.php')
				->in(sfProjectConfiguration::getActive()->getModelDirs());
		foreach ($files as $file) {
	 ...
	 */
	public function dumpData($directoryOrFile, $tables = 'all',
			$connectionName = 'propel') {
		...
		if (!is_dir($directoryOrFile)) {
			...
			foreach ($tables as $tableName) {
				...
				file_put_contents(
						sprintf("%s/%03d-%s.yml", $directoryOrFile, ++$i,
								$tableName),
						sfYaml::dump(
								array($tableName => $dumpData[$tableName]), 3));
			}
		}
	}
	...
	public function getData($tables = 'all', $connectionName = 'propel') {
		...
		foreach ($tables as $tableName) {
			...
			if ($hasParent) {
				$resultsSets[] = $this
						->fixOrderingOfForeignKeyDataInSameTable($resultsSets,
								$tableName, $fixColumn);
			} else {
				$in = array();
				foreach ($tableMap->getColumns() as $column) {
				...
				$stmt = $this->con
						->query(
								sprintf('SELECT %s FROM %s', implode(',', $in),
										constant(
												constant($tableName . '::PEER')
														. '::TABLE_NAME')));
						...
						foreach ($tableMap->getColumns() as $column) {
							...
							if ($column->isForeignKey()) {
								...
								if ($isPrimaryKey) {
									...
									$primaryKeys[$col] = $relatedTable
											->getPhpName() . '_' . $row[$col];
								} else {
									$values[$col] = $relatedTable->getPhpName()
											. '_' . $row[$col];

									$values[$col] = strlen($row[$col]) ? $relatedTable
													->getPhpName() . '_'
													. $row[$col] : '';
								}
							} elseif (!$isPrimaryKey
									|| ($isPrimaryKey
											&& !$tableMap->isUseIdGenerator())) {
