...
abstract class sfPropelBaseTask extends sfBaseTask {
	...
	protected function process(sfCommandManager $commandManager, $options) {
		...
		if ($commandManager->getOptionSet()->hasOption('phing-arg')) {
			$this->additionalPhingArgs = $commandManager
					->getOptionValue('phing-arg');
		}
	}

	protected function schemaToYML($checkSchema = self::CHECK_SCHEMA,
			$prefix = '') {
		$finder = sfFinder::type('file')->name('*schema.xml')
				->prune('doctrine');

		$schemas = array_unique(
				array_merge($finder->in(sfConfig::get('sf_config_dir')),
						$finder
								->in(
										$this->configuration
												->getPluginSubPaths('/config'))));
		...
		foreach ($schemas as $schema) {
			...
			$this
					->logSection('schema',
							sprintf('converting "%s" to YML', $schema));
			...
			$file = str_replace(basename($schema), $prefix . $yml_file_name,
					$schema);
	...
	protected function schemaToXML($checkSchema = self::CHECK_SCHEMA,
		...
		$dirs = array_merge(array(sfConfig::get('sf_config_dir')),
				$this->configuration->getPluginSubPaths('/config'));
		...
		foreach ($schemas as $schema) {
			...
			$customSchemaFilename = str_replace(
					array(
							...
							'plugins/', 'config/', '/', 'schema.yml'),
					array('', '', '', '_', 'schema.custom.yml'), $schema);
			$customSchemas = sfFinder::type('file')
					->name($customSchemaFilename)->in($dirs);

			foreach ($customSchemas as $customSchema) {
				$this
						->logSection('schema',
								sprintf('found custom schema %s', $customSchema));

				$customSchemaArray = sfYaml::load($customSchema);
				if (!isset($customSchemaArray['classes'])) {
					// Old schema syntax: we convert it
					$customSchemaArray = $dbSchema
							->convertOldToNewYaml($customSchemaArray);
				}
				$schemaArray = sfToolkit::arrayDeepMerge($schemaArray,
						$customSchemaArray);
			}
			...
			$this
					->logSection('schema',
							sprintf('converting "%s" to XML', $schema));
	...
	protected function copyXmlSchemaFromPlugins($prefix = '') {
		...
		$schemas = sfFinder::type('file')->name('*schema.xml')
				->prune('doctrine')->in($dirs);
		foreach ($schemas as $schema) {
			// reset local prefix
			$localprefix = '';
			...
			$this->getFilesystem()
					->copy($schema,
							'config' . DIRECTORY_SEPARATOR . $localprefix
									. basename($schema));
			if ('' === $localprefix) {
	...
	protected function cleanup() {
		if (null === $this->commandApplication
				|| !$this->commandApplication->withTrace()) {
			$finder = sfFinder::type('file')->name('generated-*schema.xml')
					->name('*schema-transformed.xml');
			$this->getFilesystem()
					->remove($finder->in(array('config', 'plugins')));
		}
	}

	protected function callPhing($taskName, $checkSchema, $properties = array()) {
		$schemas = sfFinder::type('file')->name('*schema.xml')->relative()
				->follow_link()->in(sfConfig::get('sf_config_dir'));
		...
		// Call phing targets
		sfToolkit::addIncludePath(
				array(sfConfig::get('sf_symfony_lib_dir'),
						sfConfig::get('sf_propel_generator_path',
								realpath(
										dirname(__FILE__)
												. '/../vendor/propel-generator/classes')),));

		$args = array();
		$bufferPhingOutput = null === $this->commandApplication
				|| !$this->commandApplication->withTrace();

		$properties = array_merge(
				array('build.properties' => 'propel.ini',
		...
		$args[] = realpath(
				dirname(__FILE__) . DIRECTORY_SEPARATOR . '..'
						...
						. 'propel-generator' . DIRECTORY_SEPARATOR
						. 'build.xml');
		...
		$args = $this->dispatcher
				->filter(new sfEvent($this, 'propel.filter_phing_args'), $args)
				->getReturnValue();
		...
		if (sfPhingListener::hasErrors()) {
			...
			foreach (sfPhingListener::getExceptions() as $exception) {
				$messages[] = '';
				$messages[] = preg_replace('/^.*build\-propel\.xml/',
						'build-propel.xml', $exception->getMessage());
				$messages[] = '';
			}
	...
	protected function getPhingPropertiesForConnection($databaseManager,
			$connection) {
		...
		return array(
				...
				'propel.database.password' => $database
						->getParameter('password'),
				'propel.database.encoding' => $database
	...
	protected function getProperties($file) {
		...
		foreach ($lines as $line) {
			...
			$pos = strpos($line, '=');
			$properties[trim(substr($line, 0, $pos))] = trim(
					substr($line, $pos + 1));
		}
