		...
		$this
				->addOptions(
						array(
								...
								new sfCommandOption('xml', null,
										sfCommandOption::PARAMETER_NONE,
										'Creates an XML schema instead of a YML one'),
...
EOF;
	...
	protected function reverseDatabase($databaseManager, $connection, $options) {
		...
		$ret = $this
				->callPhing('reverse', self::DO_NOT_CHECK_SCHEMA, $properties);
		...
		if (file_exists($xmlSchemaPath)) {
			...
			$schema = preg_replace('/<database\s+name="[^"]+"/s',
					'<database name="' . $connection . '" package="lib.model"',
					$schema);
