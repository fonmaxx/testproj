...
EOF;
	...
	protected function execute($arguments = array(), $options = array()) {
		...
		$properties = $this
				->getProperties(sfConfig::get('sf_data_dir') . '/sql/sqldb.map');
		$sqls = array();
		foreach ($properties as $file => $connection) {
			if (null !== $options['connection']
		...
		if (!$options['no-confirmation']
				&& !$this
						->askConfirmation(
								array(
										'WARNING: The data in the database'
												. (count($sqls) > 1 ? 's' : '')
												. ' related to the connection name'
												. (count($sqls) > 1 ? 's' : ''),
										sprintf(
												'         %s will be removed.',
												implode(', ', array_keys($sqls))),
										'',
		...
		$this->tmpDir = sys_get_temp_dir() . '/propel_insert_sql_'
				. rand(11111, 99999);
		...
		foreach ($sqls as $connection => $files) {
			...
			foreach ($files as $file) {
				...
				copy(sfConfig::get('sf_data_dir') . '/sql/' . $file, $dir . '/'
						. $file);
			}

			file_put_contents($dir . '/sqldb.map', $content);
			$properties = $this
					->getPhingPropertiesForConnection($databaseManager,
							$connection);
			$properties['propel.sql.dir'] = $dir;

			$ret = $this
					->callPhing('insert-sql', self::CHECK_SCHEMA, $properties);
		}
		$this->removeTmpDir();
