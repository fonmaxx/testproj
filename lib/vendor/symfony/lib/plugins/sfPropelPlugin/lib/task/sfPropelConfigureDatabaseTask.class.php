...
class sfPropelConfigureDatabaseTask extends sfBaseTask {
	...
	protected function configure() {
		$this
				->addArguments(
						array(
								new sfCommandArgument('dsn',
										sfCommandArgument::REQUIRED,
										'The database dsn'),
								new sfCommandArgument('username',
										sfCommandArgument::OPTIONAL,
										'The database username', 'root'),
								new sfCommandArgument('password',
										sfCommandArgument::OPTIONAL,
				...
				->addOptions(
						array(
								new sfCommandOption('env', null,
										...
										'The environment', 'all'),
								new sfCommandOption('name', null,
										sfCommandOption::PARAMETER_OPTIONAL,
										'The connection name', 'propel'),
								new sfCommandOption('class', null,
										sfCommandOption::PARAMETER_OPTIONAL,
...
EOF;
	...
	protected function execute($arguments = array(), $options = array()) {
		...
		if (null !== $options['app']) {
			$file = sfConfig::get('sf_apps_dir') . '/' . $options['app']
		...
		$config[$options['env']][$options['name']] = array(
				...
				'param' => array_merge(
						isset(
								$config[$options['env']][$options['name']]['param']) ? $config[$options['env']][$options['name']]['param']
								: array(),
						array('dsn' => $arguments['dsn'],
			...
			if (file_exists($propelini)) {
				...
				if (preg_match('/^(.+?):/', $arguments['dsn'], $match)) {
					$content = preg_replace(
							'/^propel\.database(\s*)=(\s*)(.+?)$/m',
							'propel.database$1=${2}' . $match[1], $content);
					$content = preg_replace(
							'/^propel\.database.driver(\s*)=(\s*)(.+?)$/m',
							'propel.database.driver$1=${2}' . $match[1],
							$content);
					$content = preg_replace(
							'/^propel\.database\.createUrl(\s*)=(\s*)(.+?)$/m',
							'propel.database.createUrl$1=${2}'
									. $arguments['dsn'], $content);
					$content = preg_replace(
							'/^propel\.database\.url(\s*)=(\s*)(.+?)$/m',
							'propel.database.url$1=${2}' . $arguments['dsn'],
							$content);

					$content = preg_replace(
							'/^propel\.database\.user(\s*)=(\s*)(.+?)$/m',
							'propel.database.user$1=${2}'
									. $arguments['username'], $content);
					$content = preg_replace(
							'/^propel\.database\.password(\s*)=(\s*)(.+?)$/m',
							'propel.database.password$1=${2}'
									. $arguments['password'], $content);
