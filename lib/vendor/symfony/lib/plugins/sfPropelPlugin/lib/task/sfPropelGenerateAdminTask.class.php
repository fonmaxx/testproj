...
class sfPropelGenerateAdminTask extends sfPropelBaseTask {
	...
	protected function configure() {
		$this
				->addArguments(
						array(
								...
								new sfCommandArgument('route_or_model',
										sfCommandArgument::REQUIRED,
		...
		$this
				->addOptions(
						array(
								new sfCommandOption('module', null,
...
EOF;
	...
	protected function execute($arguments = array(), $options = array()) {
		...
		if (false
				!== ($route = $this
						->getRouteFromName($arguments['route_or_model']))) {
		...
		if (!class_exists($arguments['route_or_model'])) {
			throw new sfCommandException(
					sprintf(
							'The route "%s" does not exist and there is no "%s" class.',
							$arguments['route_or_model'],
							$arguments['route_or_model']));
		}
		...
		if (!$r->isSubclassOf('BaseObject')) {
			throw new sfCommandException(
					sprintf('"%s" is not a Propel class.',
							$arguments['route_or_model']));
		}
		...
		if (isset($options['module'])) {
			...
			if ($route
					&& !$this->checkRoute($route, $model, $options['module'])) {
...
EOF
					, $name, $model, $module,
					isset($options['plural']) ? $options['plural'] : $module,
					$primaryKey) . $content;
			...
			if (false === file_put_contents($routing, $content)) {
				throw new sfCommandException(
						sprintf('Unable to write to file, %s.', $routing));
			}
		}
	...
	protected function generateForRoute($arguments, $options) {
		...
		if (!$arguments['route'] instanceof sfPropelRouteCollection) {
			throw new sfCommandException(
					sprintf(
							'The route "%s" is not a Propel collection route.',
		...
		$task = new sfPropelGenerateModuleTask($this->dispatcher,
				$this->formatter);
		...
		$this
				->logSection('app',
						sprintf('Generating admin module "%s" for model "%s"',
								$module, $model));

		return $task
				->run(array($arguments['application'], $module, $model),
	...
	protected function getRouteFromName($name) {
		...
		$routes = $config
				->evaluate(
						$this->configuration
								->getConfigPaths('config/routing.yml'));
	...
	protected function getPrimaryKey($model) {
		...
		return call_user_func(array($peer, 'translateFieldName'),
				$column->getPhpName(), BasePeer::TYPE_PHPNAME,
