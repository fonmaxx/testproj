		...
		$this
				->addOptions(
						array(
								...
								new sfCommandOption('skip-forms', 'F',
										sfCommandOption::PARAMETER_NONE,
										'Skip generating forms'),
								new sfCommandOption('classes-only', 'C',
										sfCommandOption::PARAMETER_NONE,
										'Do not initialize the database'),
								new sfCommandOption('phing-arg', null,
										sfCommandOption::PARAMETER_REQUIRED
...
EOF;
	...
	protected function execute($arguments = array(), $options = array()) {
		...
		if (!$options['skip-forms']) {
			$this
					->logBlock(
							array(
									...
									'fixed in symfony.',), 'INFO');

			$buildForms = new sfPropelBuildFormsTask($this->dispatcher,
					$this->formatter);
			...
			$buildFilters = new sfPropelBuildFiltersTask($this->dispatcher,
					$this->formatter);
		...
		if (!$options['classes-only']) {
			$buildSql = new sfPropelBuildSqlTask($this->dispatcher,
					$this->formatter);
			...
			$insertSql = new sfPropelInsertSqlTask($this->dispatcher,
					$this->formatter);
