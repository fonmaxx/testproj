...
abstract class sfFormFilterPropel extends sfFormFilter {
	...
	public function processValues($values) {
		...
		foreach ($originalValues as $field => $value) {
			try {
				$method = sprintf('convert%sValue',
						call_user_func(
								array(
										constant($this->getModelName()
												. '::PEER'),
	...
	protected function doBuildCriteria(array $values) {
		...
		foreach ($fields as $field => $type) {
			...
			try {
				$method = sprintf('add%sColumnCriteria',
						call_user_func(array($peer, 'translateFieldName'),
								$field, BasePeer::TYPE_FIELDNAME,
								BasePeer::TYPE_PHPNAME));
			} catch (Exception $e) {
				// not a "real" column
			...
			if (method_exists($this, $method)) {
				...
				if (!method_exists($this,
						$method = sprintf('add%sCriteria', $type))) {
					throw new LogicException(
	...
	protected function addDateCriteria(Criteria $criteria, $field, $values) {
		...
		if (isset($values['is_empty']) && $values['is_empty']) {
			...
			if (null !== $values['from'] && null !== $values['to']) {
				$criterion = $criteria
						->getNewCriterion($colname, $values['from'],
								Criteria::GREATER_EQUAL);
				$criterion
						->addAnd(
								$criteria
										->getNewCriterion($colname,
												$values['to'],
												Criteria::LESS_EQUAL));
				...
				$criterion = $criteria
						->getNewCriterion($colname, $values['to'],
								Criteria::LESS_EQUAL);
			}
	...
	protected function getColName($field) {
		return call_user_func(
				array(constant($this->getModelName() . '::PEER'),
						'translateFieldName'), $field,
