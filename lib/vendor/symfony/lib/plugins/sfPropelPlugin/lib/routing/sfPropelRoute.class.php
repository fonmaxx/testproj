...
class sfPropelRoute extends sfObjectRoute {
	...
	protected function getObjectForParameters($parameters) {
		...
		if (!isset($this->options['method'])) {
			$this->options['method'] = isset(
					$this->options['method_for_criteria']) ? $this
							->options['method_for_criteria'] : 'doSelectOne';
			...
			foreach ($variables as $variable) {
				try {
					$constant = call_user_func(
							array($className, 'translateFieldName'), $variable,
	...
	protected function getObjectsForParameters($parameters) {
		...
		if (!isset($this->options['method'])) {
			$this->options['method'] = isset(
					$this->options['method_for_criteria']) ? $this
							->options['method_for_criteria'] : 'doSelect';
	...
	protected function doConvertObjectToArray($object) {
		...
		foreach ($this->getRealVariables() as $variable) {
			try {
				$method = 'get'
						. call_user_func(
								array($className, 'translateFieldName'),
								$variable, BasePeer::TYPE_FIELDNAME,
	...
	protected function fixOptions() {
		if (!isset($this->options['object_model'])) {
			...
			$this->options['model'] = constant($this->options['model']
					. '::PEER');
		}
	}
}
