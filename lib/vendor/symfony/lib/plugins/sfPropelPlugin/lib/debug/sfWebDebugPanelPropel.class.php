...
class sfWebDebugPanelPropel extends sfWebDebugPanel {
	...
	public function getTitle() {
		if ($sqlLogs = $this->getSqlLogs()) {
			return '<img src="' . $this->webDebug->getOption('image_root_path')
					. '/database.png" alt="SQL queries" /> ' . count($sqlLogs);
		}
	}
	...
	protected function getSqlLogs() {
		...
		$flagSlow = $config
				->getParameter('debugpdo.logging.details.slow.enabled', false);
		$threshold = $config
				->getParameter('debugpdo.logging.details.slow.threshold',
						DebugPDO::DEFAULT_SLOW_THRESHOLD);
        ...
        </li>', $slowQuery ? ' class="sfWebDebugWarning"' : '', $query,
					implode(', ', $details), $backtrace);
		}
