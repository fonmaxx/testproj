		...
		$this
				->addOptions(
						array(
								...
								new sfCommandOption('with-doctrine-route',
										null, sfCommandOption::PARAMETER_NONE,
										'Whether you will use a Doctrine route'),
...
EOF;
		...
		$generatorManager
				->generate('sfDoctrineGenerator',
						array('model_class' => $arguments['model'],
								...
								'singular' => $options['singular'] ? $options['singular']
										: sfInflector::underscore(
												$arguments['model']),
								'plural' => $options['plural'] ? $options['plural']
										: sfInflector::underscore(
												$arguments['model'] . 's'),
	...
	protected function executeInit($arguments = array(), $options = array()) {
		...
		$dirs = $this->configuration
				->getGeneratorSkeletonDirs('sfDoctrineModule',
						$options['theme']);
