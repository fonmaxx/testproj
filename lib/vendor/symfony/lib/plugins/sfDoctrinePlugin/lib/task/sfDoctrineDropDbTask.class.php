...
class sfDoctrineDropDbTask extends sfDoctrineBaseTask {
	...
	protected function configure() {
		$this
				->addArguments(
						array(
								new sfCommandArgument('database',
...
EOF;
	...
	protected function execute($arguments = array(), $options = array()) {
		...
		$databases = $this
				->getDoctrineDatabases($databaseManager,
						count($arguments['database']) ? $arguments['database']
								: null);

		$environment = $this->configuration instanceof sfApplicationConfiguration ? $this
						->configuration->getEnvironment() : 'all';
				...
				&& !$this
						->askConfirmation(
								array_merge(
										array(
												sprintf(
														'This command will remove all data in the following "%s" connection(s):',
														$environment), ''),
										array_map(
												create_function('$v',
														'return \' - \'.$v;'),
		...
		foreach ($databases as $name => $database) {
			$this
					->logSection('doctrine',
							sprintf('Dropping "%s" database', $name));
			try {
