...
class sfDoctrineCreateModelTables extends sfDoctrineBaseTask {
	protected function configure() {
		$this
				->addArguments(
						array(
								new sfCommandArgument('models',
										sfCommandArgument::IS_ARRAY,
										'The list of models', array()),));
...
EOF;
	...
	protected function execute($arguments = array(), $options = array()) {
		...
		$buildModel = new sfDoctrineBuildModelTask($this->dispatcher,
				$this->formatter);
		...
		foreach ($connections as $name => $models) {
			$this
					->logSection('doctrine',
							'dropping model tables for connection "' . $name
									. '"');
			...
			foreach ($models as $model) {
				$tableName = Doctrine_Core::getTable($model)
						->getOption('tableName');
