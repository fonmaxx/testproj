...
abstract class sfDoctrineBaseTask extends sfBaseTask {
	...
	public function getCliConfig() {
		return $this->configuration->getPluginConfiguration('sfDoctrinePlugin')
				->getCliConfig();
	}
	 ...
	 */
	protected function getDoctrineDatabases(sfDatabaseManager $databaseManager,
			array $names = null) {
		$databases = array();
			...
			foreach ($names as $name) {
				...
				if (!$database instanceof sfDoctrineDatabase) {
					throw new InvalidArgumentException(
							sprintf(
									'The database "%s" is not a Doctrine database.',
									$name));
				}
	...
	protected function prepareSchemaFile($yamlSchemaPath) {
		...
		foreach ($this->configuration->getPlugins() as $name) {
			...
			foreach ($finder->in($plugin->getRootDir() . '/config/doctrine') as $schema) {
				...
				foreach ($pluginModels as $model => $definition) {
					...
					$models[$model] = isset($models[$model]) ? sfToolkit::arrayDeepMerge(
									$models[$model], $definition) : $definition;

					// the first plugin to define this model gets the package
					if (!isset($models[$model]['package'])) {
						$models[$model]['package'] = $plugin->getName()
								. '.lib.model.doctrine';
					}

					if (!isset($models[$model]['package_custom_path'])
							&& 0
									=== strpos($models[$model]['package'],
											$plugin->getName())) {
						$models[$model]['package_custom_path'] = $plugin
		...
		$file = realpath(sys_get_temp_dir()) . '/doctrine_schema_'
				. rand(11111, 99999) . '.yml';
