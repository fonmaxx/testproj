				...
				->addArguments(
						array(
								new sfCommandArgument('name',
										...
										'The name of the model you wish to delete all related files for.'),));
		...
		$this
				->addOptions(
						array(
								...
								new sfCommandOption('prefix', null,
										sfCommandOption::PARAMETER_REQUIRED
												| sfCommandOption::IS_ARRAY,
										'Class prefix to remove'),
								new sfCommandOption('suffix', null,
										sfCommandOption::PARAMETER_REQUIRED
												| sfCommandOption::IS_ARRAY,
										'Class suffix to remove'),
								new sfCommandOption('extension', null,
										sfCommandOption::PARAMETER_REQUIRED
												| sfCommandOption::IS_ARRAY,
...
EOF;
	...
	protected function execute($arguments = array(), $options = array()) {
		$paths = array_merge(
				array(sfConfig::get('sf_lib_dir') . '/model/doctrine',
		...
		$suffixPattern = $this
				->valuesToRegex(
						$options['suffix'] ? $options['suffix']
								: array('', 'Table', 'Form', 'FormFilter'));
		$extensionPattern = $this
				->valuesToRegex(
		...
		foreach ($arguments['name'] as $modelName) {
			$finder = sfFinder::type('file')
					->name(
							'/^' . $prefixPattern . $modelName . $suffixPattern
									. $extensionPattern . '$/');
			...
			if ($files) {
				...
				$this
						->logSection('doctrine',
								'No files found for the model named "'
										. $modelName . '"');
			}
		}

		$this->logSection('doctrine', 'Deleted a total of ' . $total
						. ' file(s)');
	}
