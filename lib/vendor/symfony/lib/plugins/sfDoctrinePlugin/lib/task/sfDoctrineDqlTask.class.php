...
class sfDoctrineDqlTask extends sfDoctrineBaseTask {
	...
	protected function configure() {
		$this
				->addArguments(
						array(
								new sfCommandArgument('dql_query',
										sfCommandArgument::REQUIRED,
										'The DQL query to execute', null),
								new sfCommandArgument('parameter',
				...
				->addOptions(
						array(
								...
								new sfCommandOption('env', null,
										...
										'The environment', 'dev'),
								new sfCommandOption('show-sql', null,
										sfCommandOption::PARAMETER_NONE,
										'Show the sql that would be executed'),
								new sfCommandOption('table', null,
										sfCommandOption::PARAMETER_NONE,
...
EOF;
	...
	protected function execute($arguments = array(), $options = array()) {
		...
		$q = Doctrine_Query::create()->parseDqlQuery($dql);
		...
		if ($options['show-sql']) {
			$this
					->log(
							sprintf('SQL: %s',
									$q->getSqlQuery($arguments['parameter'])));
		}
		...
		if ($count) {
			if (!$options['table']) {
				...
				$this
						->log(
								array(
										sprintf('found %s results',
												number_format($count)),
										sfYaml::dump($results, 4),));
			} else {
				$results = $q
						->execute($arguments['parameter'],
				...
				foreach ($results as $result) {
					foreach ($result as $field => $value) {
						...
						$headers[$field] = max($headers[$field],
								strlen($this->renderValue($value)));
					}
				}
				...
				foreach ($results as $result) {
					...
					foreach ($result as $field => $value) {
						$line .= ' '
								. str_pad($this->renderValue($value),
										$headers[$field]) . ' |';
					}
					$this->log($line);
				}
				...
				// find profiler
				if ($profiler = $q->getConnection()->getListener()
						->get('symfony_profiler')) {
					$events = $profiler->getQueryExecutionEvents();
					$event = array_pop($events);
					$this
							->log(
									sprintf('%s results (%s sec)',
											number_format($count),
											number_format(
