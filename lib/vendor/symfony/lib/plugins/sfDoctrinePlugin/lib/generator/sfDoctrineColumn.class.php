...
class sfDoctrineColumn implements ArrayAccess {
	 ...
	 */
	static $doctrineToSymfony = array('boolean' => 'BOOLEAN',
			'string' => 'LONGVARCHAR', 'integer' => 'INTEGER',
			'date' => 'DATE', 'timestamp' => 'TIMESTAMP',
			'time' => 'TIME', 'enum' => 'LONGVARCHAR', 'float' => 'FLOAT',
			'double' => 'DOUBLE', 'clob' => 'CLOB', 'blob' => 'BLOB',
			'object' => 'LONGVARCHAR', 'array' => 'LONGVARCHAR',
			'decimal' => 'DECIMAL',);
	...
	public function getDoctrineType() {
		return isset($this->definition['type']) ? $this->definition['type']
				: null;
	}
	...
	public function getType() {
		...
		if ('string' == $doctrineType && null !== $this->getSize()
				&& $this->getSize() <= 255) {
			return 'VARCHAR';
		}

		return $doctrineType ? self::$doctrineToSymfony[$doctrineType]
				: 'VARCHAR';
	}
