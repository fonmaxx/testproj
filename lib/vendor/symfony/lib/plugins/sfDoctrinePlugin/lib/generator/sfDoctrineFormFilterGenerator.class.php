...
class sfDoctrineFormFilterGenerator extends sfDoctrineFormGenerator {
	...
	public function generate($params = array()) {
		...
		$file = sfConfig::get('sf_lib_dir')
				. '/filter/doctrine/BaseFormFilterDoctrine.class.php';
		if (!file_exists($file)) {
			...
			file_put_contents($file,
					$this->evalTemplate('sfDoctrineFormFilterBaseTemplate.php'));
		}
		...
		foreach ($models as $model) {
			...
			file_put_contents(
					$baseDir . '/base/Base' . $model . 'FormFilter.class.php',
					$this
							->evalTemplate(
									null === $this->getParentModel() ? 'sfDoctrineFormFilterGeneratedTemplate.php'
											: 'sfDoctrineFormFilterGeneratedInheritanceTemplate.php'));
			...
			if ($isPluginModel) {
				...
				if (!file_exists(
					...
					file_put_contents($classFile,
							$this
									->evalTemplate(
											'sfDoctrineFormFilterPluginTemplate.php'));
				}
			}
			if (!file_exists(
					$classFile = $baseDir . '/' . $model
							. 'FormFilter.class.php')) {
				if ($isPluginModel) {
					file_put_contents($classFile,
							$this
									->evalTemplate(
											'sfDoctrinePluginFormFilterTemplate.php'));
				} else {
					file_put_contents($classFile,
							$this
									->evalTemplate(
											'sfDoctrineFormFilterTemplate.php'));
				}
			}
		}
	}
	...
	public function getWidgetOptionsForColumn($column) {
		...
		if ($column->isForeignKey()) {
			$options[] = sprintf(
					'\'model\' => $this->getRelatedModelName(\'%s\'), \'add_empty\' => true',
	...
	public function getValidatorOptionsForColumn($column) {
		...
		if ($column->isForeignKey()) {
			...
			$options[] = sprintf(
					'\'model\' => $this->getRelatedModelName(\'%s\'), \'column\' => \'%s\'',
					$column->getRelationKey('alias'),
					$column->getForeignTable()->getFieldName($name));
		} else if ($column->isPrimaryKey()) {
			$options[] = sprintf('\'model\' => \'%s\', \'column\' => \'%s\'',
					$this->table->getOption('name'), $column->getFieldName());
		} else {
			switch ($column->getDoctrineType()) {
			case 'boolean':
	...
	public function getValidatorForColumn($column) {
		...
		if (in_array($class = $this->getValidatorClassForColumn($column),
				array('sfValidatorInteger', 'sfValidatorNumber'))) {
			$format = 'new sfValidatorSchemaFilter(\'text\', new %s(%s))';
		}

		return sprintf($format, $class,
				$this->getValidatorOptionsForColumn($column));
	}
	...
	public function getFormClassToExtend() {
		return null === ($model = $this->getParentModel()) ? 'BaseFormFilterDoctrine'
				: sprintf('%sFormFilter', $model);
	}
}
