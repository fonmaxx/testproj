...
class sfDoctrineConnectionProfiler extends Doctrine_Connection_Profiler {
	protected $dispatcher = null, $options = array();
	...
	public function __construct(sfEventDispatcher $dispatcher,
		...
		$this->options = array_merge(
				array('logging' => false, 'slow_query_threshold' => 1,),
				$options);
	}
	...
	public function getQueryExecutionEvents() {
		...
		foreach ($this as $event) {
			if (in_array($event->getCode(),
					array(Doctrine_Event::CONN_QUERY,
	...
	static public function fixParams($params) {
		foreach ($params as $key => $param) {
			if (strlen($param) >= 255) {
				$params[$key] = '[' . number_format(strlen($param) / 1024, 2)
						. 'Kb]';
			}
		}
