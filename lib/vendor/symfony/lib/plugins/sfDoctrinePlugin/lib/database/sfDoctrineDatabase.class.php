...
class sfDoctrineDatabase extends sfDatabase {
	...
	public function initialize($parameters = array()) {
		...
		if (!strpos($dsn, '://')) {
			$dsn = array($dsn, $this->getParameter('username'),
		...
		foreach ($attributes as $name => $value) {
			...
			if (is_string($value)) {
				$valueConstantName = 'Doctrine_Core::'
						. strtoupper($stringName) . '_' . strtoupper($value);
				$value = defined($valueConstantName) ? constant(
								$valueConstantName) : $value;
			}
		...
		$eventListener = new sfDoctrineConnectionListener(
				$this->_doctrineConnection, $encoding);
		...
		if ($this->getParameter('profiler', sfConfig::get('sf_debug'))) {
			$this->profiler = new sfDoctrineConnectionProfiler($dispatcher,
					array(
							'logging' => $this
									->getParameter('logging',
											sfConfig::get('sf_logging_enabled')),));
			$this->_doctrineConnection
					->addListener($this->profiler, 'symfony_profiler');
		}
		...
		$method = sprintf('configureDoctrineConnection%s',
				ucwords($this->_doctrineConnection->getName()));

		if (method_exists($configuration, 'configureDoctrineConnection')
				&& !method_exists($configuration, $method)) {
			$configuration
					->configureDoctrineConnection($this->_doctrineConnection);
		}
		...
		$dispatcher
				->notify(
						new sfEvent($manager, 'doctrine.configure_connection',
								array(
										'connection' => $this
												->_doctrineConnection,
										'database' => $this)));
	...
	public function shutdown() {
		...
		if ($this->_doctrineConnection !== null) {
			$this->_doctrineConnection->getManager()
					->closeConnection($this->_doctrineConnection);
		}
	}
}
