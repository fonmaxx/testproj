...
abstract class sfFormDoctrine extends sfFormObject {
		...
		if (!$object) {
			...
			if (!$object instanceof $class) {
				throw new sfException(
						sprintf('The "%s" form only accepts a "%s" object.',
	...
	public function getConnection() {
		return Doctrine_Manager::getInstance()
				->getConnectionForComponent($this->getModelName());
	}
	...
	public function embedI18n($cultures, $decorator = null) {
		if (!$this->isI18n()) {
			throw new sfException(
					sprintf('The model "%s" is not internationalized.',
							$this->getModelName()));
		}
		...
		foreach ($cultures as $culture) {
			...
			if (false === $i18nObject->exists()) {
				unset($i18n[$this->getI18nModelPrimaryKeyName()],
						$i18n[$this->getI18nModelI18nField()]);
			}
	...
	public function embedRelation($relationName, $formClass = null,
		...
		$r = new ReflectionClass(
				null === $formClass ? $relation->getClass() . 'Form' : $formClass);
		...
		if (Doctrine_Relation::ONE == $relation->getType()) {
			...
			foreach ($this->getObject()->$relationName as $index => $childObject) {
				$form = $r
						->newInstanceArgs(
								array_merge(array($childObject), $formArgs));

				$subForm->embedForm($index, $form, $innerDecorator);
				$subForm->getWidgetSchema()
						->setLabel($index, (string) $childObject);
			}
		...
		foreach ($valuesToProcess as $field => $value) {
			...
			if (method_exists($this, $method)) {
				...
				if ($this->validatorSchema[$field] instanceof sfValidatorFile) {
					$values[$field] = $this
							->processUploadedFile($field, null,
									$valuesToProcess);
				}
			}
		}
	...
	public function isI18n() {
		return $this->getObject()->getTable()
				->hasTemplate('Doctrine_Template_I18n');
	}
	...
	public function getI18nModelName() {
		return $this->getObject()->getTable()
				->getTemplate('Doctrine_Template_I18n')->getI18n()
				->getOption('className');
	}
	...
	public function getI18nModelPrimaryKeyName() {
		...
		if (is_array($primaryKey)) {
			throw new sfException(
					sprintf(
							'The model "%s" has composite primary keys and cannot be used with i18n..',
							$this->getModelName()));
		}
	...
	protected function processUploadedFile($field, $filename = null,
		...
		if (!$this->validatorSchema[$field] instanceof sfValidatorFile) {
			throw new LogicException(
					sprintf(
							'You cannot save the current file for field "%s" as the field is not a file.',
							$field));
		}
	...
	protected function removeFile($field) {
		if (!$this->validatorSchema[$field] instanceof sfValidatorFile) {
			throw new LogicException(
					sprintf(
							'You cannot remove the current file for field "%s" as the field is not a file.',
							$field));
		}
	...
	protected function saveFile($field, $filename = null,
		...
		} else if (method_exists($this->getObject(),
	...
	protected function getRelatedModelName($alias) {
		...
		if (!$table->hasRelation($alias)) {
			throw new InvalidArgumentException(
					sprintf('The "%s" model has no "%s" relation.',
							$this->getModelName(), $alias));
		}
