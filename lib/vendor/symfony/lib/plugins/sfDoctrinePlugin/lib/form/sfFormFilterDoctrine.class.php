...
abstract class sfFormFilterDoctrine extends sfFormFilter {
	...
	protected function doBuildQuery(array $values) {
		$query = isset($this->options['query']) ? clone $this->options['query']
		...
		$names = array_merge($fields,
				array_diff(array_keys($this->validatorSchema->getFields()),
						array_keys($fields)));
		$fields = array_merge($fields,
				array_combine($names, array_fill(0, count($names), null)));
		...
		foreach ($fields as $field => $type) {
			...
			if ($this->getTable()->hasField($field)) {
				$method = sprintf('add%sColumnQuery',
						self::camelize($this->getFieldName($field)));
			} else if (!method_exists($this,
					$method = sprintf('add%sColumnQuery',
							self::camelize($field))) && null !== $type) {
				throw new LogicException(
						sprintf(
								'You must define a "%s" method to be able to filter with the "%s" field.',
								$method, $field));
			}
			...
			if (method_exists($this, $method)) {
				...
				if (!method_exists($this,
						$method = sprintf('add%sQuery', $type))) {
					throw new LogicException(
							sprintf('Unable to filter for the "%s" type.',
									$type));
				}
	...
	protected function addForeignKeyQuery(Doctrine_Query $query, $field, $value) {
		...
		if (is_array($value)) {
			...
			$query
					->addWhere(
							sprintf('%s.%s = ?', $query->getRootAlias(),
									$fieldName), $value);
		}
	}
	...
	protected function addTextQuery(Doctrine_Query $query, $field, $values) {
		...
		if (is_array($values) && isset($values['is_empty'])
			...
			$query
					->addWhere(
							sprintf('(%s.%s IS NULL OR %1$s.%2$s = ?)',
			...
			$query
					->addWhere(
							sprintf('%s.%s LIKE ?', $query->getRootAlias(),
	...
	protected function addDateQuery(Doctrine_Query $query, $field, $values) {
		...
		if (isset($values['is_empty']) && $values['is_empty']) {
			$query
					->addWhere(
							sprintf('%s.%s IS NULL', $query->getRootAlias(),
									$fieldName));
		} else {
