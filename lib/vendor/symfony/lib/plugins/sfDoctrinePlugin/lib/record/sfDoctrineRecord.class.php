...
abstract class sfDoctrineRecord extends Doctrine_Record {
	static protected $_defaultCulture = 'en';
	...
	public function __toString() {
		$guesses = array('name', 'title', 'description', 'subject', 'keywords',
				'id');
			...
			if (in_array($verb = substr($method, 0, 3), array('set', 'get'))) {
				...
				if ($table->hasRelation($name)) {
					$entityName = $name;
						...
						->hasField($fieldName = $table->getFieldName($name))) {
					...
					$underScored = $table
							->getFieldName(sfInflector::underscore($name));
					if ($table->hasField($underScored)
							|| $table->hasRelation($underScored)) {
						$entityName = $underScored;
					} else if ($table->hasField(strtolower($name))
							|| $table->hasRelation(strtolower($name))) {
						$entityName = strtolower($name);
					} else {
						$camelCase = $table
								->getFieldName(sfInflector::camelize($name));
						$camelCase = strtolower($camelCase[0])
								. substr($camelCase, 1, strlen($camelCase));
						if ($table->hasField($camelCase)
								|| $table->hasRelation($camelCase)) {
				...
				return call_user_func_array(array($this, $verb),
	...
	public function setDateTimeObject($dateFieldName, DateTime $dateTimeObject) {
		...
		if ($type == 'date' || $type == 'timestamp' || $type == 'datetime') {
			...
			throw new sfException(
					'Cannot call setDateTimeObject() on a field that is not of type date or timestamp.');
		}
	}
}
