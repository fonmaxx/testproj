...
class sfWebDebugPanelDoctrine extends sfWebDebugPanel {
	...
	public function getTitle() {
		if ($events = $this->getDoctrineEvents()) {
			return '<img src="' . $this->webDebug->getOption('image_root_path')
					. '/database.png" alt="SQL queries" /> ' . count($events);
		}
	}
	...
	public function getPanelContent() {
      ...
      <div id="sfWebDebugDatabaseLogs">
        <h3>Doctrine Version: ' . Doctrine_Core::VERSION
				. '</h3>
        <ol>' . implode("\n", $this->getSqlLogs())
				. '</ol>
      </div>
	...
	protected function getSqlLogs() {
		...
		foreach ($this->getDoctrineEvents() as $i => $event) {
			...
			$params = sfDoctrineConnectionProfiler::fixParams(
					$event->getParams());
			$query = $this
					->formatSql(
							htmlspecialchars($event->getQuery(), ENT_QUOTES,
			...
			foreach ($params as $param) {
				...
				$query = join(
						var_export(
								is_scalar($param) ? $param : (string) $param,
        ...
        </li>', $event->slowQuery ? ' class="sfWebDebugWarning"' : '', $query,
					number_format($event->getElapsedSecs(), 2),
					$conn->getName(), $backtrace);
		}
