			...
			if (preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+?))?\s*$#u',
				...
				if (!isset($values['value'])
					...
					if (isset($values['leadspaces'])
							...
							&& preg_match(
									'#^(?P<key>'
											. sfYamlInline::REGEX_QUOTED_STRING
											. '|[^ \'"\{].*?) *\:(\s+(?P<value>.+?))?\s*$#u',
									$values['value'], $matches)) {
					...
					'#^(?P<key>' . sfYamlInline::REGEX_QUOTED_STRING
							. '|[^ \'"].*?) *\:(\s+(?P<value>.+?))?\s*$#u',
					$this->currentLine, $values)) {
				...
				// hash
 else if (!isset($values['value']) || '' == trim($values['value'], ' ')
						|| 0 === strpos(ltrim($values['value'], ' '), '#')) {
		...
		while ($this->currentLineNb + 1 < count($this->lines)) {
			...
			if (preg_match(
				...
				$text .= str_repeat(' ',
						...
						. ($diff ? "\n" : $separator);
			} else if (preg_match('#^(?P<text> *)$#', $this->currentLine,
					$matches)) {
