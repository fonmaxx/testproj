...
$pager->init();
$t
		->isa_ok($pager->getResults(), 'array',
				'->getResults() uses the hydration mode set on the query');
...
$t
		->is($pager->getNbResults(), $count,
				'->getNbResults() returns the number of results');
...
$pager->init();
$t
		->is(count($pager), $pager->getNbResults(),
				'"Countable" interface returns the total number of objects');
...
$t
		->is($iterated, $normal,
				'"Iterator" interface loops over objects in the current pager');
...
$pager->init();
$t
		->is($pager->getNbResults(),
				count(
						Doctrine_Core::getTable('Article')
								->findByIsOnHomepage('1')),
...
$t
		->is($pager->getNbResults(),
				count(
						Doctrine_Core::getTable('Article')
								->findByIsOnHomepage('1')),
				'serialization preserves TableMethod functionality');
