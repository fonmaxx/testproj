...
$t
		->is(
				count(
						array_filter($table->getFilters(),
								'is_symfony_i18n_filter')), 1,
				'->__construct() adds i18n filter');
new Article();
$t
		->is(
				count(
						array_filter($table->getFilters(),
								'is_symfony_i18n_filter')), 1,
				'->__construct() adds i18n filter once');
...
$t
		->is($after->name, 'test',
				'->unserialize() maintains field values upon reset');
...
$t
		->is($after->title, 'test',
				'->unserialize() maintains field values on I18n records');
...
$t
		->is($after->title, 'test',
				'->unserialize() maintains field values on I18n records upon reset');
...
try {
	...
	$t
			->is($e->getMessage(),
					...
					'Making sure proper exception message is thrown');
}
...
$t
		->is($article->getTitle(), 'testing this out',
				'Making sure getTitle() is still accessible after unserializing');
