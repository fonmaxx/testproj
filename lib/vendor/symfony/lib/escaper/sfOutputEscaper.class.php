...
abstract class sfOutputEscaper {
	...
	public static function escape($escapingMethod, $value) {
		...
		if (is_object($value)) {
			if ($value instanceof sfOutputEscaper) {
				...
				return new sfOutputEscaperIteratorDecorator($escapingMethod,
						$value);
			} else {
				return new sfOutputEscaperObjectDecorator($escapingMethod,
						$value);
			}
		}
		...
		throw new InvalidArgumentException(
				sprintf('Unable to escape value "%s".',
	...
	static public function unescape($value) {
		...
		if (is_scalar($value)) {
			...
			return $value instanceof sfOutputEscaper ? $value->getRawValue()
					: $value;
		}
	...
	static public function markClassesAsSafe(array $classes) {
		self::$safeClasses = array_unique(
				array_merge(self::$safeClasses, $classes));
	}
