...
class sfXCacheCache extends sfCache {
	...
	public function initialize($options = array()) {
		...
		if (!function_exists('xcache_set')) {
			throw new sfInitializationException(
					'You must have XCache installed and enabled to use sfXCacheCache class.');
		}

		if (!ini_get('xcache.var_size')) {
			throw new sfInitializationException(
					'You must set the "xcache.var_size" variable to a value greater than 0 to use sfXCacheCache class.');
		}
	}
	...
	public function set($key, $data, $lifetime = null) {
		...
		$set = array('timeout' => time() + $lifetime, 'data' => $data,
				'ctime' => time());
	...
	public function getBaseValue($key) {
		return xcache_isset($this->getOption('prefix') . $key) ? xcache_get(
						$this->getOption('prefix') . $key) : null;
	}
	...
	protected function checkAuth() {
		if (ini_get('xcache.admin.enable_auth')) {
			throw new sfConfigurationException(
					'To use all features of the "sfXCacheCache" class, you must set "xcache.admin.enable_auth" to "Off" in your php.ini.');
		}
	}
}
