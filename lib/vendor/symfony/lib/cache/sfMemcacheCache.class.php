...
class sfMemcacheCache extends sfCache {
	...
	public function initialize($options = array()) {
		...
		if ($this->getOption('memcache')) {
			...
			if ($this->getOption('servers')) {
				...
				$method = $this->getOption('persistent', true) ? 'pconnect'
						: 'connect';
				if (!$this->memcache
						->$method($this->getOption('host', 'localhost'),
	...
	public function has($key) {
		return !(false
				=== $this->memcache->get($this->getOption('prefix') . $key));
	}
	...
	public function set($key, $data, $lifetime = null) {
		$lifetime = null === $lifetime ? $this->getOption('lifetime')
				: $lifetime;
		...
		if (false
				!== $this->memcache
						->replace($this->getOption('prefix') . $key, $data,
								false, time() + $lifetime)) {
			return true;
		}

		return $this->memcache
				->set($this->getOption('prefix') . $key, $data, false,
						time() + $lifetime);
	}
	...
	public function removePattern($pattern) {
		if (!$this->getOption('storeCacheInfo', false)) {
			throw new sfCacheException(
					'To use the "removePattern" method, you must set the "storeCacheInfo" option to "true".');
		}
	...
	protected function setCacheInfo($key, $delete = false) {
		...
		if ($delete) {
			if (($k = array_search($this->getOption('prefix') . $key, $keys))
					!== false) {
				unset($keys[$k]);
			}
		} else {
