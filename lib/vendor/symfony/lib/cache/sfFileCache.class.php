...
class sfFileCache extends sfCache {
	...
	public function initialize($options = array()) {
		...
		if (!$this->getOption('cache_dir')) {
			throw new sfInitializationException(
					'You must pass a "cache_dir" option to initialize a sfFileCache object.');
		}
	...
	public function set($key, $data, $lifetime = null) {
		...
		return $this
				->write($this->getFilePath($key), $data,
						time() + $this->getLifetime($lifetime));
	}
		...
		if (false !== strpos($pattern, '**')) {
			...
			foreach (new RecursiveIteratorIterator(
					new RecursiveDirectoryIterator(
							$this->getOption('cache_dir'))) as $path) {
	...
	public function clean($mode = sfCache::ALL) {
		...
		$result = true;
		foreach (new RecursiveIteratorIterator(
				new RecursiveDirectoryIterator($this->getOption('cache_dir'))) as $file) {
	...
	public function getTimeout($key) {
		...
		return $data[self::READ_TIMEOUT] < time() ? 0
				: $data[self::READ_TIMEOUT];
	}
	...
	public function getLastModified($key) {
		...
		$data = $this
				->read($path, self::READ_TIMEOUT | self::READ_LAST_MODIFIED);
	...
	protected function getFilePath($key) {
		return $this->getOption('cache_dir') . DIRECTORY_SEPARATOR
				. str_replace(sfCache::SEPARATOR, DIRECTORY_SEPARATOR, $key)
				. self::EXTENSION;
	}
	...
	protected function read($path, $type = self::READ_DATA) {
		if (!$fp = @fopen($path, 'rb')) {
			throw new sfCacheException(
					sprintf('Unable to read cache file "%s".', $path));
		}
		...
		if ($type != self::READ_TIMEOUT && time() < $data[self::READ_TIMEOUT]) {
			if ($type & self::READ_LAST_MODIFIED) {
				$data[self::READ_LAST_MODIFIED] = intval(
						@stream_get_contents($fp, 12, 12));
			}
	...
	protected function write($path, $data, $timeout) {
		...
		if (!$fp = @fopen($tmpFile, 'wb')) {
			throw new sfCacheException(
					sprintf('Unable to write cache file "%s".', $tmpFile));
		}
