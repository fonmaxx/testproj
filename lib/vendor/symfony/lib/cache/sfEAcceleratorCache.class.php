...
class sfEAcceleratorCache extends sfCache {
	...
	public function initialize($options = array()) {
		...
		if (!function_exists('eaccelerator_put')
			...
			throw new sfInitializationException(
					'You must have EAccelerator installed and enabled to use sfEAcceleratorCache class (or perhaps you forgot to add --with-eaccelerator-shared-memory when installing).');
		}
	}
	...
	public function set($key, $data, $lifetime = null) {
		return eaccelerator_put($this->getOption('prefix') . $key, $data,
	...
	public function removePattern($pattern) {
		...
		if (is_array($infos)) {
			$regexp = self::patternToRegexp($this->getOption('prefix')
					. $pattern);
		...
		if (is_array($infos)) {
			foreach ($infos as $info) {
				if (false !== strpos($info['name'], $this->getOption('prefix'))) {
					...
					$key = 0 === strpos($info['name'], ':') ? substr(
									$info['name'], 1) : $info['name'];
