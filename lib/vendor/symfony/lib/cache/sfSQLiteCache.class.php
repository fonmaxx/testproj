...
class sfSQLiteCache extends sfCache {
	...
	public function initialize($options = array()) {
		if (!extension_loaded('SQLite') && !extension_loaded('pdo_SQLite')) {
			throw new sfConfigurationException(
					'sfSQLiteCache class needs "sqlite" or "pdo_sqlite" extension to be loaded.');
		}
		...
		if (!$this->getOption('database')) {
			throw new sfInitializationException(
					'You must pass a "database" option to initialize a sfSQLiteCache object.');
		}
	...
	public function has($key) {
		return (boolean) $this->dbh
				->query(
						sprintf(
								"SELECT key FROM cache WHERE key = '%s' AND timeout > %d",
								sqlite_escape_string($key), time()))->numRows();
	}
	...
	public function set($key, $data, $lifetime = null) {
		...
		return (boolean) $this->dbh
				->query(
						sprintf(
								"INSERT OR REPLACE INTO cache (key, data, timeout, last_modified) VALUES ('%s', '%s', %d, %d)",
								sqlite_escape_string($key),
								sqlite_escape_string($data),
								time() + $this->getLifetime($lifetime), time()));
	}
	...
	public function remove($key) {
		return (boolean) $this->dbh
				->query(
						sprintf("DELETE FROM cache WHERE key = '%s'",
								sqlite_escape_string($key)));
	}
	...
	public function removePattern($pattern) {
		return (boolean) $this->dbh
				->query(
						sprintf("DELETE FROM cache WHERE REGEXP('%s', key)",
								sqlite_escape_string(
										self::patternToRegexp($pattern))));
	}
	...
	protected function setDatabase($database) {
		...
		if (!$this->dbh = new SQLiteDatabase($this->database, 0644, $errmsg)) {
			throw new sfCacheException(
					sprintf('Unable to connect to SQLite database: %s.',
							$errmsg));
		}

		$this->dbh
				->createFunction('regexp',
						array($this, 'removePatternRegexpCallback'), 2);
		...
		$rows = $this->dbh
				->arrayQuery(
						sprintf(
								...
								implode('\', \'',
										array_map('sqlite_escape_string', $keys)),
								time()));
	...
	protected function createSchema() {
		...
		foreach ($statements as $statement) {
			if (!$this->dbh->query($statement)) {
				throw new sfCacheException(
						sqlite_error_string($this->dbh->lastError()));
			}
		}
	}
}
